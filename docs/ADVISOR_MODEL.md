...
Run `node --test tools/ui/check_advisor_model.cjs tools/ui/check_guidance_outcomes.cjs`; the second file checks `recognize` against native field shapes. Fixtures exercise missing/malformed values, construction eligibility and exact sizes, stale dates, calendar versus funding research waits, disabled government warnings, foreign wars, incoming-offer identity/deadlines, immutable deterministic results and browser UMD loading without browser globals. The archived `/api/state` regression now always reads the checked-in [advisor fixture](../tools/ui/fixtures/advisor-archive.json), with its [extraction and provenance](../tools/ui/fixtures/ADVISOR_ARCHIVE.md). Its absence fails the test; it no longer silently skips on clean CI checkouts.
