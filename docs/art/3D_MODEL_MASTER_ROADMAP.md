...
Town-gallery scenes now enforce the unchanged 150k visible-triangle limit through the actual camera draw list: per-lot screen-size detail, frustum culling and bounded caches, with a selected building kept at close detail. The original full-close blocks remain available and retain their raw over-budget diagnostics. P0 measures the renderer's own camera planner; separate browser checks compare it with actual WebGL submissions. See `TOWN_SCENE_RENDERING.md` for scope and evidence. This does not certify the globe's separate CityMesh path or frame rate.
