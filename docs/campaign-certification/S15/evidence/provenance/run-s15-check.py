"""Source-pinned S15 qualification; new logs only, no repository mutation."""
...
commands={
 ...
 'fixture':['cargo','test','--locked','--release','-p','spheres-web','s15_export_disposable_flight_operations_fixture','--','--ignored','--nocapture'],
 'browser':['node','tools/ui/ci-flight-operations.cjs'],
}
...
if lane in ['fixture','browser']:
    fixture=pathlib.Path(sys.argv[4]).resolve();env['SPHERES_S15_FIXTURE_DIR']=str(fixture);proof['fixture']=str(fixture)
if lane=='browser':
    env.update(SPHERES_BINARY=str(exe),SPHERES_EXPECTED_REVISION=pin,SPHERES_BROWSER_CHANNEL='chrome',SPHERES_S15_BROWSER_OUTPUT=str(base/'evidence'/('S15-browser-'+label)))
