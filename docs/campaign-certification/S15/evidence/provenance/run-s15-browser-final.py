"""Qualify the committed browser harness against the unchanged S15 runtime."""
...
assert changed==['tools/ui/ci-flight-operations.cjs']
paths=['spheres-sim','spheres-web','Cargo.toml','Cargo.lock']
assert not git('diff','--name-only',runtime,head,'--',*paths)
stem=base/'evidence'/('S15-'+label); assert not stem.with_suffix('.log').exists()
binary=base/'integration-target/release/spheres-web.exe'
build=json.loads((base/'evidence/S15-final1-binary.json').read_text())
assert build['passed'] and build['revision']==runtime and sha(binary)==build['binary']['sha256']
fixture=base/'evidence/S15-fixture-final1'; driver=repo/'tools/ui/ci-flight-operations.cjs'
env=dict(os.environ);env.update(SPHERES_BINARY=str(binary),SPHERES_EXPECTED_REVISION=runtime,
 SPHERES_S15_FIXTURE_DIR=str(fixture),SPHERES_S15_BROWSER_OUTPUT=str(base/'evidence'/('S15-browser-'+label)),SPHERES_BROWSER_CHANNEL='chrome')
record={'runtime_revision':runtime,'driver_revision':head,'changed_paths':changed,'clean_before':True,'runtime_paths':paths,
 ...
 'started_utc':now(),'command':['node','tools/ui/ci-flight-operations.cjs']}
