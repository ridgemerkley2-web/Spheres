...
const output=path.join(base,'evidence/S09-preflight-startup-diagnostic');fs.mkdirSync(output,{recursive:true});const out=fs.mkdtempSync(path.join(output,'chrome-')),run=path.join(out,'server');fs.mkdirSync(run);
...
async function main(){
 ...
 try{
  ...
  browser=await chromium.launch({headless:true,channel:'chrome',timeout:30000,args:['--log-net-log='+path.join(out,'chrome-netlog.json')]});e.browser=browser.version();page=await browser.newPage({viewport:{width:1440,height:1000},reducedMotion:'reduce'});page.setDefaultTimeout(30000);
  ...
  for(const name of ['Network.requestWillBeSentExtraInfo','Network.responseReceivedExtraInfo'])cdp.on(name,data=>event(name,data));
  ...
  event('verifyBuild-start');process.env.SPHERES_EXPECTED_REVISION='94d2c094b2c69a70613af2258acbf49e10d9e871';e.preflight=await require(path.join(repo,'tools/ui/ci-integrated.cjs')).verifyBuild({page,url,root:repo,run,binary:path.join(base,'integration-target/release/spheres-web.exe')});event('verifyBuild-complete',{asset_count:Object.keys(e.preflight.assets).length});
  event('network-only-sample');event('navigate-start');
  try{await page.goto(url,{waitUntil:'domcontentloaded',timeout:30000});event('domcontentloaded-observed',{url:page.url()});e.domcontentloaded=true;}catch(error){e.navigation_failure={message:error.message,url:page.url()};event('navigation-timeout',e.navigation_failure);e.domcontentloaded=false;}
  e.current_url=page.url();
  e.cpu_profile_omitted='Network-only comparison: no Profiler.start';
