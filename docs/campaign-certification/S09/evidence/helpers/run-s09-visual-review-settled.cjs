...
async function main(){
 ...
 const shot=async(name,selector)=>{if(selector)await page.locator(selector).scrollIntoViewIfNeeded();await page.waitForTimeout(750);const filename=name+'.png';await page.screenshot({path:path.join(out,filename)});result.screenshots.push({filename,viewport:page.viewportSize(),selector:selector||null});write();};
 ...
 try{
  ...
  await page.waitForTimeout(12000);stage='designer desktop';await page.locator('#techBtn').click();await page.locator('#techMenu [data-equipment-bureau]').click();await page.locator('[data-equipment-tab="designer"]').first().click();await page.waitForFunction(()=>equipmentCurrent()&&equipmentPreviewCurrent()&&!EQUIP.review&&!EQUIP.replacement);
