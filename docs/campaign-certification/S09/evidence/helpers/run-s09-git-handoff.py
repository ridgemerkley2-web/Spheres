"""Push the completed S09 commit normally; preserve a verified bundle on failure."""
...
assert status['exit_code'] == 0 and not status['stdout'].strip(), 'Commit all reviewed S09 publication files first'
...
manifest = json.loads((repo / 'docs/campaign-certification/S09/manifest.json').read_text(encoding='utf-8'))
assert manifest['status'] == 'complete', 'S09 must be published before transport'
stamp = datetime.datetime.now(datetime.timezone.utc).strftime('%Y%m%dT%H%M%SZ')
output = base / f'S09-git-disposition-{stamp}.json'
...
if not record['remote_verified']:
    bundle = base / f'S09-campaign-certification-{head[:12]}-{stamp}.bundle'
