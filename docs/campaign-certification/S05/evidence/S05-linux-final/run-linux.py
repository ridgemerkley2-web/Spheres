...
out=source.parent/'evidence/S05-linux-final'
pin='261897909e12564275b802f99e3301adbfad61b2'
...
record={'format':'spheres-s05-linux-qualification','version':1,'candidate':pin,'started_utc':stamp(),'status':'running','qualification_scope':'Final affected spheres-web default suite, full Node and three external checks only. Prior full workspace native evidence remains at 8c9159786d3736a9953706262a52c50898225f0e, 1475 passed; it is not relabeled as a final-commit run.','source':str(source),'environment':{k:env[k] for k in ['GIT_DIR','GIT_WORK_TREE','GIT_OPTIONAL_LOCKS','GIT_CONFIG_COUNT','GIT_CONFIG_KEY_0','GIT_CONFIG_VALUE_0','CARGO_BUILD_JOBS','CARGO_HOME','RUSTUP_HOME','CARGO_TARGET_DIR','CARGO_TARGET_X86_64_UNKNOWN_LINUX_GNU_LINKER','CC','TMPDIR']},'checks':[]}
...
def run(name,args,extra=None):
 ...
 return result
try:
 record['source_before']=snapshot('source-before.json');diff=capture(['git','diff','--name-only','8c9159786d3736a9953706262a52c50898225f0e',pin,'--','*.rs']).splitlines();record['rust_diff_from_full_candidate']=diff;assert diff==['spheres-web/src/s05_master_migration_tests.rs'],diff;record['fixtures_before']=fixture_snapshot('fixtures-before.json')
