...
out=source.parent/'evidence/S05-linux-runtime-final'
pin='db9d17c8d726aa102aa143ceb3599009c558ffee'
...
record={'format':'spheres-s05-linux-qualification','version':1,'candidate':pin,'started_utc':stamp(),'status':'running','qualification_scope':'Complete workspace native suite, full Node and three external archive/lifecycle checks on this exact clean runtime candidate. Routing changed production Rust, so the full native suite is rerun.','source':str(source),'environment':{k:env[k] for k in ['GIT_DIR','GIT_WORK_TREE','GIT_OPTIONAL_LOCKS','GIT_CONFIG_COUNT','GIT_CONFIG_KEY_0','GIT_CONFIG_VALUE_0','CARGO_BUILD_JOBS','CARGO_HOME','RUSTUP_HOME','CARGO_TARGET_DIR','CARGO_TARGET_X86_64_UNKNOWN_LINUX_GNU_LINKER','CC','TMPDIR']},'checks':[]}
...
def run(name,args,extra=None):
 ...
 return result
try:
 record['source_before']=snapshot('source-before.json');diff=capture(['git','diff','--name-only','8c9159786d3736a9953706262a52c50898225f0e',pin,'--','*.rs']).splitlines();record['rust_diff_from_full_candidate']=diff;record['fixtures_before']=fixture_snapshot('fixtures-before.json')
