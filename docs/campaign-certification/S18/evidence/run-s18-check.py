"""Source-pinned S18 qualification; new logs only, no repository mutation."""
...
commands={
 'art':['node','tools/ui/aircraft-workshop-browser.cjs'],
 'assets':['node','tools/ui/build_equipment_models.cjs','--check'],
 ...
 'fixture':['cargo','test','--locked','--release','-p','spheres-web','s17_export_disposable_staff_fixture','--','--ignored','--nocapture'],
 'browser':['node','tools/ui/ci-military-staff.cjs'],
}
env['SPHERES_ART_OUTPUT']=str(base/'evidence/S18-art-final')
...
if lane in ['fixture','browser']:
    fixture=pathlib.Path(sys.argv[4]).resolve();env['SPHERES_S17_FIXTURE_DIR']=str(fixture);proof['fixture']=str(fixture)
if lane=='fixture':
    launch=json.loads((base/'S16-review-launch.json').read_text(encoding='utf8'));env['SPHERES_S17_INPUT']=str(pathlib.Path(launch['directory'])/'saves/s16-air-defense.json')
if lane=='browser':
    env.update(SPHERES_BINARY=str(exe),SPHERES_EXPECTED_REVISION=pin,SPHERES_BROWSER_CHANNEL='chrome',SPHERES_S17_BROWSER_OUTPUT=str(base/'evidence'/('S18-browser-'+label)))
