"""Preserve a clean S11 publication and attempt one ordinary authorized push."""
import datetime,hashlib,json,os,pathlib,subprocess,sys
base=pathlib.Path(__file__).resolve().parent;repo=base/'integration'
prior='8525658a4330fb535f77a5f968ab32234087ecfb'
def run(args,**extra):
...
bundle=base/('S11-incremental-'+head[:12]+'-'+stamp+'.bundle');assert not bundle.exists()
...
record={'head':head,'clean':not git('status','--porcelain'),'pushed':code==0,'push_exit_code':code,'push_log':str(log),
    ...
    'prerequisite_commit':prior,'prerequisite_bundle':'S10-closure-incremental-8525658a4330-20260914T014415Z.bundle',
    'scope':'Incremental backup of S11 source and retained qualification. Requires the recorded prior S10 closure history.'}
(base/('S11-git-disposition-'+stamp+'.json')).write_text(json.dumps(record,indent=2)+'\n',encoding='utf8')
