"""Verify staged S11 evidence preserves every published byte."""
import hashlib,json,pathlib,subprocess,sys
base=pathlib.Path(__file__).resolve().parent;repo=base/'integration';root=repo/'docs/campaign-certification/S11'
...
result=subprocess.check_output(['git','-c','core.longpaths=true','ls-files','--stage','-z','--','docs/campaign-certification/S11'],cwd=repo)
seen=set()
...
record={'passed':True,'staged_files':len(seen),'retained_source_files':len(inventory['files']),
    ...
    'scope':'Every S11 publication file matches its staged Git blob. Every retained storage file also matches the evidence inventory; no unlisted storage files exist.'}
