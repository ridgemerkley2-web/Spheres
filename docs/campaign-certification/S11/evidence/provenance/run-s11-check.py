"""Source-pinned S11 qualification; new logs only, no repository mutation."""
...
commands={
 ...
 'fixture':['cargo','test','--locked','--release','-p','spheres-web','s11_export_disposable_ground_operations_fixture','--','--ignored','--nocapture'],
 'browser':['node','tools/ui/ci-ground-operations.cjs'],
}
...
if lane in ['fixture','browser']:
    fixture=pathlib.Path(sys.argv[4]).resolve();env['SPHERES_S11_FIXTURE_DIR']=str(fixture);proof['fixture']=str(fixture)
if lane=='browser':
    env.update(SPHERES_BINARY=str(exe),SPHERES_EXPECTED_REVISION=pin,SPHERES_BROWSER_CHANNEL='chrome',SPHERES_S11_BROWSER_OUTPUT=str(base/'evidence'/('S11-browser-'+label)))
