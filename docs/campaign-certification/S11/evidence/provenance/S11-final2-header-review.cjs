...
const base=__dirname,repo=path.join(base,'integration'),out=path.join(base,'evidence/S11-final2-header-review'),url='http://127.0.0.1:7856';
...
async function main(){
 ...
 try{
  ...
  assert.deepEqual(proof.requests,[]);assert.deepEqual(proof.errors,[]);proof.passed=true;
