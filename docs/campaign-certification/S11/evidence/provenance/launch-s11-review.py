"""Launch a separate copied S11 review campaign after qualification."""
...
saved=pathlib.Path(browser['run'])/'saves'/f"{browser['saved_slot']}.json";assert saved.is_file()
refs=list((browserfile.parent/'archive-audit').glob('*-s11-audit-continued.json'));assert len(refs)==1;reference=read(refs[0]);assert reference['canonical']['ignored_paths']==[]
run=base/('review-S11-'+datetime.datetime.now().strftime('%Y%m%d-%H%M%S'));run.mkdir();(run/'saves').mkdir()
exe=run/'spheres-web.exe';target=run/'saves/s11-ground-operations.json'
...
port=None
for candidate in range(7856,7875):
...
try:
 ...
 state=request('/api/load',{'slot':'s11-ground-operations'})
 ...
 record={'url':url,'pid':process.pid,'directory':str(run),'runtime_revision':pin,'executable_sha256':sha(exe),'player':state['player'],'date':state['date'],'build':build,
  ...
  'scope':'Separate copy of the disclosed authored S11 France scenario. Existing review servers and original campaigns are preserved. This is a test campaign with manufactured preconditions, not a historical or unassisted campaign.'}
 with (base/'S11-review-launch.json').open('x',encoding='utf8') as f:json.dump(record,f,indent=2);f.write('\n')
