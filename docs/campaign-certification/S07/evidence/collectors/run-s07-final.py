...
stem=base/'evidence'/('S07-final-'+(sys.argv[3] if len(sys.argv)>3 else lane))
...
if lane.startswith('browser') or lane in ('money-browser','construction-browser'):
...
elif lane=='binary' or lane.startswith('browser') or lane in ('money-browser','construction-browser'): binary=base/'integration-target/release/spheres-web.exe'
else: binary=None
