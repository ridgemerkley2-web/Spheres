...
def prior_and_preservation(cfg,kept):
    ...
    need(len(p['files'])==len(b['files']) and len(p['worktrees'])==len(b['worktrees'])
    ...
    baseline_heads={str(path(x['path'])):x['head'] for x in b['worktrees']}
    ...
    return inherited,{'record':kept(pf),'baseline':kept(baseline),'files':len(p['files']),'worktree_heads':len(p['worktrees'])}
...
def render(manifest):
    ...
    need(all(sessions['S'+str(i)]['status']=='complete' for i in range(1,16)), 'An inherited core session is not complete')
