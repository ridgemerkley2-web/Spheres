...
/// Evaluate the exact native order on a disposable world. The trial is never
/// returned, so it needs no second rollback snapshot on an exceptional refusal.
/// Live enactment still uses the fully atomic `apply` path below.
pub(crate) fn refusal(w: &WorldState, n: NationId, order: &CompanyOrder) -> Option<String> {
    apply_inner(&mut w.clone(), n, order).err()
}
