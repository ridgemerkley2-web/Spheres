...
lane,label=sys.argv[1:3];assert lane in ('build','read','daily','feature') and re.fullmatch('[A-Za-z0-9_-]+',label)
out=base/'evidence'/('S08-diagnostic-'+label);out.mkdir(exist_ok=False)
def sha(p):
    with pathlib.Path(p).open('rb') as f:return hashlib.file_digest(f,'sha256').hexdigest()
def git(*args):return subprocess.check_output(['git','-c','core.longpaths=true',*args],cwd=repo,stderr=subprocess.PIPE)
def source():
    ...
    names=sorted(set(names+git('diff','--name-only','HEAD').decode().splitlines()+git('ls-files','--others','--exclude-standard').decode().splitlines()))
...
if lane=='build':command=['cargo','test','--locked','--release','-p','spheres-web','--no-run']
    ...
    binary=pathlib.Path(sys.argv[3]).resolve();command=[str(binary),'performance::'+{'read':'s08_read_model_diagnosis','daily':'s08_daily_subsystem_diagnosis','feature':'s08_supplier_import_profile'}[lane],'--ignored','--exact','--nocapture','--test-threads=1']
    ...
    if lane=='feature':
        env['SPHERES_S08_PERFORMANCE_INPUT']=env['SPHERES_S08_DIAGNOSTIC_INPUT']
        env['SPHERES_S08_PERFORMANCE_OUT']=env['SPHERES_S08_DIAGNOSTIC_OUT']
...
if lane!='build':
    record['input']={'path':env['SPHERES_S08_DIAGNOSTIC_INPUT'],'sha256_before':sha(env['SPHERES_S08_DIAGNOSTIC_INPUT'])}
    record['binary_sha256_before']=sha(binary)
...
if lane!='build':
    ...
    record['input']['sha256_after']=sha(env['SPHERES_S08_DIAGNOSTIC_INPUT'])
    record['inputs_unchanged']=record['input']['sha256_before']==record['input']['sha256_after'] and record['binary_sha256_before']==record['binary']['sha256']
record['passed']=r.returncode==0 and record['source_unchanged'] and record.get('inputs_unchanged',True)
