...
Use --supplier-browser-result for a verified external supplier journey and its saves.
...
def main():
    ...
    parser.add_argument('--supplier-browser-result', type=Path, action='append', default=[], help='Additional supplier browser result with a verified S08 revision; retain its named campaign saves')
    ...
    def add_browser(result_path, kind, explicit_failed=False, verify_supplier=False):
        value = read(result_path); revision = browser_revision(value)
        if verify_supplier:
            require(s08_revision(repo, revision, pin), 'Explicit supplier browser artifact does not identify an S08 revision: ' + str(result_path))
        ...
        browser_records.append({'source_result': str(result_path), 'logical_result': logical + '/' + result_path.name,
            ...
            'scope': 'Explicitly identified failed S08 attempt' if explicit_failed else 'Explicit supplier browser revision checked against S08 ancestry' if verify_supplier else 'Supplier-only S08 folder' if kind == 'supplier_browser' else 'Revision checked against S08 ancestry'})
    ...
    for path in args.supplier_browser_result: add_browser(path.resolve(), 'supplier_browser', verify_supplier=True)
    ...
    for name in ('ci-supplier-imports.cjs', 'supplier-archive-audit.cjs', 'archive-worker.py',
                 'ci-browser.cjs', 'ci-integrated.cjs', 'ci-money.cjs'):
