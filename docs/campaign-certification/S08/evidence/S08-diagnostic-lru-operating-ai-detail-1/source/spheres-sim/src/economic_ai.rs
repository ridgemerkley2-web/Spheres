...
pub fn raw_supply_forecast_with_context(
    ...
    let pending_supply = pending_supply_horizons(w, nation);
    ...
    for commodity in resources::ALL {
        ...
        let gross_pending = pending_supply[i].map(round);
...
fn review(w: &mut WorldState, nation: NationId, raw_context: &RawSupplyContext,
    ...
    let mut next = if active > 0 {
        ...
        observe_review_call(observer,"review.candidate",Some(nation),||candidate(w, nation))
    };
    ...
    let bootstrap = observe_review_call(observer,"review.bootstrap",Some(nation),||materials_bootstrap(w, nation, &industry_planning::plan(w, nation)));
    ...
    let domestic_order = observe_review_call(observer,"review.commission",Some(nation),||commission_missing_materials(w, nation));
    if active == 0 && domestic_order.as_ref().is_some_and(|(ok,_)|*ok) {
        ...
        next = observe_review_call(observer,"review.candidate",Some(nation),||candidate(w,nation));
    }

    let domestic_attempt = domestic_order.is_some();
    let goods_trade = domestic_order.or_else(||observe_review_call(observer,"review.purchase",Some(nation),||buy_missing_goods(w, nation)));
