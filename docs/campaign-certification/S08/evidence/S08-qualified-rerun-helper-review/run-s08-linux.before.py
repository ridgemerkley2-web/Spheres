...
out=source.parent/('evidence/S08-linux-'+label)
...
def snapshot(name):
 d={'captured_utc':stamp(),'head':capture(['git','-c','core.longpaths=true','rev-parse','HEAD']).strip(),'status_porcelain':capture(['git','-c','core.longpaths=true','status','--porcelain']),'git_dir':env['GIT_DIR'],'git_work_tree':str(source)}
...
record={'format':'spheres-s08-linux-qualification','version':1,'candidate':pin,'started_utc':stamp(),'status':'running','qualification_scope':'Complete workspace native suite, full Node and three external archive/lifecycle checks on this exact clean runtime candidate. Supplier/import ownership, saves, fiscal settlement and adapters changed; rerun the complete native suite.','source':str(source),'environment':{k:env[k] for k in ['GIT_DIR','GIT_WORK_TREE','GIT_OPTIONAL_LOCKS','GIT_CONFIG_COUNT','GIT_CONFIG_KEY_0','GIT_CONFIG_VALUE_0','CARGO_BUILD_JOBS','CARGO_HOME','RUSTUP_HOME','CARGO_TARGET_DIR','CARGO_TARGET_X86_64_UNKNOWN_LINUX_GNU_LINKER','CC','TMPDIR']},'checks':[]}
...
def run(name,args,extra=None):
 ...
 return result
try:
 record['source_before']=snapshot('source-before.json');diff=capture(['git','-c','core.longpaths=true','diff','--name-only','041007fbfda48cc027d0c24a1189705a1dcaa89b',pin,'--','*.rs']).splitlines();record['rust_diff_baseline_revision']='041007fbfda48cc027d0c24a1189705a1dcaa89b';record['rust_diff_from_full_candidate']=diff;record['fixtures_before']=fixture_snapshot('fixtures-before.json')
