...
fn clear_spot_market_impl(w: &mut WorldState, cache_pure_reads: bool) {
    clear_spot_market_traced(w, cache_pure_reads, None);
}
...
pub fn clear_spot_market_observed(w: &mut WorldState,
    ...
    clear_spot_market_traced(w, true, Some(observer));
}
...
fn civilian_recurring_buy_pass_impl(
    ...
    for buyer in buyers {
        ...
        // No ask or memo update could follow. Keep the full forecast for any
        // other value, including malformed non-finite components, and retain
        ...
        let forecast = if lazy_routes {
            crate::economic_ai::raw_supply_forecast_with_civilian(w, buyer, &raw_context, &civilian_components)
        } else {
            crate::economic_ai::raw_supply_forecast_with_context(w, buyer, &raw_context)
        };
