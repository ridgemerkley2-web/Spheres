...
/// The monthly civilian buyer already needs these same immutable components
/// to size its recurring gap. Share that read only until its next command.
...
fn raw_supply_forecast_impl(
    ...
    // Each pure horizon call returns all twelve inputs. Reuse the whole array
    // across commodity rows, preserving its native order and arithmetic.
    let materials_by_horizon = reuse_material_horizons.then(|| RAW_HORIZON_DAYS
        .map(|days| crate::materials::resource_demand_for_days(w, nation, days)));
    ...
    for commodity in resources::ALL {
        ...
        let demand: [f64; 3] = std::array::from_fn(|h| {
            ...
            let materials = materials_remaining.min(materials_by_horizon.as_ref().map_or_else(
