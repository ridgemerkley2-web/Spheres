...
impl ClearingRoutes {
    ...
    fn plan(&mut self, w: &WorldState, seller: NationId, buyer: NationId) -> Result<RoutePlan, String> {
        ...
        #[cfg(test)]
        if !self.reuse_pure_plan_reads { return assemble_plan(w, finish, &tree.prev); }
...
fn assemble_plan_with_segments(w: &WorldState, mut at: usize, prev: &[Option<(usize, usize)>],
    ...
    for ei in edge_ids {
        let e = &net.edges[ei];
        let (cap, name) = match segments_read.as_deref_mut() {
            Some(read) if e.kind != "terminal" => read.entry(ei).or_insert_with(|| segment_capacity(w, e)).clone(),
            _ => segment_capacity(w, e),
        };
        if cap < bottleneck.0 {
            bottleneck = (cap, name)
        }
...
fn dispatch_impl(
    ...
    let route = preplanned.unwrap_or_else(|| w
        ...
        .get(&key)
        .cloned()
        .filter(|r| routes.as_mut().map_or_else(|| route_open(w, seller, buyer, r).is_ok(),
            |routes| routes.access.is_open(w, seller, buyer, policy, r)))
        .map(Ok)
...
mod tests {
    ...
    fn s08_tree_sources(w: &WorldState, routes: &ClearingRoutes) -> Vec<NationId> {
        ...
        assert!(ids.len() >= 131, "the mapped roster must exercise the actual 128-tree limit");
        ids.truncate(131);
        ids
    }
    ...
    #[test]
    fn s08_clearing_tree_lru_matches_old_admission_full_world_and_budget() {
        let mut cached = world();
        cached.rules.daily_simulation = true;
        cached.rules.military_operations = true;
        cached.sanctions.clear();
        ...
        let ids = s08_tree_sources(&cached, &cached_routes);
        for &source in &ids[..130] {
            let target = s08_tree_buyer(source);
        ...
        for (source, target, commodity, quantity) in [
            ...
            (ids[129], s08_tree_buyer(ids[129]), Commodity::Copper, 0.25),
        ...
        assert_eq!(cached_routes.trees.len(), 128);
        assert_eq!(original_routes.trees.len(), 128);
    }
