...
// Timings are exploratory (the normal tick follows the measured tick), and are
// not the uncontended, predefined campaign performance acceptance measurement.
...
fn s08_daily_subsystem_diagnosis() {
    ...
    let mut report = json!({
        ...
        "scope":"Exploratory named daily-system timings on a disposable decoded campaign. The observed driver runs first, then normal Game advance; cache order is not independent. Clone, exact native serialization comparison and report writes are outside subsystem timings. The outer runner supplies cryptographic input/source provenance. No performance acceptance claim."
    });
    ...
    for offset in 1..=31 {
        ...
        for (name, system) in spheres_sim::SYSTEMS {
            ...
            if *name == "economic_ai" {
                let started = Instant::now();
                spheres_sim::economic_ai::tick_observed(&mut observed, &mut |nation, elapsed| {
                    let key = format!("detail.economic_ai.{}", nation.map_or("context", |id| id.name()));
                    let elapsed_ms = elapsed.as_secs_f64() * 1000.0;
                    samples.entry(key.clone()).or_default().push(elapsed_ms);
                    stages.push(json!({"name":key,"elapsed_ms":elapsed_ms}));
                });
