...
impl SearchTree {
    ...
    fn extend<const OBSERVED: bool>(&mut self, net: &Network, policy: RoutePolicy,
        ...
        while finish.is_none() {
            ...
            self.settled[node] = self.next_rank;
            self.next_rank += 1;
            if OBSERVED {
            ...
            for step in &net.traversal[node] {
                ...
                if nc < self.dist[next] {
                    ...
                    self.dist[next] = nc;
                    self.prev[next] = Some((node, step.edge));
    ...
    fn s08_retained_heap_diagnostics_preserve_routes_and_count_actual_work() {
        for military in [false, true] {
            ...
            for (key, tree) in &observed.trees {
                ...
                assert_eq!(tree.prev, original.prev);
                assert_eq!(tree.settled, original.settled);
            ...
            let edges: usize = observed.trees.values().map(|tree| tree.settled.iter().enumerate()
