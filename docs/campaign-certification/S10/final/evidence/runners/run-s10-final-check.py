...
env=os.environ.copy(); env['CARGO_TARGET_DIR']=str(base/'integration-target'); env['CARGO_BUILD_JOBS']='4'
commands={
 'matrix':['node','tools/ui/ci-government-country-matrix.cjs'],
 ...
 'performance':['cargo','test','--locked','--release','-p','spheres-web','--bin','spheres-web','diplomatic_sanctions::tests::s10g_mature_open_route_preview_latency','--','--ignored','--nocapture'],
}
...
if lane=='matrix':
    env.update(SPHERES_BINARY=str(exe),SPHERES_EXPECTED_REVISION=pin,SPHERES_BROWSER_CHANNEL='chrome',SPHERES_COUNTRY_MATRIX_OUTPUT=str(base/'evidence/S10-final-matrix'))
...
if lane in ('binary','browser','matrix'): proof['binary']={'path':str(exe),'sha256':sha(exe)}
if lane=='performance':
    proof['source']['sha256_after']=sha(source)
    proof['measurements']=[json.loads(s.split('S10G_SANCTIONS_PERF ',1)[1]) for s in log.splitlines() if 'S10G_SANCTIONS_PERF ' in s]
proof['passed']=p.returncode==0 and proof['revision_after']==pin and proof['clean_after']
if lane in ('browser','matrix'): proof['passed'] &= proof['binary_sha256_before']==proof['binary']['sha256']
