...
pin=sys.argv[1];buildfile=base/'evidence/S10-final-binary.json';browserfile=pathlib.Path(sys.argv[2])
...
run=base/('review-S10-final-'+datetime.datetime.now().strftime('%Y%m%d-%H%M%S'));run.mkdir();(run/'saves').mkdir()
...
port=None
for candidate in range(7855,7875):
...
try:
 ...
 (base/'S10-final-review-launch.json').write_text(json.dumps(record,indent=2)+'\n',encoding='utf-8')
