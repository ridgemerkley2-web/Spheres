...
stem=base/'evidence'/('S10-browser-driver-'+label);assert not stem.with_suffix('.json').exists() and not stem.with_suffix('.log').exists()
env=os.environ.copy();env.update(SPHERES_BINARY=str(exe),SPHERES_EXPECTED_REVISION=pin,SPHERES_BROWSER_CHANNEL='chrome',SPHERES_DECISIONS_OUTPUT=str(base/'evidence'/('S10-browser-'+label)))
command=['node','tools/ui/ci-government-decisions.cjs']
proof={'runtime_revision':pin,'driver_revision':head,'complete_runtime_source_equal':True,'runner_sha256':sha(__file__),'driver_sha256':sha(repo/'tools/ui/ci-government-decisions.cjs'),'binary_sha256_before':binary_hash,'command':command,'started_utc':datetime.datetime.now(datetime.timezone.utc).isoformat()}
