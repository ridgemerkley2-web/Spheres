...
pin=sys.argv[1];buildfile=base/'evidence/S10-final-binary-1.json';browserfile=pathlib.Path(sys.argv[2])
...
run=base/('review-S10-'+datetime.datetime.now().strftime('%Y%m%d-%H%M%S'));run.mkdir();(run/'saves').mkdir()
...
port=None
for candidate in range(7848,7861):
...
try:
 ...
 (base/'S10-review-launch.json').write_text(json.dumps(record,indent=2)+'\n',encoding='utf-8')
