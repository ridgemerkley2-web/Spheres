...
web=json.loads((base/'evidence/S10-g-web.json').read_text())
build=json.loads((base/'evidence/S10-g-binary.json').read_text())
...
out=base/('evidence/S10g-sanctions-'+label);out.mkdir(exist_ok=False)
fixture=base/'evidence/S10g-sanctions-fixture-1'
env=dict(os.environ)
env.update(SPHERES_S10G_SANCTIONS_FIXTURE_DIR=str(fixture),SPHERES_BINARY=str(exe),
    SPHERES_EXPECTED_REVISION=pin,SPHERES_S10G_SANCTIONS_FIXTURE=str(fixture/'manifest.json'),
    SPHERES_SANCTIONS_OUTPUT=str(out/'browser'),SPHERES_BROWSER_CHANNEL='chrome')
commands=[]
if not fixture.exists():
    commands.append(('export',[str(test_binary),'s10g_sanctions_fixture_tests::s10g_export_disposable_sanctions_fixture','--exact','--ignored','--nocapture','--test-threads=1']))
commands.append(('browser',['node','tools/ui/ci-sanctions-desk.cjs']))
