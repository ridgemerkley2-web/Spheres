...
pin=sys.argv[1];buildfile=base/'evidence/S10-g-binary.json';browserfile=pathlib.Path(sys.argv[2])
...
run=base/('review-S10g-'+datetime.datetime.now().strftime('%Y%m%d-%H%M%S'));run.mkdir();(run/'saves').mkdir()
...
port=None
for candidate in range(7853,7864):
...
try:
 ...
 (base/'S10g-review-launch.json').write_text(json.dumps(record,indent=2)+'\n',encoding='utf-8')
