...
pin=sys.argv[1];buildfile=base/'evidence/S10-c-binary-3.json';browserfile=pathlib.Path(sys.argv[2])
...
run=base/('review-S10c-'+datetime.datetime.now().strftime('%Y%m%d-%H%M%S'));run.mkdir();(run/'saves').mkdir()
...
port=None
for candidate in range(7849,7861):
...
try:
 ...
 (base/'S10c-review-launch.json').write_text(json.dumps(record,indent=2)+'\n',encoding='utf-8')
