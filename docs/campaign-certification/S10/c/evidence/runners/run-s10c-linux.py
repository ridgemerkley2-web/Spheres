...
out=source.parent/('evidence/S10c-linux-'+label)
...
record={'format':'spheres-s10c-linux-qualification','version':1,'candidate':pin,'started_utc':stamp(),'status':'running','qualification_scope':'Complete web native suite, full Node and three external archive/lifecycle checks on this exact clean runtime candidate. Includes S10 government and agency review tokens, party/executive role separation, pure cancellation and command receipts. Full native and Node execution use Linux tools; source provenance uses Windows git.exe against the same NTFS checkout.','source':str(source),'environment':{k:env[k] for k in ['GIT_DIR','GIT_WORK_TREE','GIT_OPTIONAL_LOCKS','GIT_CONFIG_COUNT','GIT_CONFIG_KEY_0','GIT_CONFIG_VALUE_0','CARGO_BUILD_JOBS','CARGO_HOME','RUSTUP_HOME','CARGO_TARGET_DIR','CARGO_TARGET_X86_64_UNKNOWN_LINUX_GNU_LINKER','CC','TMPDIR']},'checks':[]}
...
try:
 ...
 record['derived_from']={'path':str(source.parent/'run-s09-linux-windows-git.py'),'sha256':sha(source.parent/'run-s09-linux-windows-git.py')}
 assert not capture(['git','-c','core.longpaths=true','diff','--name-only','d361c2d567b0704bdffb5160760c87a336b0c7c9',pin,'--','spheres-sim','spheres-cli','Cargo.toml','Cargo.lock']).strip(), 'UI/portrait qualification requires unchanged simulation source'
 record['source_before']=snapshot('source-before.json');diff=capture(['git','-c','core.longpaths=true','diff','--name-only','872442d7411d9986321248b41ccadf79e2851d1c',pin,'--','*.rs']).splitlines();record['rust_diff_baseline_revision']='872442d7411d9986321248b41ccadf79e2851d1c';record['rust_diff_from_full_candidate']=diff;record['fixtures_before']=fixture_snapshot('fixtures-before.json')
 ...
 run('native',['cargo','test','--locked','--release','-p','spheres-web','--no-fail-fast'])
 ...
 run('c01-tests',['python3','-X','utf8','-m','unittest','discover','-s','tools/avatars','-p','test_campaign*.py'])
 for tool in ['campaign_census','import_cnccfp_census','campaign_research']:run('c01-'+tool,['python3','-X','utf8','tools/avatars/'+tool+'.py','--check'])
 if len(found)==1:
 ...
 record['status']='passed' if len(record['checks'])==9 and all(x['exit_code']==0 and x.get('exact_one_test_passed',True) for x in record['checks']) and record['checks'][0].get('totals',{}).get('completed_targets',0)>0 and record['checks'][0]['totals']['failed']==0 else 'failed';record['finished_utc']=stamp();persist();print('FINAL '+record['status'],flush=True)
