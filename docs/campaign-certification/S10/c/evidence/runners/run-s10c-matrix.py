...
stem=base/'evidence'/('S10c-matrix-driver-'+label);assert not stem.with_suffix('.json').exists() and not stem.with_suffix('.log').exists()
env=os.environ.copy();env.update(SPHERES_BINARY=str(exe),SPHERES_EXPECTED_REVISION=pin,SPHERES_BROWSER_CHANNEL='chrome',SPHERES_COUNTRY_MATRIX_OUTPUT=str(base/'evidence'/('S10c-matrix-'+label)))
