"""Preserve selected S09 evidence, with byte-exact reconstructed large files."""
import datetime,gzip,hashlib,json,pathlib,sys
...
def collect(selection,out):
 ...
 manifest={'created_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'selection_sha256':selection_hash,'selection':inputs,'files':records,
  'scope':'Selected complete S09 test logs, proof records, browser outputs including failures, raw campaign snapshots and review launch. Compiled binaries are identified by SHA-256 in proofs and reproducible from the recorded source; they are not repackaged here.',
...
if __name__=='__main__':
 collect(pathlib.Path(sys.argv[1]).resolve(),repo/'docs/campaign-certification/S09/evidence')
