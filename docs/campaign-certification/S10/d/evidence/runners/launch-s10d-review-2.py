...
pin=sys.argv[1];buildfile=base/'evidence/S10-d-binary-2.json';browserfile=pathlib.Path(sys.argv[2])
...
run=base/('review-S10d-'+datetime.datetime.now().strftime('%Y%m%d-%H%M%S'));run.mkdir();(run/'saves').mkdir()
...
port=None
for candidate in range(7850,7861):
...
try:
 ...
 (base/'S10d-review-launch.json').write_text(json.dumps(record,indent=2)+'\n',encoding='utf-8')
