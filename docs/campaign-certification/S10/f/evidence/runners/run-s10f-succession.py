...
web=json.loads((base/'evidence/S10-f-web.json').read_text())
build=json.loads((base/'evidence/S10-f-binary.json').read_text())
...
out=base/('evidence/S10f-succession-'+label);out.mkdir(exist_ok=False)
fixture=base/'evidence/S10f-succession-fixture-2'
env=dict(os.environ)
env.update(SPHERES_S10F_SUCCESSION_FIXTURE_DIR=str(fixture),SPHERES_BINARY=str(exe),
    SPHERES_EXPECTED_REVISION=pin,SPHERES_S10F_SUCCESSION_FIXTURE=str(fixture/'manifest.json'),
    SPHERES_SUCCESSION_OUTPUT=str(out/'browser'),SPHERES_BROWSER_CHANNEL='chrome')
commands=[]
if not fixture.exists():
    commands.append(('export',[str(test_binary),'s10f_succession_fixture_tests::s10f_export_disposable_succession_fixtures','--exact','--ignored','--nocapture','--test-threads=1']))
commands.append(('browser',['node','tools/ui/ci-succession-context.cjs']))
