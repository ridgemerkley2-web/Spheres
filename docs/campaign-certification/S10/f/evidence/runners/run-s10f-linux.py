...
out = base / 'evidence/S10f-linux-final'
...
record={'format':'spheres-s10f-linux/v1','revision':pin,'started_utc':stamp(),
    'scope':'Full native web suite and party-leadership unit and focused native agency/succession integration suites on Linux. No new Linux Node, external old-save or long-campaign qualification is claimed.',
...
record['passed']=len(record['checks'])==3 and all(c['exit_code']==0 and c['totals']['completed_targets']>0 and not c['totals']['failed'] for c in record['checks'])
