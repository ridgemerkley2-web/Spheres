...
web=json.loads((base/'evidence/S10-e-web.json').read_text())
build=json.loads((base/'evidence/S10-e-binary.json').read_text())
...
out=base/('evidence/S10e-commitments-'+label);out.mkdir(exist_ok=False)
fixture=base/'evidence/S10e-commitments-fixture-2'
env=dict(os.environ)
env.update(SPHERES_S10E_COMMITMENT_FIXTURE_DIR=str(fixture),SPHERES_BINARY=str(exe),
    SPHERES_EXPECTED_REVISION=pin,SPHERES_S10E_COMMITMENT_FIXTURE=str(fixture/'manifest.json'),
    SPHERES_COMMITMENT_OUTPUT=str(out/'browser'),SPHERES_BROWSER_CHANNEL='chrome')
commands=[]
if not fixture.exists():
    commands.append(('export',[str(test_binary),'s10e_commitment_fixture_tests::s10e_export_disposable_diplomatic_commitment_fixture','--exact','--ignored','--nocapture','--test-threads=1']))
commands.append(('browser',['node','tools/ui/ci-diplomatic-commitments.cjs']))
