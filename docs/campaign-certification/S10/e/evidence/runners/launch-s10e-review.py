...
pin=sys.argv[1];buildfile=base/'evidence/S10-e-binary.json';browserfile=pathlib.Path(sys.argv[2])
...
run=base/('review-S10e-'+datetime.datetime.now().strftime('%Y%m%d-%H%M%S'));run.mkdir();(run/'saves').mkdir()
...
port=None
for candidate in range(7851,7862):
...
try:
 ...
 (base/'S10e-review-launch.json').write_text(json.dumps(record,indent=2)+'\n',encoding='utf-8')
