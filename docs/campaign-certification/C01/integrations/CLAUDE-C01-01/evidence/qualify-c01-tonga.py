...
out=base/'evidence/C01-tonga-review-20260921/qualification'
out.mkdir(exist_ok=False)
def git(*args):return subprocess.check_output(['git','-c','core.longpaths=true',*args],cwd=repo,text=True).strip()
def sha(p):return hashlib.sha256(p.read_bytes()).hexdigest()
pin=git('rev-parse','HEAD');assert pin=='79c082ed96b86699184a00723a79ef7422188e83' and not git('status','--porcelain')
