...
**UPDATED 2026-09-06 (H-1, H-6):** the presence clause IS built now (M1: `blocs::PRESENCE_BACKING` 0.05, the sponsors' creating clause and standing programme), so the chain starts in Kabul from month 0 — and stops at the pinned lines by arithmetic: share <= the flat seed's split (0.133 with three present; `RULING_SEED` pinned by the drift test), backing <= `BACKING_TOTAL_CAP` 0.25, so I <= 0.383 against the 0.45 literal, and Kabul's army at its transcribed 15% share reads 0.85 on the share arm (M2's pay arm at weight 0, H-3), so coercion never fails. N=200 on the history branch: 1/200 by 2000 (Algeria once). The paragraph below is the S5 reading of cbcc364.
...
**UPDATED 2026-09-06 (H-6):** re-measured on the history branch (N=200 median 2, mean 1.650; levers 0 in 200/200) and probed: 16-17 regimes reach discontent >= 0.40 during 1990-96 but the largest non-ruling movement sits at 0.12-0.25 in all but Chad and Sao Tome (Zambia D 0.60 at 0.24, Zaire 0.67 at 0.24), so the AI's four lines coincide in 0 nation-months and even the lever's own refusal lines admit two regimes. The binding pieces are the flat seed (pinned at 0.5988 by `a_regime_s_movements_move_with_the_pains_and_revert`) and the record line, which reads prices, growth and war and NOT the order pain that carries half of discontent; the AI's lines are pinned by `the_ai_takes_each_lever_only_under_its_thresholds`. The historical basis for the AI move that was drafted and not made (Benin 1989-90, Zambia 1990-91, fetched) is quoted in H-6. The paragraph below is the S5 reading.
...
**UPDATED 2026-09-06 (H-3, H-5, H-6):** the per-head term A1 lacked was built as ruled (M2) and sized to ZERO by its own measurement — the ruled ratio reads the coup-prone armies as the best paid in the roster (H-3) — so A1's concentration reads top-3 share 1.00 in 200/200 on the history branch still (Sao Tome 541, Philippines 447, Comoros 200 of 1252); A5 is re-expressed to the successor flag (R3(d)) and reads 0/200 for the reason below (`drift_support`); A7 reads 0.148 (R2 took four takeovers a seed off the denominator and the ballot side did not move). None of the three is reachable by a listed constant (H-6).
...
**UPDATED 2026-09-06 (H-2, H-4, H-5, H-6):** 1 is closed by R2 (a merely-present army annuls nothing; Belarus and Ukraine 200 → 0 annulments) and its table half stands as R3(a)'s re-pin question (H-5); 2 is transcribed (Algeria's 1987 table) and stopped as R3(b)'s re-pin question (H-5); 3 is answered by M1 (the presence is created from month 0) and the bloc then stops at the pinned lines (H-6); 4 is transcribed and stopped as R3(c)'s re-pin question (H-5); 5 is closed on the transcription half by the successor flag (R3(d)); and D4 is wired (R1, H-4). A sixth: Cambodia's live defence share near the 1% floor is what stages the regime coup that A3's residual rides (H-6).
...
**RULED 2026-09-06 (H-2, H-4):** both questions were ruled by Ridge ("Make determinations for these and continue. I want it based on historical data.") — (1) D4 wired under the switch, the one inertness clause narrowed as ruled (R1, H-4); (2) the hostile-army annulment landed, the Jordan assertion re-expressed (R2, H-2). The standard stands: those two are the only existing tests touched on the branch, and the calibration pass (H-6) made no move because every remaining permitted move is a third.
...
**CLOSED on the transcription half 2026-09-06 (R3(d), H-5, H-6):** the bar reads the sourced `successor_of_ruling_party` flag (24 rows), so the LDDP's and the MSZP's returns can register whatever their family; it reads 0/200 on the history branch for the model reason alone (`drift_support`, S5-7).
...
### H-4 — R1 landed: D4's Nepal and Haiti tables wired under the switch (5086cfa cherry-picked), the one clause narrowed as ruled

**The ruling, quoted:** "Nepal and Haiti: WIRE their transcribed tables (D4_POLITIES beside POLITIES, commit 5086cfa reverted in 62af57b and cherry-pickable) under the ideology_blocs switch so the default path is byte-identical; the ONE existing test clause that must narrow to admit it (the government clause of the_bloc_layer_is_inert_over_time, for those two nations, switched-on state only) is re-expressed and the narrowing is recorded in the test comment and BUGS with this ruling quoted." P-8, S4-10, R-6 and S7-3's first question close with this.

**What landed.** `git cherry-pick -x 5086cfa` on top of M1, R2 and M2, with six conflict hunks resolved by hand and nothing of 5086cfa dropped: `government::polity_in(w, id)` serves the `D4_POLITIES` block only while `rules.ideology_blocs` is on and `POLITIES` otherwise, `table_of` / `spec` / `base_share` resolve a party id in whichever table carries it, the 21 `polity` readers in `government.rs`, 4 in `blocs.rs` and `government_json` in the web ask `polity_in`, `bloc_appeal` and `largest_party_of` take the world, and the id-only readers stay on `POLITIES` (asserted by `d4_tables_are_read_under_the_switch_and_invisible_off`, watched red AGAIN on this branch with the switch test removed from `polity_in`: the OFF world's table pointer differed). The resolution where M1 and D4 met: `bloc_present` keeps M1's shape (`bloc_in_table || bloc_backed`) and `bloc_in_table` now takes the world and reads `polity_in` (its one caller in `politics::ai_back_bloc_choice` and the M1 test follow); `bloc_can_win` keeps M1's doc and reads `polity_in`; `flat_seed`'s doc keeps M1's wording; the inertness test's doc carries both paragraphs, D4's "the pin above (month 35, Libya) is unmoved" re-worded to this branch's month-0 pin.

**The narrowed clause, recorded.** `the_bloc_layer_is_inert_over_time`'s government clause compares `support` for every nation EXCEPT the two whose ON-world table is the D4 block (`polity_in(&on, id)` differs from `polity(id)`) — seating a table the OFF world cannot see is the wiring itself, not a leak; coalition and pillars are still compared for them, and `support` for every other nation. Switched-on state only, by construction (the OFF world never reads D4). 5086cfa measured the clause's need on its tree ("parted at month 0 with no stem: [rng, Nepal government, Haiti government, …]"); on THIS branch the first act is month 0 (M1, H-1), so the clause is reached in that one month and the parting carries M1's stems either way — it stands as ruled and is not what the bar turns on today. What the two tables seat under the switch is what 5086cfa's own test asserts, and that test is green on this branch.

**Measured.** Default path byte-identical: 0xe26e4bf8d6c60066 and 0xbe94d6125631829c unmoved; `run 35 1990` d1a2cfbf7c6958d7 (3501), `run 35 7` 39dea3341a7f6e8c (3983) unmoved. Sim lib 456 passed / 3 failed / 25 ignored (158.2 s; the three deliberate reds; the +1 is the D4 test); `bloc_census` 4 pinned bars green (23.1 s); web 160 / 0 / 2 (`government_json` moved); cli 1 / 0; the inertness trio, the M1, R2 and M2 tests green on the resolved tree. The N=200 census (`census_n200_r1`) against M2's: 3 of the 100 printed seed lines moved (seeds 21, 34, 78 — coups against elected governments 5 → 7, 6 → 6, 9 → 6) and no summary line past the third decimal: A1 6.265 → 6.260, annulments 0.530 → 0.525 (Algeria 106 → 105), regime coups 10.590 → 10.575 (Nepal 47 → 45, Haiti 202 → 201), A2 1/200, A3 118 → 117 of 200 (Cambodia), A4 1.650 both, A5 0, A6 0 in 200/200, A7 ratio med 0.14 → 0.15, A8 200/200, A9 3472/3472 = 1.000, A10 med 0.28 both, deaths 43.600 → 43.530.

### H-5 — R3(d) and R3(e) landed; R3(a), R3(b) and R3(c) transcribed, measured against the default path, and NOT landed — three re-pin questions for Ridge

The integrator's pass of 2026-09-06 on `feat/ideology-history`, every number below measured this run on this worktree (`CARGO_TARGET_DIR` its own), the gate after every item: the two goldens with `--exact`, the four inertness tests, both market-OFF digests, the 27 data-integrity tests. The protocol given to the pass, quoted: "if a transcription changes a 1990 seating that is ON the default path ... MEASURE whether the 1990 start hash moves; if it does, STOP on that item and report it as a re-pin question rather than landing it, because the protocol forbids a re-pin while E-3 is red."

**LANDED.** R3(d) in 78d14fe: `PartySpec.successor_of_ruling_party`, the const builder `.successor()`, the reader `government::successor_of_ruling_party`, 24 rows flagged from `blocs4-data/successor-flag.json` with a sourced `// successor_of_ruling_party:` comment each (nine candidates refused there: the PDS has no row; Romania's FSN "split from" the PCR; SNEK, Equal Rights, Moldova's Socialist bloc, Georgia's CP and three more unsourced), the new test `every_successor_flag_has_a_source` (watched red twice), and A5 in `bloc_census.rs` re-expressed to the ruling — a ballot after which a flagged party that did not lead before leads the coalition, counted as distinct ex-communist nations, the bar >= 3 by end-1996 in a majority of seeds. Reading N=200 (`census_n200_flag`): 0/200 with >= 1 nation, 0/200 with >= 3, 0/200 over the run; every other summary line identical to H-4's. R3(e) in f247909: `military.personnel_1990` in 125 nation files and `military.milex_1990_usd_bn` in 118, one sources entry each; not written and recorded in the file — Comoros, Tonga, Yugoslavia (neither field, the data pass's refusal), Bahamas, Seychelles, Taiwan, USSR (no personnel), Bhutan, Czechoslovakia, Equatorial Guinea, Iraq, Libya, Maldives, Sao Tome (no milex), the personnel refusals for Iceland, Samoa, Solomon Islands and Vanuatu (the World Bank series is NULL for all five years, fetched 2026-09-06 — no figure, so REFUSED; this entry and the four files first called them 'sourced zeros' on the Factbook's 'none', corrected in H-7), the zeros the loader cannot carry (the World Bank's 0 milex for Albania, Costa Rica, Cuba, Iceland, Mongolia, North Korea), and ONE refusal of the integrator's own: Costa Rica's 8,000 (the Fuerza Publica) against the roster's `mil_spend_gdp` 0.0. `the_army_target_is_the_share_arm_off_and_the_pay_arm_is_the_ruled_form` now reads 125 of 137 with data, 12 refused, every Army target bit-identical off and on at the shipped weight. Default path after both: 0xe26e4bf8d6c60066 / 0xbe94d6125631829c, `run 35 1990` d1a2cfbf7c6958d7 (3501), `run 35 7` 39dea3341a7f6e8c (3983), twice each. Full suite on f247909: sim lib 457 / 3 / 25 (the three deliberate reds; +1 the flag test), the 28 integration binaries green (`bloc_census` 4 pinned bars, 26.5 s), web 160 / 0 / 2, cli 1 / 0, the two P-12 bars green.

**NOT LANDED, measured (the transcriptions and the exact patches measured are kept under `docs/political-arm/*-pending*`, the way D4's were).**

1. R3(a), the successors' dormant tables (`successors-soviet-1990-pending.txt`, `successors-yugoslav-1990-pending.txt`; ids kept, `term_months` and `ruling` kept from the standing blocks because neither was fetched, `ba_srsj` the one new row in an existing block). Applied to `POLITIES` (Russia held back, see below): the 1990 start UNMOVED at 0xe26e4bf8d6c60066 — the tables are dormant on day one — but the timeline golden MOVES, 0xbe94d6125631829c → 0x331bcb3aec5ff3cd, and both digests with it (250907c447b10799 / 3914 lines; 70d32d556423cb94 / 3631), because a successor born inside the run seats its coalition from its table and both unions come apart inside the twenty-year golden and the thirty-five-year digests. Bisected: the six Yugoslav blocks alone move it to 0x5cf655a9e27fe598 (cb13644a5f4d5394 / 3732; 5047e0bfee888e76 / 4037), the fourteen Soviet blocks alone to 0x23d6e6903ded6141 (74f93ed187b9fc20 / 4022; 57dc43f607ec8a1e / 3828). Existing tests that go red with it: `the_bloc_layer_is_inert_over_time` and `the_resource_layer_is_inert_over_time` (both pin the timeline actual), and `every_bloc_override_has_a_source`, whose override count is pinned at 34 "as integrated on 2026-09-05" and reads 36 (+`ua_demblok` +`uz_birlik` +`md_fpm` −`az_msi`; 37 with Russia's `ru_dr`). RUSSIA is a fourth question on its own: the March 1990 Congress table drops `ru_apr` (the Agrarian Party was founded in 1993), and `blocs::tests::the_pillar_map_and_overrides_read_as_transcribed` spot-checks `bloc_of(Russia, "ru_apr") == Communist`; keeping the id would be inventing a 1990 row, so Russia's 1993 table stands until Ridge rules on the one fixture line. Note the `Electoral::TwoRound` on Kazakhstan, Tajikistan, Turkmenistan and Moldova is the transcription's own flagged assumption.

2. R3(b), Algeria's 1987 table (`algeria-1987-pending.txt`: FLN 1.00, the FIS present at 0.0, `next` (1991, 6), the June 1990 and December 1991 results as calibration, not data). Algeria's table is LIVE, so the 1990 start itself MOVES, 0xe26e4bf8d6c60066 → 0x4d97dccde6c9d69f; the timeline 0x86abedab335ac3af; digests c4f9341ea1e37afb / 3572 and 91f30e0bf2067e65 / 4017. Existing tests red: `the_bloc_layer_is_inert_at_1990` (pins the start actual), and the two Algeria tests `the_algeria_shaped_annulment_fires_under_its_conditions_and_not_under_the_court` and `a_paid_army_seats_the_islamists_and_a_hostile_one_annuls_the_algerian_way`, because `would_be_leader` now reads `dz_fln` — the FIS's rise from 0.0 to the 1991 first round is the model's to produce, as the ruling intends, and on this tree it does not produce it (the FLN wins the December 1991 vote in the off world). That is the measurement the ruling asked for, and it says the transcription cannot land until the model's discontent → movement channel carries a party from zero, which is S5-7's `drift_support` finding again with a harder case.

3. R3(c), the seven regimes (`seven-regimes-1990-pending.json`, `seven-regimes-overrides-pending-a8b35a1.patch`): the fetched sources say four rows are wrong on 1 January 1990 and three are right — Angola (MPLA, Marxist-Leninist until its third congress in December 1990) and Sao Tome (MLSTP, Marxist-Leninist until the October 1990 congress) read Communist; Fiji (the unelected interim administration of 5 December 1987 under the 1990 ethnic-Fijian constitution) and South Africa (the National Party the source dates far-right, Afrikaner-nationalist, 1948-1990) read Nationalist; Cape Verde (no fetched source says the PAICV itself was Marxist-Leninist; Communist REFUSED), Malawi (a one-party conservative, anti-communist state) and Mozambique (Marxism-Leninism dropped at the fifth congress, 24-30 July 1989) stay Western. Built as sourced `bloc_override`s on the four leader rows (the party rows carry dated post-1990 shares whose families are right for those dates), URLs into `sources`, the evidence into `note`. Measured: the default path BYTE-IDENTICAL (both goldens, both digests unmoved), the inertness four green, `every_leader_row_loads_and_ties_to_its_polity` green, and exactly ONE red: `blocs::tests::the_1990_ruling_bloc_census` pins the 1990 census `[67, 17, 7, 3, 43]` "as transcribed on 2026-09-05" and reads `[63, 19, 9, 3, 43]`. Nothing on the default path moves; the question is only whether that pinned vector is re-expressed to the fetched sources, which is a test touch and so Ridge's.

**The shape of the re-pin question.** All three stops are the same protocol: E-3 red forbids a re-pin, and the branch's rule that only the two named tests are touched holds. The D4 precedent (a table beside `POLITIES` served under the switch) would carry R3(a) and R3(b) with the default path byte-identical, but `d4_tables_are_read_under_the_switch_and_invisible_off` pins `D4_POLITIES.len() == 2`, so that route is a test touch too. Ridge's options, as the pass reads them: (i) re-pin the goldens and the six pinned readings once E-3 is settled and land all three; (ii) rule that the successor and Algeria tables join D4's under the switch and re-express the D4 shape test; (iii) re-pin only `the_1990_ruling_bloc_census`'s vector and land R3(c) now, since it moves nothing on the default path.


### H-6 — the calibration pass: the census re-read at N=60 and N=200, no constant moved (each permitted move pinned by an existing test or short by arithmetic), the four pins re-derived and re-watched, the switch stays OFF

Every number here was measured by this pass on this tree (`feat/ideology-history` at 6f95332, whose code is f247909's), each build watched (`Compiling spheres-sim` read before any reading was believed), `CARGO_TARGET_DIR` the worktree's own (`blocs4-target`). Raw outputs in the run's scratchpad `blocs4-data/cal/` (`census_n60_base.txt`, `census_n200_cal.txt`, `census_n60_420.txt`, `verbose_s2.txt`, `derive.py` and its output, `mut_M1_bars_nocapture.txt`, `bars_unmutated.txt`, `gate_after_census_edit.txt`). The brief, quoted: "make the smallest permitted move with a HISTORICAL basis written in the comment (a dated event or a published rate), one move per commit, census at N=60 after each with before/after in the body; keep a move only if it helps its anchor without breaking another; revert and record otherwise" — and, binding on top of it, "NOTHING ELSE MOVES: no bar widened, no test beyond the two named re-expressed, the default path byte-identical".

**(1) The N=60 census on this tree against the 2e164ae baseline** (`census_n60_base`, 34.0 s; the baseline is S7-2's N=200 reading of 2e164ae, quoted in the brief). Per seed min/med/max (mean, sd): A1 coups against elected governments 5/6/8 (6.400, 0.887) against 4/6/9; top-3 share 0.75/1.00/1.00 (0.953) against 1.00 — Sao Tome 163, Philippines 141, Comoros 60, Algeria 16, Mozambique 2, Suriname 2 of 384; annulments 0/0/1 (0.483; Algeria 29/60) against Belarus 200, Ukraine 200, Algeria 105 of 200; regime coups 9/10/12 (10.467; Nigeria 62, Ghana 61, CAR / Comoros / Equatorial Guinea / Haiti / Mozambique / Zaire 60, Chad 59, Cambodia 57, Nepal 9, Bangladesh 6, Indonesia 4, Madagascar 3 of 628). A2 0/60 by 2000 (0/60 over the run; Algeria annulled 29/60; the largest Islamist influence in a winnable non-Islamist state 0.43/0.64/0.65) against 0/200. A3 33/60 = 0.550, Cambodia 33 of 33, per seed 0/1/1, against 197/200 (Belarus 181, Cambodia 142, Ukraine 91). A4 1/2/3 (1.583, 0.591; Sao Tome 60, South Africa 60, Kuwait 3, Morocco 3) against median 2. A5 0/60 against 0/200. A6 0 in 60/60, Argentina's collapse beside in 60/60. A7 ballot bloc flips 2/2/5 (2.633), party flips 2/4/6 (3.950), takeovers 12/15/17 (15.250), ratio 0.12/0.16/0.33 (bloc), 0.27 median (party), against 2/3/6 vs 14/19/24, ratio 0.16. A8 8/8 in 60/60. A9 1041/1041 = 1.000 against 0.953. A10 0.26/0.28/0.31 (0.281, sd 0.011) against median 0.326. Deaths 32/45/57 (43.883); round tables, suspensions, programmes 0 in every seed; uprisings by winner W 1/4/6, C 0/1/1, N 0, I 0, NA 0/2/3, none 7/7/10; the clause stab12 5/7/10, movement 3/6/9, both 0, none 0/1/4; the flag missed 39 of 377 crowns. What moved from 2e164ae is R2's: the two annulments of a merely-present army are gone, and with them A3's Belarus and Ukraine, A9's only rich-nation strikes, four takeovers a seed on A7's denominator, and A10's highest-lead events.

**(2) The moves examined, and why NONE was made.** The brief's WHAT-MAY-MOVE list, taken item by item against the rule that no existing test is touched and every change carries a fetched basis:

- *The AI's round-table lines* (A4; `government::ai_lever`: discontent >= 0.50, the largest non-ruling movement >= 0.35, the armed pillars' mean < 0.50, 60 PC). Historical basis fetched for the move that was drafted — the lines set to the lever's own refusal lines (0.40 / 0.25) and the 30 PC price, because the states that convened round tables were BROKE, not rich: Benin — "In 1989, riots broke out when the regime did not have enough money to pay its army. The banking system collapsed." (Wikipedia, Benin), "In December 1989 Marxism-Leninism was dropped as the state ideology" and "a national conference was held in February 1990" (Wikipedia, Mathieu Kérékou; Kérékou took about 32% against Soglo in 1991); Zambia — "In July [1990], amid three days of rioting in the capital, Kaunda announced a referendum", the coup attempt, the September reversal and "a snap general election for the following year", Kaunda "receiving only 24 per cent of the vote to Chiluba's 75 per cent" (Wikipedia, Kenneth Kaunda). NOT MADE: the four lines are pinned by the existing `the_ai_takes_each_lever_only_under_its_thresholds` (the largest at 0.34, an armed mean of 0.50, discontent under 0.50 and 59 held each assert `None`), so the move is a test touch. And it would not have reached the band anyway — measured with a scratch probe (four seeds, 84 months, not committed): 16-17 of the 1990 regimes reach discontent >= 0.40 at some month, but the largest non-ruling movement sits at 0.12-0.25 in all of them but Chad (0.40) and Sao Tome (0.99): Zambia holds D 0.60 for 36-37 months at 0.24 with PC 28-34 and an armed mean of 0.50, Zaire D 0.67 at 0.24, Mozambique D 0.57 at 0.23, Cambodia D 0.70 at 0.25; the AI's four lines coincide in 0 nation-months in every seed (the census's 0 levers in 200/200), and the lever's own refusal lines with the price admit Chad (1-51 months) and Sao Tome (15-21) alone — with the armed-mean line kept, Sao Tome alone (9-12 months in three seeds, 0 in one), which opens by drift in 200/200 already.
- *The flat seed* (A2, A4; `blocs::RULING_SEED` 0.60). The fetched figures that would calibrate it are the wave's first free votes (Kaunda 24%, Kérékou about 32%, above). NOT MADE: the existing `a_regime_s_movements_move_with_the_pains_and_revert` pins the seed at 0.5988 (`(seed - 0.5988).abs() < 1e-3`) and the quiet equilibrium in 0.65..0.80 — a test touch, and one for Ridge (S5-4's list said the flat seed could move; it cannot without that test).
- *The drift constants* (A2, A4; `drift_movements`' 0.006 rate, 0.015 bound, 0.005 reversion). The same test's fixed point, h = (0.35·r + 0.003) / (0.35·r + 0.005), bars any rate over 0.0143 (h = 0.80); the fetched rate — Zambia's MMD from nothing to 75% in the fifteen months from July 1990 to October 1991, 0.05 a month — is over the 0.015 bound itself, and no fetched figure sits under 0.0143. NOT MADE. And the rate is not the blocker: the record line the movements wear reads prices, growth and war and not the ORDER pain that carries half of discontent (S3's design, the party model's own line), so a regime at D 0.60 on order alone moves its movements at 0.35·0.006 toward the ruler. That is a model finding for the list under S5-6, not a constant.
- *The backing caps and the gravity view* (A2). By arithmetic on the pinned lines: an Islamist bloc present in Kabul through backing (M1 creates it from month 0, H-1) has share <= the flat seed's split, 0.40 over the blocs present (0.133 with three), and backing <= `BACKING_TOTAL_CAP` 0.25, so I <= 0.383 against the pinned 0.45 line; raising the cap to 0.35 would lift I to 0.48, but Kabul's army at its transcribed 15% defence share reads 0.85 on the share arm (M2's pay arm at weight 0, H-3) against `COERCION_ARMED` 0.35, so coercion never fails there. No fetched figure names a cap; NOT MADE. Gravity is dead by construction (no aid flow enters any regime, S5-6); NOT MADE.
- *The coercion lines* (A3). Both already quote the pillar model's unpaid line 0.35 (S5-2); the residual is Cambodia alone — the pre-existing regime coup by an Army whose live share sits near the 1% floor (weakest 0.28 at the coup, `verbose_s2`), painting the state Nationalist, then the deposed Communist movement retaking it where coercion fails at the same 0.28. No fetched figure sits under 0.35; NOT MADE. History's own Cambodian event is the CPP's seizure of sole power from FUNCINPEC, 5 July 1997 (fetched: "Co-premier Norodom Ranariddh is ousted by co-premier Hun Sen and exiled until 1998") — a successor party taking power by force, which the census reads as a Communist non-ballot takeover and the anchor's <= 10% does not admit. That is a question for Ridge about the anchor, recorded, not a widening.
- *The electoral pressure rate and gate, the uprising and round-table lines* (A1, A3): pinned as literals by `every_road_reads_closed_while_takeover_is_off` (S5-4). NOT MADE.
- *The two model pieces and the two rulings*: landed by the history pass (H-1..H-4) and not re-opened; M2's weight stands at the 0.0 its own sizing measured (H-3).
- *A5 and A7*: no listed constant enters `drift_support` or the ballot/takeover construction (S5-7). NOT MADE.

So the tree at the end of this pass is the tree at its start, code for code, and the census is the same census. Zero commits carry a move; the record above is what the brief asked for in their place.

**(3) The N=200 census on this tree** (`census_n200_cal`, 112.6 s; identical, summary line for summary line and seed for seed, to H-5's `census_n200_flag`, which is the same code). Per seed min/med/max (mean, sd): A1 5/6/9 (6.260, 0.858), top-3 share 0.75/1.00/1.00 (0.955, 0.072; 0 of 200 under 0.5; Sao Tome 541, Philippines 447, Comoros 200, Algeria 45, Mozambique 12, Suriname 7 of 1252), annulments 0/1/1 (0.525; Algeria 105), regime coups 8/11/14 (10.575; Nigeria 214, Ghana 202, Haiti 201, CAR / Comoros / Equatorial Guinea / Zaire 200, Mozambique 196, Chad 193, Cambodia 178, Nepal 45, Tanzania 24, Bangladesh 16, Sao Tome 14 of 2115); A2 1/200 by 2000 (Algeria once, over the run too), Algeria annulled 105/200, the largest Islamist influence 0.43/0.64/0.65; A3 117/200 = 0.585 (Cambodia 117), per seed 0/1/1 (0.585, 0.494); A4 1/2/3 (1.650, 0.616; Sao Tome 200, South Africa 200, Morocco 13, Kuwait 12, Mozambique 3 of 428), openings drift 2/3/5, uprising 0/1/2, lever 0, card 1/3/7; A5 0/200 by 1996 and over the run; A6 0 in 200/200, Argentina beside 200/200; A7 bloc flips 2/2/5 (2.650), party flips 2/4/6 (3.865), takeovers 11/15/21 (15.360, 1.463), ratio 0.11/0.15/0.33 (bloc, mean 0.174, sd 0.053), 0.25 median (party); A8 8/8 in 200/200; A9 3472/3472 = 1.000; A10 0.26/0.28/0.32 (0.279, sd 0.010); deaths 32/44/57 (43.530); levers, suspensions, programmes 0; uprisings W 1/4/6, C 0/1/1, N 0, I 0/0/1, NA 0/2/3, none 6/7/10; clause stab12 4/7/10, movement 2/6/9, both 0, none 0/1/5; the flag missed 132 of 1284 crowns (Sao Tome 46, Comoros 20, Mozambique 20, Zaire 20, Philippines 18, Cambodia 6, Chad 2). At 420 months, N=60 (`census_n60_420`, 59.0 s): A6 0 in 60/60, Argentina beside 60/60; A1 6/8/11 (8.333); A3 34/60; A8 60/60 keep six, per seed min 7, median 8 (7.967); the flag missed 46 of 473 crowns.

**The derivations (rule 7, `derive.py` over the 200 seed lines, a fixed-seed bootstrap of 4000 resamples), bar by bar.** A1 band arm: median 6, P(median outside 4..14 | n=12) = 0.0000 — pinnable alone; concentration arm: top-3 median 1.00, 0 of 200 under 0.5 — red at any n; the bar stays `#[ignore]`d (one test, two arms; splitting it is a re-expression for Ridge, S6-6). A2: p = 0.005, n = (2.326·0.071 / 0.495)² = 0.1 — red at any n; its annulment sub-anchor holds: Algeria annulled p = 0.525, P(0 of 12) = 0.0001. A3: p = 0.585, sd 0.493, n = (2.326·0.493 / 0.485)² = 5.6, P(more than 2 of 20 | 0.585) = 1.000 — red at any n. A4: median 2, mean 1.650, sd 0.616, P(median in 15..40 | n=12) = 0.0000 — red at any n. A5: p = 0.000 — red at any n, decorative until the model moves. A6: 0 in 200/200 and 60/60 at 420 months; an invariant, n a power budget (1 − (1 − q)^12: q = 0.10 → 0.72, 0.20 → 0.93, 0.30 → 0.99), unchanged. A7: median 0.148, sd 0.053, P(median >= 3 | n=12) = 0.0000 — red at any n. A8: p = 1.000 in 200/200, Bernoulli n = 0; 40 for power, exact binomial P(fewer than 24 of 40) at a true rate of 0.50 / 0.40 / 0.30 = 0.866 / 0.992 / 1.000, unchanged. A9: pooled 1.000, per-seed share 1.000 in 200/200, sd 0.0000, n = 0; power at twelve seeds, the per-seed shares shifted down and pooled: by 0.25 → red 1.000, by 0.20 → 0.400, by 0.15 → 0.000 — the bar sees a quarter of the coups moving into rich nations and not a fifth (on 2e164ae: sd 0.0151, n = 0.05, power 0.75 / 0.80 / 0.85 → 1.000 / 0.30 / 0.00). A10: per-seed median 0.279 median, 0.2791 mean, sd 0.0103, min 0.257, max 0.32; n = (2.326·0.0103 / 0.0291)² = 0.68; P(median < 0.25 | n=12) = 0.0000; power: the lead falling by 0.04 → red 1.000, by 0.03 → 0.652, by 0.02 → 0.005 — the bar sees a seventh and not a fourteenth (on 2e164ae: sd 0.0333, n = 1.1, 0.10 / 0.08 / 0.06 → 0.92 / 0.57 / 0.18; the margin to the bar is 0.03 now where it was 0.07).

**The four pinned bars on this tree** (`bars_unmutated`, a fresh watched build, 28.8 s together): A6 0 in 12/12 at 420 months, the collapse beside in 12/12; A8 40/40, per seed 8; A9 206/206 = 1.000; A10 median 0.280, per seed 0.265..0.311. **Re-watched red** (`mut_M1_bars_nocapture`, a fresh watched build with `COERCION_ARMED` / `COERCION_MEAN` 0.35 → 1.00 and `UPRISING_DISCONTENT` / `UPRISING_INFLUENCE` 0.45 → 0.00, the source restored and `git diff` empty after): A6 RED, 35..53 route events a seed in 12/12; A8 RED, 8/40 kept six, per seed 1..7; A10 RED, median lead 0.020, per seed 0.014..0.026; A9 GREEN at 44/44 = 1.000 (decorative against the roads, as its comment says). The comments beside all four now carry this tree's sample, n and power with 2e164ae's beside them; the six ignored bars' READING paragraphs carry this tree's figures and the reasons above.

**(4) THE SWITCH: `ideology_takeover` stays OFF** (`play_rules` untouched; the watch keeps reading `calibration pending`). Out at N=200: A1's concentration arm — top-3 share 1.00 against under 0.5 (three micro-polities hold every coup of the median seed); A2 — 1/200 seeds against a majority (the whole bar); A3 — 58.5% of seeds against <= 10% (48.5 points, Cambodia alone; 2e164ae was 88.5 points out); A4 — median 2 against 15..40 (13 short of the floor); A5 — 0/200 against a majority; A7 — 0.148 against >= 3 (twenty-fold). Inside: A1's band arm (median 6), A2's annulment sub-anchor (Algeria 105/200), A6 (0 in 200/200; 0 in 60/60 at 420 months), A8 (200/200), A9 (1.000), A10 (0.279), A12 (`the_daily_clock_preserves_the_political_arm_on_world`, green in the suite below), A13 (the suite below, the switches off). What each out-of-band bar needs is not a constant: A1 a per-soldier reading of the Army target with a denominator the ruling did not name (H-3's table of Powell's own variable, for Ridge); A2 a movement that can grow from discontent (the record line's missing order term) under an army whose loyalty can fall when its paymaster stops (the pay arm's denominator again), or the seed and the caps re-pinned; A3 Ridge's reading of Cambodia 1997 against the anchor, or Cambodia's live defence share; A4 the flat seed and the record line above; A5 `drift_support` (the party model's own, S5-7); A7 the pre-existing regime coups' count and the bloc-level construction (S5-7).

**(5) The suite on the tree that ships this pass**, the census file's comments the only change to the code tree: the gate after the edit (`gate_after_census_edit`) — `the_1990_start_is_pinned` actual 0xe26e4bf8d6c60066 and `golden_hash_of_a_known_run` actual 0xbe94d6125631829c unmoved, the inertness four green (8.9 s), `run 35 1990` d1a2cfbf7c6958d7 (3501 lines, stderr 0) and `run 35 7` 39dea3341a7f6e8c (3983, 0) unmoved, data integrity 27/27; the full suite's counts are in the census commit's body and ROADMAP's entry.

### H-7 — the ship pass: the two skeptics' findings repaired at the source, the suite watched from a clean build, the digests and the live page read, the branch fast-forwarded

The repair-and-ship pass of 2026-09-06 on `feat/ideology-history` (commits 78d14fe..37c091f plus this one), every number below measured or fetched by this pass on this worktree (`CARGO_TARGET_DIR` `blocs4-target`, `cargo clean -p spheres-sim -p spheres-web -p spheres-cli --release` first, every `Compiling spheres-*` line read). The history skeptic and the determinism skeptic both re-ran the census and the gate and CONFIRMED every measured figure in H-5 and H-6; what they DISPROVED was wording, and each item is corrected at the place it was written rather than in a note beside it:

1. **Powell (2012) was overstated in `army_loyalty_target`'s doc.** The comment said the paper "finds military expenditure per soldier the strongest deterrent of coup attempts in the 1950-2000 record". Fetched this pass (OpenAlex, doi 10.1177/0022002712445732, JCR 56(6) 1017-1040): the abstract's sample is "a global sample from 1961 to 2000" and its stated finding is that "characteristics of military appear to be far more important than economic influences on coups"; "per soldier" and "strongest deterrent" are not in the abstract, and the body was not fetched (SAGE 403 for the skeptic). The comment now quotes the abstract and says the per-soldier reading is the ruling's, not the paper's. The arm itself is unchanged — `ARMY_PAY_WEIGHT` 0.0, the form as ruled and labelled INVENTED (H-3).
2. **Operation Cyclone was overstated in `politics.rs`' programme comment and in H-1.** "Appropriated and delivered every fiscal year from 1980 to 1992" is not on the page: fetched (Wikipedia, Operation Cyclone), the programme ran 1979-1992, "$695,000 USD in mid-1979, ... $20-$30 million per year in 1980, and rose to $630 million per year in 1987", and Saudi Arabia "agreed to match dollar for dollar the money the CIA was sending"; "every fiscal year" appears nowhere on it. Both sentences now carry the page's figures and nothing past them. `SPONSOR_PROGRAMME_HEAT` is unchanged (derived, 0.06 = 0.18 − 0.012 · (0.06 / 0.006), re-checked by the skeptic against `statecraft.rs`).
3. **"Sourced zero" was the wrong word for four personnel refusals.** `iceland.json`, `samoa.json`, `solomonislands.json` and `vanuatu.json` said the personnel figure was "a SOURCED ZERO (no armed force)"; fetched this pass, `MS.MIL.TOTL.P1` is NULL for ISL, WSM, SLB and VUT in every year 1988-1992, so no figure was fetched and the field is a REFUSAL under iron rule 4 — the Factbook's "none" for defence is a statement about branches, not a personnel count. The four entries and H-5's sentence now say so. No field value changes (the fields were absent already), so the data pass's count stands: 125 nations carry `personnel_1990`, 12 living nations are refused, and the M2 test prints the same line.
4. **Three existing tests were touched on this branch, not two.** The rulings named `the_bloc_layer_is_inert_over_time` (R1's government clause) and `the_algeria_shaped_annulment_fires_under_its_conditions_and_not_under_the_court` (R2's Jordan reading). H-1 recorded, and the skeptic confirmed, two more changed assertions: the inertness test's `FIRST_ACT` pin moved from (35, Libya) to (0, Afghanistan) because M1's programme acts in month 0, and `gravity_appears_with_an_aid_flow_and_vanishes_the_month_it_stops`' "nothing was stored" narrowed from the whole backing store to Egypt (the clause's own claim) because the same month-0 act stores Pakistan's backing in Afghanistan. Both are consequences of building M1 as ruled, both are narrowings (the skeptic's diff of every changed `assert` found no widening), and neither is undone here because undoing them means not building M1. They are listed for Ridge as the deviation they are.
5. **R3(a), R3(b) and R3(c) are NOT on the tree**, exactly as H-5 says and the brief's summary sentence does not: `government.rs` still carries Russia's 1993 and Ukraine's 1994 tables, Algeria's June-1990 shares (the FIS at 0.542), and the seven regimes' 2e164ae families. The three stops are re-pin questions (E-3 red forbids a re-pin; the D4 route is a test touch on `D4_POLITIES.len() == 2`) and stay under `docs/political-arm/*-pending*`. The skeptic also read the FRELIMO date: Wikipedia gives only "1989" for the fifth congress (fetched again this pass, the same), and the 24-30 July date comes from Marshall's Southern Africa Report piece (marxists.org PDF), which is the source the pending JSON attributes it to.

**The suite, watched from a clean build** (`blocs4-suite.txt` in the run's scratchpad; `cargo test --release --workspace --no-fail-fast`, the build 1 m 06 s, the run 11:23:07-11:28:43): 32 result lines, 839 passed / 3 failed / 65 ignored in all; sim lib 457 / 3 / 25 (192.94 s) with the three deliberate reds EXACTLY — `tests::the_1990_start_is_pinned` actual 0xe26e4bf8d6c60066, `tests::golden_hash_of_a_known_run` actual 0xbe94d6125631829c, `tech::tests::the_1990_endowment_does_not_move_year_one_growth` (E-3) — and `the_resource_pass_stays_under_budget` and `the_resources_row_is_free` green in the same run (no P-12 re-run needed); `bloc_census` 4 / 0 / 7 (25.65 s); `daily` 7 / 0 (37.90 s); every other integration binary green; web 160 / 0 / 2 (46.45 s); cli 1 / 0. Digests, market OFF, twice each on the fresh `spheres-cli.exe`: `run 35 1990` d1a2cfbf7c6958d7 / 3501 lines / stderr 0 (a and b); `run 35 7` 39dea3341a7f6e8c / 3983 / 0 (a and b). The N=200 census is H-6's (no code moved since 6f95332; the files this pass touches are two comments, a nation-file `sources` string each, and the documents).

**The live page** (`spheres-web.exe --port 7930 --no-open`, the release exe built from this tree, driven through the setup screen; console errors: none — the only console entries were the browser harness's own notes that it suppressed the page's "replace this server's live world?" confirm, which was then answered by stubbing `confirm` to true from the console). ALGERIA, seed 7: on 1 Jan 1990 the bar reads Western 4.5% / Communist 0.3% / Nationalist 32% / Islamist 61% / Non-Aligned 1.8%, the FIS at 61% support and 79% of the seats "leads the government", Chadli Bendjedid President — the June-1990 table, R3(b)'s stop, seated on day one. Twenty-four months at speed 5 later (8 Jan 1992): Islamist 62.1% (the FIS 62.1% support, 79.4% seats, still leading), Nationalist 31.6%, discontent 21%, the December 1991 vote held and the chamber's next election 1996-12 — the FIS does not RISE because it starts at the top; nothing is annulled because the ANP reads 65% in play (the pillars do not walk while `ideology_takeover` is off). The takeover watch reads all four roads CLOSED — CALIBRATION PENDING, with the gauges: coup (army loyalty 65% ≤ 35%, discontent 21% ≥ 25%, coup pressure 0% ≥ 100%), uprising (discontent 21% ≥ 45%, challenger influence Nationalist 32% ≥ 45%), round table (Western influence 4.4% ≥ 40%, party loyalty 65% ≥ 55%, stability 41 in 30..70), collapse (stability 41 ≤ 12). AFGHANISTAN, seed 7: on 1 Jan 1990 an Islamist segment IS on the bar at 0.2% (the flat seed's split, M1's presence), Communist 59.9% rules, discontent 57%, no backing yet; by 28 May 1990 the segment carries "+0.7% abroad" hatched, backed by IRAN, marked exposed — the dispatch of 31 Jan 1990 reads "Afghanistan exposes Iran backing the Islamist movement in Afghanistan — the scandal rallies the country behind its government"; by 24 Feb 1991 Iran's stock has cooled away and the backer is PAKISTAN (+1.9% abroad, exposed 31 Dec 1990 with the same headline), the army at 74% — the presence is created and re-created by the programme and Kabul exposes it, which is H-1's (a) seen on the page. IRAQ, seed 7, a year at speed 5 (1 Jan 1990 → 1 Feb 1991): the clock stopped on Iraq's own escalation interrupts ("Iraq escalates to rung 2 — sanctions", "rung 3 — arms to a proxy") and the dispatch log holds 32 political headlines, every one read — 25 ballots (Japan, Nicaragua, Costa Rica in February 1990; Hungary, Colombia, Australia, Comoros in March; Greece and Peru in April; Romania and the Dominican Republic in May; Czechoslovakia, Bulgaria — "Bulgarian Socialist Party forms a 2-party coalition holding 84% of the seats" — and Ecuador in June; Brazil, Austria, Malaysia, New Zealand in October; Guatemala in November; Germany, Denmark, Guyana on 1 December; Argentina and El Salvador after their governments fell on 31 December), "Revolution in Peru — the old regime falls" (2 Jun 1990), two conscription endings (Norway, Australia), and three deaths in office (Jaber Al-Ahmad Al-Sabah 31 Jan 1990, Carlos Menem and Steingrimur Hermannsson 31 Jan 1991); Iraq's own government screen after the year: Saddam Hussein, the RCC, Nationalist 58.9% rules, Non-Aligned 40.5%, discontent 17%, pillars 74 / 77 / 75%, all four roads closed — calibration pending. The server was killed at the end of the readings.

**The switch:** `ideology_takeover` stays OFF in play (H-6 (4) unchanged; the watch reads calibration pending on every road above). `ideology_blocs` is on in play and off headless, as before. Pushed as a fast-forward of `feat/hoi4-map-and-tech`.
