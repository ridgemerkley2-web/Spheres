...
**Construction/industry amendment, approved 2026-09-07:** browser play enables
`GameRules.industry_rebuild`. [INDUSTRY_REBUILD.md](INDUSTRY_REBUILD.md) supersedes
earlier cash-only construction and estate prerequisites with shared daily
capacity, installation inputs and funded operation. Buildings and mines share
one frozen daily allocation; offices and advanced components use the existing
GDP and trade ledgers; naval slots are separate from military factories.
Default-false legacy replay and its calibration pins remain unchanged.
...
**Shared recovery amendment, approved 2026-09-08:** the daily opt-in model in
`docs/FISCAL_RECOVERY.md` supersedes the two-path accounting and free fiscal AI
below for enabled campaigns. All countries seat the existing explicit books;
missing cash opens at zero while debt is preserved. Actual cash receipts feed a
bounded trailing monthly history, debt trajectory and interest/revenue review.
After an initial twelve observed months, sustained unsustainable finances can
accumulate a single fiscal-confidence stability arm, capped at 0.20 raw pressure
(0.05 stability points per model month before other pressures and mean reversion).
No duplicate inflation or service-cut penalty is charged. Quarterly AI uses
ordinary priced tax, budget and existing restructuring commands; the player uses
the same assessment without automatic policy enactment. Forecasts disclose their
constant-policy assumptions and remain distinct from full-authority budget drafts.
The existing unlimited financing helper remains; this model does not invent an
arrears ledger, credit market, maturity schedule or automatic civil-war event.
