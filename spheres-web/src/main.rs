...
mod tests {
    ...
    fn the_effort_dial_reaches_the_sim_through_the_command_route() {
        ...
        let cmd = parse_command(&g.world, &posted, NationId::USA).expect("the UI's shape must parse");
        match &cmd {
        ...
        let cmd = parse_command(&g.world, &posted, NationId::USA).expect("abandon must parse");
        apply_command(&mut g.world, &cmd).unwrap();
