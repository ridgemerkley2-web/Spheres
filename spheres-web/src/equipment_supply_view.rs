...
fn equipment_project_supply(_w:&WorldState,_me:NationId,p:&eq::ProjectSupply)->Value {
    ...
    if p.earliest_work_day==i32::MAX {
        // The equipment state uses MAX for finance that has not been activated.
        // It is not a calendar date and must never reach the date formatter.
        warnings.push("Equipment programme funding has not been activated; no eligible work date is available yet.".into());
    ...
    let mut metrics=vec![metric("Planning date",super::settled_day_json(p.plan_day)["label"].clone()),
...
mod supply_view_tests {
    ...
    fn fixture()->super::super::Game {
        ...
        g.world.production.provinces.push(spheres_sim::production::ProvinceCapabilities{
            district:district.clone(),infrastructure:0,civilian_industry:0,power_grid:1,research_centers:0,arms_plants:3});
        ...
        g.world.production.operations.advanced_components.insert(NationId::USA,5.0);
        g
    }
    ...
    #[test]
    fn inactive_equipment_finance_has_no_invented_calendar_date_in_a_pure_review() {
        let mut g=fixture();
        ...
        assert!(q["supply"]["warnings"].as_array().unwrap().iter().any(|r|
            r.as_str().is_some_and(|s|s.contains("funding has not been activated")&&s.contains("no eligible work date"))),"{q}");
        assert!(q["supply"]["resources"].as_array().unwrap().iter().all(|r|r["planned_day"]==0.0));
        assert_eq!(spheres_sim::save(&g.world),before);
    }
    #[test]
    ...
    fn fabrication_gap_survives_save_and_is_visible_in_the_project_review() {
        ...
        // The plant has operating fuel, but no fabrication iron or copper.
        // This isolates a material gap from a separate factory-power outage.
        g.world.resources.market=Some(spheres_sim::resources::MarketState{
            ...
            stocks:vec![spheres_sim::resources::Stock{nation:NationId::USA,commodity:spheres_sim::resources::Commodity::Coal,quantity:1000.0,reserve_target:0.0}],cash:vec![],
        ...
        let plan=eq::supply_plan(&g.world,NationId::USA);
        assert!(plan[0].planned_work_days>0.0);
        assert_eq!(plan[0].executable_work_days,0.0,"missing fabrication inputs must prevent actual work");
        assert!(plan[0].advanced_components_available>=plan[0].advanced_components_required);
