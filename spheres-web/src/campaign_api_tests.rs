...
#[test]
fn warfare_adoption_is_explicit_and_load_retains_the_selected_rule() {
    let mut g=Game::new(1990,Some(NationId::France));play_rules(&mut g);
    let old=save(&g.world);
    assert!(!spheres_sim::campaign::enabled(&g.world));
    let reading=warfare_adoption_json(&g.world,NationId::France);
    assert_eq!(reading["enabled"],false);assert_eq!(save(&g.world),old);
    let command=parse_command(&g.world,&json!({"kind":"enable_operational_warfare","nation":"Japan"}),NationId::France).unwrap();
    assert!(matches!(command,Command::EnableOperationalWarfare{nation:NationId::France}));
    apply_command(&mut g.world,&command).unwrap();
    assert!(spheres_sim::campaign::enabled(&g.world));
    let adopted=save(&g.world);
    assert_eq!(save(&loaded_play_game(load(&adopted).unwrap()).world),adopted);
    assert!(!spheres_sim::campaign::enabled(&loaded_play_game(load(&old).unwrap()).world));
}
...
#[test]
fn recorded_warfare_review_uses_native_adoption_and_executed_daily_reports() {
    let mut adoption=Game::new(1990,Some(NationId::France));play_rules(&mut adoption);
    let before_saved=save(&adoption.world);
    let before=warfare_adoption_json(&adoption.world,NationId::France);
    assert_eq!(before["available"],true);assert_eq!(save(&adoption.world),before_saved);
    let command=parse_command(&adoption.world,&json!({"kind":"enable_operational_warfare"}),NationId::France).unwrap();
    apply_command(&mut adoption.world,&command).unwrap();
    let after=warfare_adoption_json(&adoption.world,NationId::France);
    assert_eq!(after["enabled"],true);
    let (mut game,cid)=fixture();
    let request=json!({"session_id":game.session_id,"client_id":"s04-review","request_seq":1,"commands":[operation(cid)]});
    assert_eq!(immediate_request(&mut game,&request).unwrap()["errors"],json!([]));
    for _ in 0..3 {game.advance_days(1,vec![]);}
    let saved=save(&game.world);let state=state_json(&game,None);
    let conflict=conflict_value(&state,cid);
    assert_eq!(conflict["operation"]["enabled"],true);
    assert_eq!(conflict["operation"]["order"]["reserve_bp"],1234);
    assert!(conflict["operation"]["last_report"].is_object(),"The recorded display must have a real resolved daily report");
    assert_eq!(save(&game.world),saved,"Exported views must be pure");
    if let Ok(file)=std::env::var("SPHERES_S04_UI_FIXTURE") {
        std::fs::write(file,serde_json::to_vec_pretty(&json!({
            "format":"spheres-s04-ui-fixture","version":1,
            "fixture_note":"Synthetic native QA: France reviews and explicitly enables warfare; a separate Iraq/Iran test conflict starts at authored fixture escalation. Its operation is issued through the real browser parser/receipt channel and resolved by three complete daily ticks. No battlefield report, equipment, supply or historical war outcome is fabricated.",
            "adoption":{"nation":"France","date":adoption.world.date_str(),"before":before,"after":after},
            "conflict":conflict,"state":state})).unwrap()).unwrap();
    }
}
