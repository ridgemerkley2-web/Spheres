...
fn equipment_service_board(w:&WorldState,me:NationId)->Value {
    ...
    let recorded=s.filter(|s|s.last_tick_day.is_some());
    ...
    let mut metrics=vec![metric("Delivered custom vehicles",delivered),metric("Available for operations",available),metric("Withdrawn for refit",reserved),metric("Completed vehicles in transit",incoming),metric("New vehicles awaiting fabrication",fabrication),metric("Current fleet maintenance requirement",service_money(requirement))];
    if let Some(s)=recorded {
        metrics.push(metric("Last settled maintenance coverage",format!("{:.1}%",s.maintenance_fraction*100.0)));
        metrics.push(metric("Last maintenance earmark",service_money(s.maintenance_allocated_today_bn)));
    }
    ...
    json!({"title":"Your fleet in service","status":if delivered<=0.0&&incoming>0.0{"Awaiting delivery"}else if delivered<=0.0{"No custom vehicles delivered"}else if available==0{"Fleet withdrawn for refit"}else if !warnings.is_empty(){"Review fleet support"}else{"Fleet in service"},
        "detail":"Delivered holdings include refit reservations; available vehicles exclude them. Transit and unfinished production add no fielded capability. Maintenance requirements reflect current holdings, while coverage and earmarks describe the last settlement. National composition includes inherited equipment and recorded support; these are operational role effects, not per-vehicle ratings or battle predictions.",
...
fn equipment_retirement_preview(w:&WorldState,me:NationId,session:&str,command:&Value,parsed:&Command,revision:&str,quantity:u32)->Value {
    ...
    let effects=json!({"title":"Fleet after retirement","status":if valid{"Review permanent withdrawal"}else{"Retirement unavailable"},
        ...
        "warnings":["Retirement cannot be undone by the game. Replacing these vehicles requires new production or delivery.","Reduced maintenance need is not a cash saving or a budget cut. Existing appropriations and already-recorded payments stay unchanged.","Refit reservations, pending deliveries, production contracts and saved design revisions are retained."]});
...
mod service_view_tests {
    ...
    fn service_reading_separates_available_reserved_and_incoming_without_changing_world() {
        ...
        assert_eq!(metric_value(&data["service"],"Current fleet maintenance requirement"),"$2.478k / day");
    ...
    fn empty_fleet_explains_next_step_without_inventing_service_coverage() {
        ...
        assert_eq!(metric_value(&data,"Current fleet maintenance requirement"),"$0.00 / day");
