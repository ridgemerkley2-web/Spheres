...
fn ammunition_board(w:&WorldState,me:NationId)->Value {
    ...
    if let Some(state)=state {for o in &state.orders {
        ...
        orders.push(json!({"id":o.id,"name":format!("{} · batch {}",eq::ammo_def(&o.family).map_or(o.family.as_str(),|d|d.name),o.id),"status":format!("{:?}",o.status),"detail":o.reason,
            "progress":o.work_rounds/o.quantity as f64,"metrics":[metric(if aviation{"Completed mission stores"}else{"Completed rounds"},format!("{} / {}",ammo_quantity(o.completed_rounds as f64),ammo_quantity(o.quantity as f64))),metric("Province",spheres_sim::districts::name_of(&o.district).unwrap_or(&o.district)),metric("Maximum physical production",format!("{} {unit} / day",ammo_quantity(o.rounds_per_day))),metric("Current factory operating capacity",format!("{} {unit} / day before batch funding and raw inputs",ammo_quantity(o.rounds_per_day*eq::ammo_operating_fraction(w,&o.district))))],
    ...
    let detail=match order {
        EquipmentOrder::AmmoOrder{family,district,quantity,daily_budget_bn}=>{
            ...
            if spheres_sim::industry_operations::enabled(w) {
                metrics.push(metric("Current factory operating capacity",format!("{} / day",ammo_quantity(q.operating_rounds_per_day))));
                requirements.push(format!("Workers, electricity and operating support currently permit {:.1}% of nominal factory throughput. Raw inputs, the batch funding ceiling and shared fleet-maintenance payments can reduce actual production further.",q.operating_fraction*100.0));
            }
