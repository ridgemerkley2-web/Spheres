fn maintenance_action(w:&WorldState,me:NationId)->Value {
    ...
    let required=eq::fleet_maintenance_requirement(n)+eq::legacy_maintenance_requirement(n);
...
fn maintenance_board(w:&WorldState,me:NationId)->Value {
    let n=w.nation(me);let plan=n.equipment.as_ref().and_then(|s|s.maintenance_plan.as_ref());
    let custom=eq::fleet_maintenance_requirement(n);let legacy=eq::legacy_maintenance_requirement(n);
    ...
    readiness.push(json!({"label":"Refit withdrawal","value":format!("{reserved} custom vehicles unavailable"),"detail":"Reserved vehicles return when conversion finishes or their reservation is released. Manufacturer contracts cancel and release only unstarted vehicles."}));
...
fn maintenance_preview(w:&WorldState,me:NationId,session:&str,command:&Value,limit:f64)->Value {
    let n=w.nation(me);let custom=eq::fleet_maintenance_requirement(n);let legacy=eq::legacy_maintenance_requirement(n);let required=custom+legacy;
