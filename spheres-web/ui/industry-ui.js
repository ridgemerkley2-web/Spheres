...
function industryUnifiedHtml(data) {
  ...
  const cards=facilities.map(row=>{
    ...
    const metrics=[metric("Installed capacity",number(row.installed_capacity),"Facility equivalents"),metric("Operable capacity now",number(row.operating_capacity),"After current operating constraints"),metric("Latest output",output,row.inherited?"Inherited output is already included in national GDP":recorded?"Latest settled day":"No settled output yet"),metric("Estimated jobs filled",number(row.jobs_filled),`${number(row.jobs_required)} required`),metric("Power used / required",`${number(row.power_used_daily)} / ${number(row.power_required_daily)}`,"Modeled units per day")];
    if(Number.isFinite(row.actual_operating_capacity)) metrics.push(metric("Last operated capacity",number(row.actual_operating_capacity),recorded?"Dated operation; may differ from current potential":"No settled operation yet"));
