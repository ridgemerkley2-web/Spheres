...
function industryUnifiedHtml(data) {
  ...
  const cards=facilities.map(row=>{
    const recorded=Object.prototype.hasOwnProperty.call(row,"recorded_day")?Number.isFinite(row.recorded_day):settled;
    const receiptDay=Number.isInteger(row.recorded_day)?row.recorded_day:ops.as_of_day;
    const receipt=recorded?(Number.isInteger(receiptDay)&&receiptDay>=0?"Recorded · "+new Date(Date.UTC(1990,0,1+receiptDay)).toLocaleDateString("en-GB",{day:"numeric",month:"short",year:"numeric",timeZone:"UTC"}):"Latest settled day"):"No settled output yet";
    const percent=Number.isFinite(row.utilization)?Math.max(0,Math.min(100,row.utilization*100)):null;
    const origin=row.inherited?"Inherited national industry":"Completed facility";
    const unit=row.output_unit||"units / day", name=row.name||row.kind||"Facility";
    const district=row.district_name||(typeof DINDEX!=="undefined"?DINDEX[row.district]?.name:null)||(data.sites||[]).find(site=>site.district===row.district)?.district_name||row.district||data.name||"National";
    const actions=[];
    if(row.power_fraction<.999) actions.push({action:"construction",kind:"generation",district:row.district,label:"Review a Power Plant"});
    if(row.input_fraction<.999) actions.push({action:"resources",label:"Find imports"});
    if(row.funding_fraction<.999) actions.push({action:"budget",ministry:"industry",label:"Fund operations"});
    if(!row.inherited&&row.district) actions.push({action:"construction",district:row.district,label:"Review facility upgrades"});
    const output=recorded?(/\$bn\s*\/\s*day/.test(unit)?`${industryMoney(row.output_daily)} / day`:`${number(row.output_daily)} ${unit}`):"Awaiting first operation";
    const metrics=[metric("Installed capacity",number(row.installed_capacity),"Facility equivalents"),metric("Operable capacity now",number(row.operating_capacity),"After current operating constraints"),metric("Latest output",output,row.inherited?"Inherited output is already included in national GDP":receipt),metric("Estimated jobs filled",number(row.jobs_filled),row.inherited?`${number(row.jobs_required)} required`:`${receipt} · ${number(row.jobs_required)} required now`),metric("Power used / required",`${number(row.power_used_daily)} / ${number(row.power_required_daily)}`,row.inherited?"Modeled units per day":`${receipt} / requirement now · modeled units` )];
    if(Number.isFinite(row.actual_operating_capacity)) metrics.push(metric("Last operated capacity",number(row.actual_operating_capacity),recorded?receipt+"; may differ from current potential":"No settled operation yet"));
    ...
    if(Number.isFinite(row.cash_spent_daily_bn)) metrics.push(metric("Latest operating spending",recorded?industryMoney(row.cash_spent_daily_bn):"Awaiting first operation",receipt));
...
function industryContentHtml() {
  ...
  return `${hero}${status}${industryUnifiedHtml(data)}${data.enabled===false?`<p class="industry-message">Daily facility operation is not enabled in this campaign. Completed assets remain visible below.</p>`:""}<dl class="industry-stock">${goods}${power}</dl>
    ...
    ${data.industry_rebuild?.operations?`<details class="industry-receipts-details"><summary>Detailed facility receipts and research assignments</summary>`:""}<section class="industry-facilities" aria-labelledby="industryFacilitiesTitle"><div class="industry-section-heading"><div><p class="industry-kicker">Completed assets</p><h2 id="industryFacilitiesTitle">${data.industry_rebuild?.operations?"Facility receipts and supporting assets":"Your facilities"}</h2></div><button type="button" data-industry-refresh ${IDESK.loading||industryOrdersPending()?"disabled":""}>Refresh reading</button></div>${sites.length&&!data.industry_rebuild?.operations?toolbar:""}${cards}</section>${data.industry_rebuild?.operations?"</details>":""}
    ${industryQueueHtml(data)}${data.industry_rebuild?.operations?"":`<aside class="industry-inherited"><div><h2>The economy beyond these facilities</h2><p>Your inherited industry remains part of the national economy. Review its broader production and economic accounts alongside these facilities.</p></div>${industryButton({action:"inherited"},"See the wider national economy")}</aside>`}
