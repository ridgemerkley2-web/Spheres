...
(function (root, factory) {
  ...
  function adoptionHtml(view) {
    if(!view||view.enabled)return "";
    return `<section class="campaign-command" aria-label="Campaign operations upgrade"><span class="campaign-kicker">Theatre command</span><h3>Connect your military campaign</h3><p>${esc(view.effect)}</p>${view.reason?`<p role="status">${esc(view.reason)}</p>`:""}<button type="button" data-warfare-enable ${view.available?"":"disabled"}>Review operations upgrade</button></section>`;
  }
  return { html, peaceHtml, bind, adoptionHtml };
});
