...
(function (root, factory) {
  ...
  function successionStatus(status) {
    if (!status) return "";
    const fact = (name, value, hook) => `<div><dt>${esc(name)}</dt><dd data-gov-succession-${hook}>${esc(value || "Status not supplied")}</dd></div>`;
    return `<aside class="gov-ui-succession" data-gov-succession aria-label="Campaign succession status"><p class="gov-ui-kicker">Succession in your campaign${status.date ? ` · ${esc(status.date)}` : ""}</p><dl>${fact("Party leadership",status.party?.label,"party")}${fact("National office",status.national_office?.label,"executive")}${status.death_date ? fact("Campaign death recorded",status.death_date,"death") : ""}${status.office_excluded_date ? fact("Excluded from national office since",status.office_excluded_date,"exclusion") : ""}</dl>${status.party?.note ? `<p class="gov-ui-note">${esc(status.party.note)}</p>` : ""}${status.national_office?.note ? `<p class="gov-ui-note">${esc(status.national_office.note)}</p>` : ""}${status.note ? `<p class="gov-ui-note">${esc(status.note)}</p>` : ""}</aside>`;
  }
  function leadershipPerson(entry, party, context, key, allowCampaign = false) {
    ...
    const status = allowCampaign ? entry.campaign_succession : null;
  ...
  function changeCards(changes) {
    if (!rows(changes).length) return "";
    return `<section class="gov-ui-changes" aria-label="Decision effects before and after"><h3>What this decision changes</h3><dl class="gov-ui-change-list">${changes.map((change,index) => `<div class="gov-ui-change" data-gov-change="${index}"><dt>${esc(change.label || change.key)}</dt><dd><div class="gov-ui-change-values"><div><span>Before</span><strong data-change-before>${esc(valueText(change.before))}</strong></div><div><span>After</span><strong data-change-after>${esc(valueText(change.after))}</strong></div></div>${change.detail ? `<p>${esc(change.detail)}</p>` : ""}</dd></div>`).join("")}</dl></section>`;
  }
