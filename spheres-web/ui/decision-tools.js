...
(function(root,factory){const api=factory();if(typeof module==="object"&&module.exports)module.exports=api;else root.DecisionTools=api;})(typeof globalThis!=="undefined"?globalThis:this,function(){
  ...
  function research(nodes,domain,query,filter="available"){
    const q=String(query||"").toLocaleLowerCase();
    return nodes.filter(n=>(domain==="all"||n.domain===domain)&&(!q||`${n.name} ${(n.effects||[]).join(" ")}`.toLocaleLowerCase().includes(q))&&(filter==="all"||filter==="known"?filter==="all"||n.state==="known":n.state==="open"))
...
if(typeof window!=="undefined"){
  ...
  function toolsDialog(title,body){
    ...
    if(!box){box=document.createElement("dialog");box.id="decisionDialog";box.className="decision-dialog";document.body.append(box);box.addEventListener("close",()=>DTOOLS.returnFocus?.focus({preventScroll:true}));}
  ...
  window.openBuildInfo=async function(){const box=toolsDialog("About SPHERES","<p>Reading build information…</p>");try{const b=await api("/api/build");box.querySelector(".decision-body").innerHTML=`<p><strong>SPHERES ${toolsEsc(b.version)}</strong> · ${toolsEsc(b.revision)}</p><p>${toolsEsc(b.distribution)}</p><p>Save directory: <code>${toolsEsc(b.save_directory)}</code></p><p>${toolsEsc(b.campaign_format)}</p><button onclick="openPlaytestFeedback()">Record playtest feedback</button>`;}catch(e){box.querySelector(".decision-body").textContent=e.message;}};
