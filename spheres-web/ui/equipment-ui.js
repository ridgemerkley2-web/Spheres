...
function equipmentServiceHtml(service,effects=false){
  ...
  const actions=effects?"":(Array.isArray(service.actions)?service.actions:[]).map((action,index)=>action?.navigate&&!action.command?`<button type="button" data-equipment-service-action="service.actions.${index}" ${equipmentCurrent()&&action.enabled!==false&&action.available!==false?"":"disabled"}${action.reason?` title="${equipmentText(action.reason)}"`:""}>${equipmentText(action.label||"Review fleet")}</button>`:"").join("");
  return `<section class="eq-panel eq-service${effects?" eq-service-effects":""}" aria-label="${equipmentText(title)}"><div class="eq-service-heading"><div><p class="eq-eyebrow">${effects?"Before you confirm":"Fleet overview"}</p><h3>${equipmentText(title)}</h3></div>${service.status?`<span class="eq-status">${equipmentText(service.status)}</span>`:""}</div>${service.detail?`<p>${equipmentText(service.detail)}</p>`:""}${equipmentMetrics(service.metrics)}${roles.length?`<div class="eq-service-roles"><h4>National fleet composition</h4><dl>${roles.map(row=>`<div><dt>${equipmentText(row.label)}</dt><dd>${equipmentText(typeof row.value==="string"?row.value:equipmentNumber(row.value))}${row.detail?`<small>${equipmentText(row.detail)}</small>`:""}</dd></div>`).join("")}</dl></div>`:""}${equipmentBlockers(service.warnings)}${actions?`<div class="eq-actions eq-service-actions">${actions}</div>`:""}</section>`;
}
...
function equipmentContentHtml(){
  ...
  else if(EQUIP.tab==="production")body=equipmentSupplyHtml(data.supply,"supply")+equipmentCollectionHtml("production","Build the model you approved","Follow the exact revision through tooling, manufacture and delivery. Funded work and fielded vehicles are separate stages.","No equipment production is listed. A certified design can be assigned to an eligible production site.");
  else body=equipmentServiceHtml(data.service)+equipmentModernizationHtml()+equipmentCollectionHtml("lots","Equipment in service","Inspect delivered models and compatible refits. A refit withdraws real equipment until the work is complete.","No designer-built equipment has entered service. Your inherited equipment remains in the existing arsenal.");
