...
const EQUIPMENT_TABS=[["research","Research"],["designer","Designer"],["library","Library"],["development","Development"],["production","Production"],["ammunition","Ammunition"],["service","In service"]];
...
function equipmentSupplyNumber(value){return Number.isFinite(value)?value.toLocaleString("en-US",value!==0&&Math.abs(value)<.001?{maximumSignificantDigits:3}:{maximumFractionDigits:3}):"—";}
function equipmentSupplyCurrent(scope,data,state){return scope==="intent"?equipmentReviewQuoteCurrent()&&data===EQUIP.review.quote&&state===EQUIP.review.quoteState:scope==="preview"?equipmentPreviewCurrent(data,state):equipmentCurrent(data,state);}
function equipmentSupplyNavigate(path,scope,data,state){
  if(!equipmentSupplyCurrent(scope,data,state))return false;const action=equipmentActionAt(path,scope);
  if(!action?.navigate||action.command||action.enabled===false||action.available===false)return false;
  equipmentNavigate(equipmentCopy(action.navigate));return true;
}
function equipmentSupplyHtml(supply,path,scope="data",compact=false){
  if(!supply||typeof supply!=="object")return "";
  const resources=Array.isArray(supply.resources)?supply.resources:[],data=scope==="intent"?EQUIP.review?.quote:scope==="preview"?EQUIP.preview:EQUIP.data,state=scope==="intent"?EQUIP.review?.quoteState:scope==="preview"?EQUIP.previewState:EQUIP.state,current=equipmentSupplyCurrent(scope,data,state);
  const actions=(Array.isArray(supply.actions)?supply.actions:[]).map((action,index)=>action?.navigate&&!action.command?`<button type="button" data-equipment-supply-action="${equipmentText(`${path}.actions.${index}`)}" data-equipment-scope="${equipmentText(scope)}" ${current&&action.enabled!==false&&action.available!==false?"":"disabled"}${action.reason?` title="${equipmentText(action.reason)}"`:""}>${equipmentText(action.label||"Review resources")}</button>`:"").join("");
  const heading=`<span>${equipmentText(supply.title||"Production supply")}</span>${supply.status?`<span class="eq-status">${equipmentText(supply.status)}</span>`:""}`;
  const table=resources.length?`<div class="eq-supply-scroll" role="region" aria-label="Production material requirements" tabindex="0"><table class="eq-supply-table"><caption>National stock is shared across programmes. This reading does not reserve materials.</caption><thead><tr><th scope="col">Material · unit</th><th scope="col">Remaining work</th><th scope="col">Next work</th><th scope="col">${supply.stage==="national"?"National stock":"Stock for this review"}</th><th scope="col">Next work gap</th></tr></thead><tbody>${resources.map(row=>`<tr data-equipment-supply-material="${equipmentText(row.commodity)}"><th scope="row"><span${row.detail?` title="${equipmentText(row.detail)}"`:""}>${equipmentText(row.name||row.commodity)}</span>${row.unit?`<small>${equipmentText(row.unit)}</small>`:""}</th><td>${equipmentText(equipmentSupplyNumber(row.remaining))}</td><td>${equipmentText(equipmentSupplyNumber(row.planned_day))}</td><td>${equipmentText(equipmentSupplyNumber(row.stock))}</td><td${Number.isFinite(row.shortfall)&&row.shortfall>0?' class="eq-supply-shortfall"':""}>${equipmentText(equipmentSupplyNumber(row.shortfall))}</td></tr>`).join("")}</tbody></table></div><p class="eq-field-note">Remaining work is the full unfinished bill, including future fabrication. Next work and its gap cover the planned work for this reading. Stock is shared and is not reserved.</p>`:"";
  const body=`${supply.detail?`<p>${equipmentText(supply.detail)}</p>`:""}${equipmentMetrics(supply.metrics)}${table}${equipmentBlockers(supply.warnings)}${actions?`<div class="eq-actions eq-supply-actions">${actions}</div>`:""}`;
  return compact?`<details class="eq-details eq-supply eq-supply-compact" data-equipment-detail="${equipmentText(`supply:${path}`)}" ${EQUIP.details.has(`supply:${path}`)?"open":""}><summary>${heading}</summary><div class="eq-supply-body">${body}</div></details>`:`<section class="eq-panel eq-supply" aria-label="${equipmentText(supply.title||"Production supply")}"><div class="eq-supply-heading"><h3>${equipmentText(supply.title||"Production supply")}</h3>${supply.status?`<span class="eq-status">${equipmentText(supply.status)}</span>`:""}</div>${body}</section>`;
}
...
function equipmentFamilyLabel(platform){return platform?.family_name||platform?.family_label||(equipmentFamily(platform)==="tanks"?"Tanks":platform?.name||"Equipment");}
...
function equipmentSpecificationGroups(slots){
  const groups=[['drive','Engine & running gear',['mobility','transmission','tracks','wheels','suspension']],['weapon','Turret & armament',['turret','armament','ammunition','artillery_loader']],['armor','Protection & troop compartment',['protection','active_protection','troop_compartment']],['electronics','Observation & control',['sensors','fire_control','communications','recon_package','radar']],['airframe','Airframe & endurance',['air_engine','air_wing','air_fuel']],['airmission','External stores & payload',['air_hardpoints','air_payload']],['airsystems','Mission systems & protection',['air_radar','air_avionics','air_countermeasures']]];
...
function equipmentDesignerHtml(){
  ...
  return `<div class="eq-section-heading eq-designer-heading"><div><h2>Make a model that fits your force</h2><p>Choose a vehicle family and starting configuration, then inspect and customize every system.</p></div></div>${equipmentFamilyHtml()}${equipmentPresetHtml()}
    ...
    <label><span>Equipment platform</span><select id="equipmentPlatform" data-equipment-focus="platform" ${equipmentPending()?"disabled":""}>${equipmentRows("platforms").filter(row=>equipmentFamily(row)===equipmentFamily(platform)).map(row=>`<option value="${equipmentText(row.id)}" ${row.id===EQUIP.draft.platform?"selected":""}>${equipmentText(row.name)}</option>`).join("")}</select></label></div>
    ...
    <section class="eq-visual" aria-label="Interactive ground and aviation model"><div class="eq-visual-heading"><div><p class="eq-eyebrow">3D model · Live configuration</p><h2>${equipmentText(EQUIP.draft.name||"Unnamed model")}</h2></div><span class="eq-status">${equipmentText(platform?.name||"Equipment")}</span></div>${equipmentModelHtml()}<div class="eq-labels">${slots.map(slot=>{const part=equipmentComponent(EQUIP.draft.components?.[slot.id]);return part?`<button type="button" data-equipment-inspect="${equipmentText(slot.id)}">${equipmentText(part.name)}</button>`:"";}).join("")}</div><p class="eq-visual-note">Exterior specifications change the model’s visible equipment. Internal specifications remain in game ratings and export metadata. Paint changes appearance only. Review the game ratings and costs before placing an order.</p></section>${equipmentPreviewHtml()}</div>${equipmentComparisonHtml()}`;
}
function equipmentRecordHtml(row,index,key){
  return `<article class="eq-card" data-equipment-record="${equipmentText(row.id)}" tabindex="-1"><div class="eq-card-header"><h3>${equipmentText(row.name||row.id)}</h3>${row.status?`<span class="eq-status">${equipmentText(row.status)}</span>`:""}</div>${row.detail?`<p>${equipmentText(row.detail)}</p>`:""}${equipmentProgress(row.progress)}${equipmentMetrics(row.metrics)}${equipmentCosts(row.costs)}
    ...
    ${key==="production"?equipmentSupplyHtml(row.supply,`${key}.${index}.supply`,"data",true):""}<div class="eq-actions">${key==="designs"&&row.spec?`<button type="button" data-equipment-edit="${equipmentText(row.id)}" ${equipmentPending()?"disabled":""}>Use as a starting point</button>`:""}${equipmentActions(row.actions,`${key}.${index}.actions`)}</div></article>`;
}
...
function equipmentServiceNavigate(path,data,state){
  if(!equipmentCurrent(data,state))return false;const action=equipmentActionAt(path,"data");
  if(!action?.navigate||action.command||action.enabled===false||action.available===false)return false;
  equipmentNavigate(equipmentCopy(action.navigate));return true;
}
function equipmentServiceHtml(service,effects=false,actionPath=null){
  if(!service||typeof service!=="object")return "";
  const title=service.title||(effects?"Effects on the fleet":"Fleet in service"),roles=Array.isArray(service.roles)?service.roles:[];
  const actions=effects?"":actionPath?equipmentActions(service.actions,`${actionPath}.actions`):(Array.isArray(service.actions)?service.actions:[]).map((action,index)=>action?.navigate&&!action.command?`<button type="button" data-equipment-service-action="service.actions.${index}" ${equipmentCurrent()&&action.enabled!==false&&action.available!==false?"":"disabled"}${action.reason?` title="${equipmentText(action.reason)}"`:""}>${equipmentText(action.label||"Review fleet")}</button>`:"").join("");
  return `<section class="eq-panel eq-service${effects?" eq-service-effects":""}" aria-label="${equipmentText(title)}"><div class="eq-service-heading"><div><p class="eq-eyebrow">${effects?"Before you confirm":actionPath?"Fleet planning":"Fleet overview"}</p><h3>${equipmentText(title)}</h3></div>${service.status?`<span class="eq-status">${equipmentText(service.status)}</span>`:""}</div>${service.detail?`<p>${equipmentText(service.detail)}</p>`:""}${equipmentMetrics(service.metrics)}${roles.length?`<div class="eq-service-roles"><h4>${equipmentText(service.roles_title||"National fleet composition")}</h4><dl>${roles.map(row=>`<div><dt>${equipmentText(row.label)}</dt><dd>${equipmentText(typeof row.value==="string"?row.value:equipmentNumber(row.value))}${row.detail?`<small>${equipmentText(row.detail)}</small>`:""}</dd></div>`).join("")}</dl></div>`:""}${equipmentBlockers(service.warnings)}${actions?`<div class="eq-actions eq-service-actions">${actions}</div>`:""}</section>`;
}
function equipmentTargetsHtml(){
  const rows=equipmentRows("targets");
  return `<section class="eq-targets" aria-labelledby="equipmentTargetsTitle"><div class="eq-section-heading"><div><p class="eq-eyebrow">Replacement planning</p><h2 id="equipmentTargetsTitle">Fleet quantity targets</h2><p>Choose how many vehicles of each model you want. Incoming deliveries and conversions count before another order is suggested.</p></div><button type="button" data-equipment-tab="library">Set targets in the library</button></div>${rows.length?`<div class="eq-card-grid">${rows.map((row,index)=>equipmentRecordHtml(row,index,"targets")).join("")}</div>`:'<div class="eq-empty"><p>No quantity targets saved. Open a certified model in the library and choose Set fleet target. A target creates no spending or vehicles.</p></div>'}</section>`;
}
function equipmentSupplyAutomationHtml(){
  const automation=EQUIP.data?.supply_automation;
  if(!automation||typeof automation!=="object")return "";
  const rows=Array.isArray(automation.reviews)?automation.reviews:[],recent=rows.slice(0,3),historyKey="supply-automation-history";
  const cards=(items,offset)=>`<div class="eq-card-grid">${items.map((row,index)=>equipmentRecordHtml(row,index+offset,"supply_automation.reviews")).join("")}</div>`;
  return `${equipmentServiceHtml(automation.overview,false,"supply_automation.overview")}<section aria-labelledby="equipmentSupplyReviewsTitle"><div class="eq-section-heading"><div><h2 id="equipmentSupplyReviewsTitle">Recent purchasing reviews</h2><p>Recorded decisions and spending from the material purchasing policy.</p></div></div>${rows.length?cards(recent,0):'<div class="eq-empty"><p>No purchasing reviews recorded yet. Future policy reviews and their outcomes will appear here.</p></div>'}${rows.length>3?`<details class="eq-details" data-equipment-detail="${historyKey}" ${EQUIP.details.has(historyKey)?"open":""}><summary data-equipment-focus="${historyKey}">Show ${rows.length-3} earlier purchasing ${rows.length-3===1?"review":"reviews"}</summary>${cards(rows.slice(3),3)}</details>`:""}</section>`;
}
function equipmentAmmunitionHtml(){
  const ammunition=EQUIP.data?.ammunition,query=EQUIP.query.trim().toLocaleLowerCase("en-US");
  if(!ammunition||typeof ammunition!=="object")return '<div class="eq-empty"><h2>Ammunition</h2><p>Ammunition information is not available in this reading. Refresh the equipment bureau to check this campaign.</p></div>';
  const groups=[["reserves","Reserve plans","Set a stock target for each ammunition family and choose whether to replenish it automatically.","No reserve plans saved. Choose an ammunition family below and set a reserve target. Automatic replenishment is optional.","No reserve plans saved"],["families","Ammunition families","Review each family's stock, compatible weapons and production requirements.","No ammunition families are listed in this campaign.","No families listed"],["orders","Ammunition production","Follow funded work from its material requirements to completed ammunition.","No ammunition batches are scheduled. Review a family above to choose a production site and funding.","No batches scheduled"]].map(([key,title,description,empty,emptyTitle])=>{
    const rows=Array.isArray(ammunition[key])?ammunition[key]:[],filtered=rows.map((row,index)=>({row,index})).filter(({row})=>!query||[row.name,row.id,row.status,row.detail,...(Array.isArray(row.metrics)?row.metrics.flatMap(metric=>[metric.label,metric.value]):[])].join(" ").toLocaleLowerCase("en-US").includes(query));
    return {key,title,description,empty,emptyTitle,rows,filtered};
  });
  const count=groups.reduce((sum,group)=>sum+group.rows.length,0),shown=groups.reduce((sum,group)=>sum+group.filtered.length,0);
  const search=count?`<div class="eq-toolbar"><label><span>Find ammunition, a reserve plan or a production batch</span><input id="equipmentSearch" data-equipment-focus="search" value="${equipmentText(EQUIP.query)}" placeholder="Search by family, province or status"></label><p role="status">${shown} of ${count} shown</p>${query?'<button type="button" data-equipment-clear>Clear search</button>':""}</div>`:"";
  return `${equipmentServiceHtml(ammunition.overview,false,"ammunition.overview")}${search}${groups.map(group=>`<section aria-labelledby="equipmentAmmo${group.key}Title"><div class="eq-section-heading"><div><h2 id="equipmentAmmo${group.key}Title">${group.title}</h2><p>${group.description}</p></div></div>${group.filtered.length?`<div class="eq-card-grid">${group.filtered.map(({row,index})=>equipmentRecordHtml(row,index,`ammunition.${group.key}`)).join("")}</div>`:`<div class="eq-empty"><h3>${query&&group.rows.length?"No matches":group.emptyTitle}</h3><p>${equipmentText(query&&group.rows.length?"Try another family, province or status, or clear your search.":group.empty)}</p></div>`}</section>`).join("")}`;
}
...
function equipmentContentHtml(){
  ...
  const header=`<header class="eq-header"><div><p class="eq-eyebrow">Military industry · Ground &amp; aviation</p><h1 id="equipmentTitle" tabindex="-1" data-equipment-focus="title">From concept to service</h1><p>${equipmentText(context)}</p></div><div class="eq-header-actions"><button type="button" data-equipment-refresh data-equipment-focus="refresh" ${EQUIP.loading||equipmentPending()?"disabled":""}>Refresh reading</button><button type="button" data-equipment-close aria-label="Close equipment designer">Back to map</button></div></header>
    <div class="eq-process" aria-label="Equipment lifecycle"><span>Research</span> → <span>Design</span> → <span>Development</span> → <span>Production</span> → <span>Ammunition</span> → <span>Service & refit</span></div>
  ...
  else if(EQUIP.tab==="production")body=equipmentSupplyHtml(data.supply,"supply")+equipmentSupplyAutomationHtml()+equipmentServiceHtml(data.replenishment,false,"replenishment")+equipmentCollectionHtml("production","Build the model you approved","Follow the exact revision through tooling, manufacture and delivery. Funded work and fielded vehicles are separate stages.","No equipment production is listed. A certified design can be assigned to an eligible production site.");
  else if(EQUIP.tab==="ammunition")body=equipmentAmmunitionHtml();
  else body=equipmentServiceHtml(data.service)+equipmentServiceHtml(data.aviation,false,"aviation")+equipmentServiceHtml(data.maintenance,false,"maintenance")+equipmentTargetsHtml()+equipmentModernizationHtml()+equipmentCollectionHtml("lots","Equipment in service","Inspect delivered models and compatible refits. A refit withdraws real equipment until the work is complete.","No designer-built equipment has entered service. Your inherited equipment remains in the existing arsenal.");
...
function equipmentRememberView(){
  ...
  if(key&&!EQUIP.focus)EQUIP.focus={key,start:active.selectionStart,end:active.selectionEnd};
}
...
async function equipmentConfirm(index=null){
  ...
  try{const result=await equipmentCommand(equipmentCopy(action.command));if(session!==S?.session_id||!equipmentActive())return false;
    ...
    EQUIP.message=result?.message||(action.command.kind==="equipment_retire"?"Vehicles retired. Your remaining fleet is shown below; funding allocations and existing orders are unchanged.":"Order received. The updated programme will show its progress and spending.");EQUIP.review=null;return true;
...
function equipmentBind(fetchIfNeeded=true){
  if(!equipmentActive())return;const root=document.querySelector("#equipmentRoot");if(!root)return;const data=EQUIP.data,state=EQUIP.state,preview=EQUIP.preview,previewState=EQUIP.previewState,quote=EQUIP.review?.quote,quoteState=EQUIP.review?.quoteState;
  ...
  root.querySelectorAll("[data-equipment-supply-action]").forEach(button=>{const scope=button.dataset.equipmentScope;button.onclick=()=>equipmentSupplyNavigate(button.dataset.equipmentSupplyAction,scope,scope==="intent"?quote:scope==="preview"?preview:data,scope==="intent"?quoteState:scope==="preview"?previewState:state);});
  root.querySelectorAll("[data-equipment-service-action]").forEach(button=>button.onclick=()=>equipmentServiceNavigate(button.dataset.equipmentServiceAction,data,state));
  ...
  const review=EQUIP.review;
  root.querySelectorAll("[data-equipment-intent]").forEach(button=>button.onclick=()=>{if(review!==EQUIP.review||quote!==EQUIP.review?.quote||quoteState!==S||!equipmentReviewQuoteCurrent())return false;return equipmentConfirm(Number(button.dataset.equipmentIntent));});
