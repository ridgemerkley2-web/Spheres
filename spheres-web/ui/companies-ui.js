...
function companiesContentHtml(){
  ...
  const hero=`<header class="company-hero"><div><p class="company-kicker">${companyText(data?.name||"Your country")} · Company directory</p><h1>Your companies.<br><em>Your next move.</em></h1><p>Commission equipment. Buy finished stock.<br>Choose specialists for funded public work.</p></div><div class="company-market"><strong>${data?rows.length:"—"}<span>${rows.length===1?"company":"companies"}</span></strong><div><span>${suppliers} manufacturer${suppliers===1?"":"s"}</span><span>${rows.length-suppliers} specialist${rows.length-suppliers===1?"":"s"}</span></div></div></header>`;
