...
struct Country {
    ...
    end_in_flight_advanced_components: f64,
...
fn record_open_commerce(row: &mut Country, ledger: &commerce::Commerce, id: NationId) {
    ...
    row.end_in_flight_advanced_components = 0.0;
    ...
    for cargo in ledger.cargo.iter().filter(|c| c.buyer == id) {
        match cargo.good {
            ...
            commerce::Good::AdvancedComponents => {
                row.end_in_flight_advanced_components += cargo.quantity
            }
        }
