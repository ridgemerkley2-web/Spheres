...
impl CampaignState {
    pub fn is_empty(&self) -> bool {
        !self.initialized
            ...
            && self.ai_review.is_empty()
            && self.alerts.is_empty()
            && self.last_prepared.is_none()
    }
}
...
/// Inspect persisted state without performing enrollment, reconciliation or a tick.
/// Ended wars, deceased participants and past-due movements are retained until
/// prepare/resolve owns their transition. Current military strength is not an
/// equality constraint here: commands can change it between operational ticks.
pub fn validate(w: &WorldState) -> Result<(), String> {
    let state = &w.campaign;
    if let Some(ids) = &state.migration_conflicts {
        if ids.contains(&0) { return Err("campaign migration contains conflict zero".into()); }
    }
    if let Some(day) = state.last_prepared { validate_saved_day(w, day, "campaign preparation")?; }
    for (key, order) in &state.orders {
        validate_identity(w, order.conflict, order.nation, "campaign order")?;
        if key != &order_key(order.conflict, order.nation) || order.reserve_bp > 10_000 {
            return Err(format!("invalid campaign order {key}"));
        }
        if let Some(d) = &order.target { validate_district(w, d, "campaign target")?; }
    }
    for (key, sector) in &state.sectors {
        validate_identity(w, sector.conflict, sector.nation, "campaign sector")?;
        validate_district(w, &sector.district, "campaign sector")?;
        if key != &sector_key(sector.conflict, sector.nation, &sector.district) {
            return Err(format!("campaign sector identity mismatch {key}"));
        }
        validate_amount(sector.strength, "campaign sector strength")?;
        validate_unit(sector.cohesion, "campaign sector cohesion")?;
        validate_unit(sector.preparation, "campaign sector preparation")?;
    }
    for (&nation, reserve) in &state.reserves {
        validate_nation(w, nation, "campaign reserve")?;
        validate_amount(reserve.strength, "campaign reserve strength")?;
        validate_unit(reserve.cohesion, "campaign reserve cohesion")?;
    }
    // A transfer can use district adjacency or the freight network. Only static
    // topology is checked; lost access is a valid saved hold, not corrupt data.
    let routes = (!state.transfers.iter().all(|t| t.route.is_empty()))
        .then(|| crate::campaign_supply::SavedRoutes::new(w));
    for transfer in &state.transfers {
        validate_nation(w, transfer.nation, "campaign transfer")?;
        validate_amount(transfer.strength, "campaign transfer strength")?;
        validate_unit(transfer.cohesion, "campaign transfer cohesion")?;
        validate_saved_day(w, transfer.departed_day, "campaign transfer departure")?;
        if transfer.arrives_day < transfer.departed_day {
            return Err("campaign transfer arrives before departure".into());
        }
        match (transfer.conflict, transfer.district.as_deref()) {
            (Some(cid), Some(d)) => {
                validate_identity(w, cid, transfer.nation, "campaign transfer")?;
                validate_district(w, d, "campaign transfer destination")?;
                if transfer.route.last().map(String::as_str) != Some(d) {
                    return Err("campaign transfer destination differs from booked route".into());
                }
            }
            (None, None) => {}
            _ => return Err("campaign transfer has partial destination identity".into()),
        }
        if let Some(routes) = &routes { routes.transfer(&transfer.route)?; }
    }
    for (key, estimate) in &state.observations {
        validate_key(w, key, false)?;
        validate_amount(estimate.low, "campaign observed lower strength")?;
        validate_amount(estimate.high, "campaign observed upper strength")?;
        validate_unit(estimate.confidence, "campaign observation confidence")?;
        validate_saved_day(w, estimate.observed_day, "campaign observation")?;
        if estimate.low > estimate.high || estimate.observed_label != date_label(estimate.observed_day) {
            return Err(format!("invalid campaign observation range/date {key}"));
        }
    }
    for (key, report) in &state.reports {
        validate_key(w, key, false)?;
        validate_saved_day(w, report.day, "campaign report")?;
        for value in [report.advanced, report.retreated, report.losses] {
            validate_amount(value, "campaign report movement/loss")?;
        }
        validate_unit(report.readiness, "campaign report readiness")?;
        validate_unit(report.supply, "campaign report supply")?;
        if report.day_label != date_label(report.day) {
            return Err(format!("campaign report date label mismatch {key}"));
        }
    }
    for (label, book) in [("campaign AI review", &state.ai_review), ("campaign alert", &state.alerts)] {
        for (key, &day) in book {
            validate_key(w, key, false)?;
            validate_saved_day(w, day, label)?;
        }
    }
    // Missing counters in older master saves are promoted by the explicit save
    // adapter. Validation never rewrites high-water IDs or infers new armies.
    Ok(())
}

pub(crate) fn validate_nation(w: &WorldState, nation: NationId, label: &str) -> Result<(), String> {
    if w.nation_opt(nation).is_none() { Err(format!("{label} refers to unknown nation")) } else { Ok(()) }
}
pub(crate) fn validate_identity(w: &WorldState, cid: u32, nation: NationId, label: &str) -> Result<(), String> {
    if cid == 0 { return Err(format!("{label} has conflict zero")); }
    validate_nation(w, nation, label)
}
pub(crate) fn validate_district(w: &WorldState, district: &str, label: &str) -> Result<(), String> {
    if !w.districts.contains_key(district) { Err(format!("{label} refers to unknown district {district}")) } else { Ok(()) }
}
pub(crate) fn validate_saved_day(w: &WorldState, day: i32, label: &str) -> Result<(), String> {
    if day < 0 || day > clock::absolute_day(w) { Err(format!("{label} has an impossible receipt date")) } else { Ok(()) }
}
pub(crate) fn validate_amount(value: f64, label: &str) -> Result<(), String> {
    if !value.is_finite() || value < 0.0 { Err(format!("{label} must be finite and nonnegative")) } else { Ok(()) }
}
pub(crate) fn validate_unit(value: f64, label: &str) -> Result<(), String> {
    validate_amount(value, label)?;
    if value > 1.0 + EPS { Err(format!("{label} exceeds one")) } else { Ok(()) }
}
pub(crate) fn validate_key(w: &WorldState, key: &str, debug_nation: bool) -> Result<(u32, NationId), String> {
    let Some((cid, name)) = key.split_once(':') else { return Err(format!("invalid campaign identity {key}")); };
    let id = cid.parse::<u32>().ok().filter(|id| *id > 0 && id.to_string() == cid)
        .ok_or_else(|| format!("invalid campaign conflict identity {key}"))?;
    let nation = w.nations.iter().map(|n| n.id).find(|n| {
...
mod saved_validation_tests {
    ...
    fn retained() -> WorldState {
        ...
        let key = order_key(77, NationId::Germany);
        w.campaign = CampaignState {
            initialized: true, conflict_id_high_water: 900, migration_conflicts: Some(BTreeSet::from([77])),
            orders: BTreeMap::from([(key.clone(), OperationOrder { conflict: 77, nation: NationId::Germany,
                target: Some("DE-BB".into()), approach: Approach::Advance, reserve_bp: 1500,
                air: AirMission::None, naval: NavalMission::None })]),
            sectors: BTreeMap::from([(sector_key(77, NationId::Germany, "DE-BB"), Sector {
                conflict: 77, nation: NationId::Germany, district: "DE-BB".into(), strength: 4.0,
                cohesion: 0.8, preparation: 0.5, isolated_days: 3 })]),
            reserves: BTreeMap::from([(NationId::Germany, Reserve { strength: 12.0, cohesion: 0.6 })]),
            transfers: vec![Transfer { nation: NationId::Germany, conflict: None, district: None,
                strength: 2.0, cohesion: 0.7, route: vec![], arrives_day: 14, departed_day: 0 }],
            observations: BTreeMap::from([(key.clone(), EnemyEstimate { low: 10.0, high: 30.0,
                confidence: 0.5, observed_day: 0, observed_label: date_label(0) })]),
            reports: BTreeMap::from([(key.clone(), BattleReport { day: 0, day_label: date_label(0),
                advanced: 0.1, retreated: 0.0, losses: 1.0, readiness: 0.8, summary: "Saved battle".into(), supply: 0.4 })]),
            ai_review: BTreeMap::from([(key.clone(), 0)]), alerts: BTreeMap::from([(key, 0)]), last_prepared: Some(0),
        };
        w
    }
    #[test]
    fn campaign_validation_preserves_ended_wars_dead_participants_and_overdue_returns() {
        let w = retained();
        assert!(w.conflict(77).is_none());
        let before = serde_json::to_value(&w.campaign).unwrap();
        assert_eq!(validate(&w), Ok(()));
        assert!(before == serde_json::to_value(&w.campaign).unwrap(), "validation must not reconcile a saved army");
        assert_eq!(w.campaign.conflict_id_high_water, 900);
        assert_eq!(w.campaign.transfers[0].arrives_day, 14);
    }
    #[test]
    fn campaign_validation_rejects_corrupt_identity_dates_and_quantities() {
        let w = retained();
        let mut corrupt = w.clone();
        let order = corrupt.campaign.orders.pop_first().unwrap().1;
        corrupt.campaign.orders.insert("77:unknown".into(), order);
        assert!(validate(&corrupt).is_err());
        let mut corrupt = w.clone(); corrupt.campaign.transfers[0].departed_day = 25;
        assert!(validate(&corrupt).is_err());
        let mut corrupt = w.clone(); corrupt.campaign.transfers[0].arrives_day = -1;
        assert!(validate(&corrupt).is_err());
        let mut corrupt = w.clone(); corrupt.campaign.reserves.get_mut(&NationId::Germany).unwrap().strength = f64::NAN;
        assert!(validate(&corrupt).is_err());
        let mut corrupt = w.clone(); corrupt.campaign.sectors.values_mut().next().unwrap().cohesion = -0.1;
        assert!(validate(&corrupt).is_err());
        let mut corrupt = w.clone(); corrupt.campaign.observations.values_mut().next().unwrap().low = 31.0;
        assert!(validate(&corrupt).is_err());
        let mut corrupt = w.clone(); corrupt.campaign.reports.values_mut().next().unwrap().day_label = "1991-01-01".into();
        assert!(validate(&corrupt).is_err());
        let mut corrupt = w; corrupt.nations.retain(|n| n.id != NationId::Germany);
        assert!(validate(&corrupt).is_err());
    }
    #[test]
    fn campaign_is_empty_observes_every_persisted_field() {
        let sample = retained().campaign;
        assert!(CampaignState::default().is_empty());
        for field in 0..13 {
            let mut state = CampaignState::default();
            match field {
                0 => state.initialized = true,
                1 => state.conflict_id_high_water = 900,
                2 => state.migration_conflicts = Some(BTreeSet::new()),
                3 => state.orders = sample.orders.clone(),
                4 => state.sectors = sample.sectors.clone(),
                5 => state.reserves = sample.reserves.clone(),
                6 => state.transfers = sample.transfers.clone(),
                7 => state.observations = sample.observations.clone(),
                8 => state.reports = sample.reports.clone(),
                9 => state.ai_review = sample.ai_review.clone(),
                10 => state.alerts = sample.alerts.clone(),
                11 => state.last_prepared = Some(0),
                // Missing and explicitly empty enrollment sets are distinct.
                12 => state.migration_conflicts = sample.migration_conflicts.clone(),
                _ => unreachable!(),
            }
            assert!(!state.is_empty(), "campaign field {field} would be silently omitted");
        }
    }
    #[test]
    fn campaign_decode_rejects_unknown_nested_force_property() {
        let state = retained().campaign;
        let mut shape = serde_json::to_value(&state).unwrap();
        assert!(serde_json::from_value::<CampaignState>(shape.clone()).is_ok());
        shape["sectors"].as_object_mut().unwrap().values_mut().next().unwrap()
            .as_object_mut().unwrap().insert("unrecorded_force".into(), serde_json::json!(2.0));
        assert!(serde_json::from_value::<CampaignState>(shape).is_err());
    }
