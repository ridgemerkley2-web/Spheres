...
pub fn tick(w: &mut WorldState) {
    ...
    for id in &ids {
        // An explicit plan or open books has its own fiscal authority. An
        // enrolled opponent uses priced commands through economic_ai; legacy
        // aggregate writes must not bypass either owner's plan or price.
        if Some(*id) == w.player || w.nation(*id).on_the_books()
            || w.nation(*id).annual_budget.is_some() || crate::programs::enrolled(w, *id)
        ...
        let n = w.nation_mut(*id);
        if n.debt_gdp > 0.85 {
            // Bounds limit the direction of this policy; they are not targets
            // that authorize tax cuts or spending increases during austerity.
            n.tax_rate = (n.tax_rate + 0.002 * dt).min(0.55_f64.max(n.tax_rate));
            // A funded Army's operating appropriation is a shared priority
            // with government AI. This only limits a cut; it never raises
            // spending implicitly. The .01 legacy floor also only limits cuts.
            let floor = army_floor.map_or(0.01, |v| v.max(0.01)).min(n.mil_spend_gdp);
            n.mil_spend_gdp = (n.mil_spend_gdp * fiscal_cut).max(floor);
            n.state_invest_gdp = (n.state_invest_gdp * fiscal_cut)
                .max(0.02_f64.min(n.state_invest_gdp));
        } else if n.debt_gdp < 0.3 {
            if n.tax_rate > 0.30 {
                n.tax_rate = (n.tax_rate - 0.001 * dt).max(0.30);
            }
            ...
            if let Some(base) = n.state_invest_1990 {
                // An allocation already above its reference is not a cut to restore.
                n.state_invest_gdp = (n.state_invest_gdp * fiscal_restore)
                    .min(base.max(n.state_invest_gdp));
            }
        }
    }
...
fn dissolve_ussr(w: &mut WorldState) {
    ...
    if w.rules.historical_party_leadership { crate::government::ensure_all(w); }
}
