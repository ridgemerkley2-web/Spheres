...
fn s16_legacy_raid_losses_preserve_parked_fighters_and_their_loss_carry() {
    for missiles in [0, 100] {
        ...
        w.nation_mut(USA)
            ...
            .held
            .iter_mut()
            .find(|h| h.design_id.as_deref() == Some("parked-fighter"))
            .unwrap()
        ...
        let ammo = w
            ...
            .as_ref()
            .unwrap();
        let missile_stock = ammo
            .stocks
            .get("air_missile_short_range")
            .copied()
            .unwrap_or(0.0);
        let missile_consumed = ammo
            .consumed
            .get("air_missile_short_range")
            .copied()
            .unwrap_or(0.0);
        ...
        let fighter_stores = planned
            .families
            .iter()
            .find(|r| r.family == "air_missile_short_range")
            .unwrap();
        assert_eq!(fighter_stores.required, 0.0);
        assert_eq!(fighter_stores.used, 0.0);
        let bomber_loss =
            1000.0 * snapshot.rows[&(400, USA)].deployed / w.nation(USA).mil_strength * 0.4 * 0.5;
        assert!(
            bomber_loss >= 1.0,
            "This must exercise a real whole-aircraft writeoff"
        );
        snapshot.record_loss(400, USA, 0.4);
        snapshot.settle(&mut w);
        let holdings = &w.nation(USA).arsenal.held;
        ...
        assert_eq!(
            ...
            serde_json::to_value(&fighter).unwrap()
        );
        let ammo = w
            .nation(USA)
            .equipment
