...
fn company_work_capacity(w: &WorldState, job: &EquipmentProject, company: companies::CompanyModifiers) -> f64 {
    company.work_rate * production_operating_fraction(w, job)
}
...
pub fn supply_plan(w: &WorldState, id: NationId) -> Vec<ProjectSupply> {
    ...
    let mut components_stock=crate::commerce::stock(w,id,crate::commerce::Good::AdvancedComponents);
    let mut out = vec![];
    for job in jobs {
        let department = supply_department(job); let terms = work_terms(job);
        let company = work_company(w, id, job);
        let billed_rate = terms.rate * (1.0 + company.fee_rate);
        ...
        p.planned_work_days = company_work_capacity(w, job, company).min(terms.stage_remaining)
            .min(job.daily_budget_bn / billed_rate).min(funds[department] / billed_rate);
        ...
        p.advanced_components_required=project_components_with_company(w,job,p.planned_work_days,company);
        let mut fraction = if p.advanced_components_required>0.0 {
            (components_stock/p.advanced_components_required).clamp(0.0,1.0)
        } else {1.0_f64};
        ...
        if p.executable_work_days > 1e-12 {
            ...
            let components_used = project_components_with_company(w,job,p.executable_work_days,company);
            ...
            if p.executable_raw.iter().zip(stock).any(|(need,have)|*need>1e-12 && *need>have)
                || components_used>components_stock
                ...
                p.advanced_components_used=components_used;
                components_stock=(components_stock-p.advanced_components_used).max(0.0);
        ...
        if p.advanced_components_required>p.advanced_components_available+1e-9 {
            p.blockers.push("Advanced components limit this equipment project. Import components or operate Advanced Industry.".into());
        }
        if production_operating_fraction(w,job)<1.0 {
            p.blockers.push("Workers or electricity limit this production facility.".into());
        }
...
pub fn next_work_supply(w: &WorldState, id: NationId) -> NextWorkSupply {
    ...
    for job in jobs {
        ...
        let payment=company_payment(job,step,company);
        ...
        out.advanced_components+=project_components_with_company(w,job,step,company);
    }
    out
}
...
mod supply_tests {
    ...
    fn company_operation(w:&mut WorldState,district:&str,fraction:f64) {
        let mut receipt=crate::industry_operations::site(w,district,crate::production::ProjectKind::ArmsPlant);
        receipt.utilization=fraction;
        w.production.operations.receipts=vec![receipt];
        w.production.operations.support_day=Some(clock::absolute_day(w));
    }
    fn assign_company(w:&mut WorldState,project:u32,saving:bool)->companies::CompanyModifiers {
        companies::enable(w);
    ...
    fn company_supply_preview_and_settlement_share_joint_raw_component_and_operating_limits() {
        let (mut fixture,d,r)=fixture();
        let id=start_production(&mut fixture,USA,&r,&d,2,1.0).unwrap();
        let tooling=job(&fixture,id).tooling_days;for _ in 0..tooling {next(&mut fixture);}
        open_next(&mut fixture);
        fixture.rules.industry_rebuild=true;
        company_operation(&mut fixture,&d,0.5);
        for saving in [false,true] {
            // Keep both supplies short in each case, alternating the binding
            // input. Half a component bundle is exactly representable; 30%
            // can reconstruct one float above stock and correctly trigger the
            // separate strict-rejection contract instead of fractional work.
            for (raw_share,component_share) in [(0.4,0.65),(0.8,0.5)] {
                let mut w=fixture.clone();let company=assign_company(&mut w,id,saving);
                let full=plan(&w,id);let ci=Commodity::Iron.idx();
                approx(full.planned_work_days,company.work_rate*0.5);
                assert!(full.advanced_components_required>0.0 && full.planned_day[ci]>0.0);
                resources::set_stockpile_for_test(&mut w,USA,Commodity::Iron,full.planned_day[ci]*raw_share);
                let components=full.advanced_components_required*component_share;
                w.production.operations.advanced_components.insert(USA,components);
                let before=crate::save(&w);let planned=plan(&w,id);let funded=next_work_supply(&w,USA);
                assert_eq!(funded.raw,full.planned_day);
                approx(funded.advanced_components,full.advanced_components_required);
                assert_eq!(raw_supply_demand(&w,USA).next_work,funded.raw);
                assert_eq!(crate::save(&w),before,"all company supply views must remain pure");
                assert!(planned.executable_work_days>0.0 && planned.executable_work_days<planned.planned_work_days,
                    "saving={saving}, raw_share={raw_share}, component_share={component_share}, plan={planned:?}");
                ...
                approx(components-crate::commerce::stock(&w,USA,crate::commerce::Good::AdvancedComponents),planned.advanced_components_used);
                ...
                let save=crate::save(&w);assert_eq!(crate::save(&crate::load(&save).unwrap()),save);
            }
        }
    }
    #[test]
    fn company_raw_horizon_tracks_base_contract_and_signed_ingredient_receipts() {
        ...
        open_next(&mut w);w.rules.industry_rebuild=true;company_operation(&mut w,&d,0.5);
        assign_company(&mut w,id,true);
        w.production.operations.advanced_components.insert(USA,1_000_000.0);
        let forecast=raw_supply_demand(&w,USA);let old=job(&w,id).clone();
        for day in 0..crate::economic_ai::RAW_HORIZON_DAYS[0] {
            if day>0 {open_next(&mut w);company_operation(&mut w,&d,0.5);}
            tick_day(&mut w);crate::programs::finish_day(&mut w);
        }
        let actual=job(&w,id);
        for i in 0..12 {approx(actual.resources_used[i]-old.resources_used[i],forecast.horizons[i][0]);}
        assert!(actual.company_fees_bn>0.0);
        validate_state(w.nation(USA)).unwrap();
    }
    #[test]
