...
pub fn jobs_per_level(kind: K) -> f64 {
    match kind {
        K::CivilianIndustry => 0.0, K::OfficeDistrict => 10_000.0,
...
pub fn worker_fraction(w: &WorldState, nation: NationId, kind: K) -> f64 {
    ...
    let general = ratio(available, workforce_demand(w, nation, false)).min(1.0);
...
pub fn power_per_level(w: &WorldState, d: &str, kind: K) -> f64 {
    ...
    match kind {
        K::CivilianIndustry => 0.0,
        K::OfficeDistrict => 0.25 * efficient,
        K::Shipyard => 0.50 * efficient,
        K::AdvancedIndustry => 0.80 * efficient,
        K::ArmsPlant => 0.0,
        K::ResearchCenter => 0.0,
...
pub fn power_fraction(w: &WorldState, district: &str) -> f64 {
    ...
    ratio(industry::power_capacity(w, nation), power_required(w, nation))
        .min(ratio(grid_capacity(w, district), local_power_required(w, district))).min(1.0)
}
...
pub fn site(w: &WorldState, district: &str, kind: K) -> FacilityOperation {
    ...
    if kind == K::CivilianIndustry {
        constraint = "Industrial estates provide passive prerequisites for civilian production; they do not sell construction capacity or charge a daily operating fee.".into();
    }
    FacilityOperation { nation:owner, recorded_day:None, district: district.into(), kind: kind.key().into(), name: production::catalog(kind).name.into(), inherited: false,
        ...
        input_fraction, funding_fraction, status: if kind == K::CivilianIndustry { "passive_capability" } else if fraction <= EPS { "blocked" } else if fraction + EPS < 1.0 { "limited" } else { "awaiting_operation" }.into(),
        reason: constraint, annual_gdp_bn: 0.0, annual_tax_bn: 0.0, cash_spent_daily_bn: 0.0 }
}

pub fn civilian_operating_fraction(_w: &WorldState, _district: &str) -> f64 {
    // Existing estates remain passive prerequisites, not construction services.
    1.0
}
...
fn operate(w: &mut WorldState, d: &str, k: K) {
    ...
    let raw = operating_raw_recipe(w,nation,d,k,work,power);
...
/// Settle new dock support before operating consumers. Construction never reads
/// staffing, power or operating receipts. Legacy estates remain passive enablers.
...
pub fn snapshot(w: &WorldState, nation: NationId) -> OperationsSnapshot {
    ...
    OperationsSnapshot { nation, as_of_day: w.production.operations.last_day.or(w.production.industry.last_day),
        ...
        advanced_components_required_daily:crate::manufacturing::advanced_components_demand_daily(w,nation),
...
/// Historical legacy ledgers remain owned by their existing validators.
pub fn has_state(w: &WorldState) -> bool {
    let advanced=crate::commerce::Good::AdvancedComponents;
    let commercial_components=w.commerce.as_ref().is_some_and(|c|
        c.policies.iter().any(|r|r.good==advanced)
        || c.offers.iter().any(|r|r.good==advanced)
        || c.contracts.iter().any(|r|r.good==advanced)
...
pub fn validate(w: &WorldState) -> Result<(), String> {
    let state=&w.production.operations;
    let new_kind=|k|matches!(k,K::OfficeDistrict|K::Shipyard|K::AdvancedIndustry);
    let used=has_state(w);
    if used && !enabled(w) { return Err("Saved industry facilities, components or dock reservations require their enabled daily industry rule.".into()); }
    for (district,levels) in &w.production.rebuild_sites {
        if !w.districts.contains_key(district) || levels.iter().any(|v|*v>production::MAX_PROVINCE_LEVEL) {
            return Err(format!("Invalid saved industry facility levels for {district}."));
        }
    }
    let day=clock::absolute_day(w);
    ...
    for p in w.production.projects.iter().filter(|p|new_kind(p.kind)) {
        if w.nation_opt(p.nation).is_none() || !w.districts.contains_key(&p.district)
            || p.total_days==0 || !p.progress_days.is_finite() || p.progress_days<0.0
            || p.progress_days>p.total_days as f64+EPS
            || p.resources_used.iter().any(|v|!v.is_finite()||*v<0.0)
            || p.started_day.is_some_and(|d|d>day) {
            return Err("Saved industry construction has invalid ownership, work or dates.".into());
        }
        if let Some(f)=w.production.industry.projects.get(&p.id) {
            if !f.spent_bn.is_finite() || f.spent_bn<0.0
                || [f.contract_cost_bn,f.last_spent_bn].iter().flatten().any(|v|!v.is_finite()||*v<0.0)
                || [f.goods_used.intermediates,f.goods_used.capital_goods].iter().any(|v|!v.is_finite()||*v<0.0)
                || f.contract_cost_bn.is_some_and(|cost|f.spent_bn>cost+EPS)
                || f.last_day.is_some_and(|d|d>day) {
                return Err("Saved industry construction funding is invalid.".into());
            }
        }
    }
    for (nation,amount) in &state.advanced_components {
        if w.nation_opt(*nation).is_none() || !amount.is_finite() || *amount<0.0 {
            return Err("Advanced-component stock has an invalid owner or quantity.".into());
        }
    }
    let mut seen=std::collections::BTreeSet::new();
    for r in &state.receipts {
        let numbers=[r.installed_capacity,r.operating_capacity,r.utilization,r.output_daily,
            r.jobs_required,r.jobs_filled,r.power_required_daily,r.power_used_daily,
            r.worker_fraction,r.power_fraction,r.input_fraction,r.funding_fraction,
            r.annual_gdp_bn,r.annual_tax_bn,r.cash_spent_daily_bn];
        if numbers.iter().any(|v|!v.is_finite()||*v<0.0)
            || [r.actual_operating_capacity,r.actual_utilization].iter().flatten().any(|v|!v.is_finite()||*v<0.0) {
            return Err("Industry operating receipt contains an invalid quantity.".into());
        }
        if r.inherited || !K::parse(&r.kind).is_some_and(new_kind)
            || !w.districts.contains_key(&r.district)
            || r.nation.is_none_or(|n|w.nation_opt(n).is_none())
            || r.recorded_day.is_none() || r.recorded_day!=state.support_day
            || !seen.insert((&r.district,&r.kind)) {
            return Err("Industry operating receipt has an invalid identity or date.".into());
        }
        if r.operating_capacity>r.installed_capacity+EPS || r.utilization>1.0+EPS
            || r.jobs_filled>r.jobs_required+EPS || r.power_used_daily>r.power_required_daily+EPS {
            return Err("Industry operating receipt exceeds installed capacity.".into());
        }
    }
    crate::manufacturing::validate_state(w)
}
