...
/// Static route identities used only while inspecting a save. Operational
/// access, current capacity and current hub ownership deliberately do not enter
/// this check: a booked journey can remain on hold after those facts change.
pub(crate) struct SavedRoutes {
    nodes: BTreeSet<String>,
    districts: BTreeSet<String>,
    links: BTreeSet<(String, String)>,
    segments: BTreeMap<String, (String, String, bool)>,
}
impl SavedRoutes {
    pub(crate) fn new(w: &WorldState) -> Self {
        ...
        for edge in edges {
            let a = nodes[edge.a].id.clone();
            let b = nodes[edge.b].id.clone();
            links.insert((a.clone(), b.clone()));
            links.insert((b.clone(), a.clone()));
            segments.insert(edge.key, (a, b, edge.sea));
        }
        Self { nodes: nodes.into_iter().map(|n| n.id).collect(), districts: w.districts.keys().cloned().collect(), links, segments }
    }
    fn freight(&self, route: &[String]) -> Result<(), String> {
        if route.iter().any(|id| !self.nodes.contains(id))
            || route.windows(2).any(|pair| !self.links.contains(&(pair[0].clone(), pair[1].clone())))
        {
            return Err("campaign supply contains unknown/disconnected route geometry".into());
        }
        Ok(())
    }
    pub(crate) fn transfer(&self, route: &[String]) -> Result<(), String> {
        // Local sector redeployment uses geographical district adjacency; hub
        // deployments and returns use the freight graph, including port nodes.
        if route.iter().any(|id| !self.nodes.contains(id) && !self.districts.contains(id)) {
            return Err("campaign transfer contains an unknown route node".into());
        }
        for pair in route.windows(2) {
            if !self.links.contains(&(pair[0].clone(), pair[1].clone()))
                && !districts::adj_of(&pair[0]).contains(&pair[1])
            {
                return Err("campaign transfer contains disconnected route geometry".into());
            }
        }
        Ok(())
    }
    fn cargo(&self, cargo: &ServiceCargo) -> Result<(), String> {
        self.freight(&cargo.route)?;
        if cargo.route.is_empty() || cargo.segments.len() != cargo.route.len() - 1
            || cargo.route.last() != Some(&cargo.district)
        {
            return Err("campaign cargo route/segment/destination mismatch".into());
        }
        let mut at_sea = false;
        for (pair, key) in cargo.route.windows(2).zip(&cargo.segments) {
            let Some((a, b, sea)) = self.segments.get(key) else {
                return Err(format!("campaign cargo contains unknown booked segment {key}"));
            };
            if !((&pair[0] == a && &pair[1] == b) || (&pair[0] == b && &pair[1] == a)) {
                return Err("campaign cargo segment does not join its booked nodes".into());
            }
            at_sea |= *sea;
        }
        if at_sea != cargo.at_sea { return Err("campaign cargo sea flag differs from booked route".into()); }
        Ok(())
    }
}
fn saved_key(w: &WorldState, key: &str) -> Result<(u32, NationId, String), String> {
    let (key, garrison) = key.strip_prefix("garrison:").map_or((key, false), |key| (key, true));
    let Some((participant, district)) = key.rsplit_once(':') else { return Err("invalid campaign supply identity".into()); };
    let (cid, nation) = crate::campaign::validate_key(w, participant, garrison)?;
    crate::campaign::validate_district(w, district, "campaign supply")?;
    Ok((cid, nation, district.into()))
}
/// Validate saved service quantities and booking identities without generating,
/// consuming, moving or expiring support. No stock-to-force equality is imposed:
/// national strength and ownership can change between the dated settlements.
pub fn validate(w: &WorldState) -> Result<(), String> {
    use crate::campaign::{validate_amount, validate_district, validate_identity,
        validate_nation, validate_saved_day, validate_unit};
    let state = &w.campaign_supply;
    if let Some(day) = state.last_day { validate_saved_day(w, day, "campaign supply settlement")?; }
    for (&nation, source) in &state.sources {
        validate_nation(w, nation, "campaign supply source")?;
        validate_district(w, &source.district, "campaign supply source")?;
        validate_amount(source.service, "campaign source service")?;
    }
    for (key, buffer) in &state.buffers {
        validate_identity(w, buffer.conflict, buffer.nation, "campaign supply buffer")?;
        if saved_key(w, key)? != (buffer.conflict, buffer.nation, buffer.district.clone()) {
            return Err(format!("campaign buffer identity mismatch {key}"));
        }
        validate_amount(buffer.service, "campaign buffered service")?;
        validate_amount(buffer.demand, "campaign buffer demand")?;
    }
    let routes = (!state.cargo.is_empty() || state.deliveries.values().any(|d| !d.route.is_empty()))
        .then(|| SavedRoutes::new(w));
    let mut dispatches = BTreeSet::new();
    for cargo in &state.cargo {
        validate_identity(w, cargo.conflict, cargo.nation, "campaign cargo")?;
        if saved_key(w, &cargo.key)? != (cargo.conflict, cargo.nation, cargo.district.clone()) {
            return Err(format!("campaign cargo identity mismatch {}", cargo.key));
        }
        validate_amount(cargo.service, "campaign cargo service")?;
        validate_saved_day(w, cargo.dispatched_day, "campaign cargo dispatch")?;
        if cargo.due_day < cargo.dispatched_day {
            return Err("campaign cargo is due before dispatch".into());
        }
        // prepare deduplicates request keys and posts at most once on a date.
        if !dispatches.insert((&cargo.key, cargo.dispatched_day)) {
            return Err("duplicate campaign service dispatch for the same request/date".into());
        }
        routes.as_ref().expect("cargo creates route index").cargo(cargo)?;
    }
    for (key, delivery) in &state.deliveries {
        let (_, _, district) = saved_key(w, key)?;
        if key != &delivery.key { return Err(format!("campaign delivery identity mismatch {key}")); }
        validate_unit(delivery.coverage, "campaign delivered coverage")?;
        if !delivery.route.is_empty() {
            routes.as_ref().expect("delivery creates route index").freight(&delivery.route)?;
            if delivery.route.last() != Some(&district) {
                return Err("campaign delivery route has a different destination".into());
            }
        }
    }
    Ok(())
...
mod saved_validation_tests {
    ...
    fn retained() -> WorldState {
        let mut w = crate::init::world_1990(crate::world::GameRules {
            daily_simulation: true, military_operations: true, operational_warfare: 1,
            resource_market: true, logistics_routes: true, physical_logistics: true,
            ..crate::world::GameRules::default()
        });
        w.nation_mut(NationId::Germany).mil_strength = 20.0;
        w.campaign_supply.sources.insert(NationId::Germany, Source { district: "DE-BE".into(), service: 0.0 });
        let request = SupplyRequest { key: crate::campaign::sector_key(77, NationId::Germany, "DE-BB"),
            nation: NationId::Germany, conflict: 77, district: "DE-BB".into(), deployed: 1.0,
            burn_monthly: 0.0, sea_escort: 1.0, sea_denial: 0.0 };
        logistics::begin_month(&mut w);
        prepare(&mut w, &[request]);
        assert!(!w.campaign_supply.cargo.is_empty(), "fixture must contain a real dispatch");
        w.day = 25;
        w.nation_mut(NationId::Germany).alive = false;
        w.campaign_supply.cargo[0].hold_reason = Some("Saved closed corridor".into());
        w
    }
    #[test]
    fn supply_validation_preserves_real_dispatch_after_expiry_and_loss_of_access() {
        let w = retained();
        assert!(w.campaign_supply.cargo[0].due_day < clock::absolute_day(&w));
        let before = serde_json::to_value(&w.campaign_supply).unwrap();
        assert_eq!(validate(&w), Ok(()));
        assert!(before == serde_json::to_value(&w.campaign_supply).unwrap(), "validation must not expire or deliver booked service");
    }
    #[test]
    fn supply_validation_rejects_corrupt_identity_dates_quantities_and_routes() {
        let w = retained();
        let mut corrupt = w.clone(); corrupt.campaign_supply.cargo[0].nation = NationId::USA;
        assert!(validate(&corrupt).is_err());
        let mut corrupt = w.clone(); corrupt.campaign_supply.cargo[0].dispatched_day = 25;
        assert!(validate(&corrupt).is_err());
        let mut corrupt = w.clone(); corrupt.campaign_supply.cargo[0].due_day = -1;
        assert!(validate(&corrupt).is_err());
        let mut corrupt = w.clone(); corrupt.campaign_supply.cargo[0].service = -1.0;
        assert!(validate(&corrupt).is_err());
        let mut corrupt = w.clone(); corrupt.campaign_supply.sources.values_mut().next().unwrap().service = f64::INFINITY;
        assert!(validate(&corrupt).is_err());
        let mut corrupt = w.clone(); corrupt.campaign_supply.cargo[0].segments[0] = "invented-segment".into();
        assert!(validate(&corrupt).is_err());
        let mut corrupt = w.clone(); corrupt.campaign_supply.cargo[0].at_sea = !corrupt.campaign_supply.cargo[0].at_sea;
        assert!(validate(&corrupt).is_err());
        let mut corrupt = w.clone(); let duplicate = corrupt.campaign_supply.cargo[0].clone(); corrupt.campaign_supply.cargo.push(duplicate);
        assert!(validate(&corrupt).is_err());
        let mut corrupt = w; corrupt.campaign_supply.deliveries.values_mut().next().unwrap().coverage = 1.1;
        assert!(validate(&corrupt).is_err());
    }
    #[test]
    fn supply_is_empty_observes_every_persisted_field() {
        let sample = retained().campaign_supply;
        assert!(SupplyState::default().is_empty());
        for field in 0..5 {
            let mut state = SupplyState::default();
            match field {
                0 => state.sources = sample.sources.clone(),
                1 => state.buffers = sample.buffers.clone(),
                2 => state.cargo = sample.cargo.clone(),
                3 => state.last_day = Some(0),
                4 => state.deliveries = sample.deliveries.clone(),
                _ => unreachable!(),
            }
            assert!(!state.is_empty(), "supply field {field} would be silently omitted");
        }
    }
    #[test]
    fn supply_decode_rejects_unknown_nested_service_property() {
        let state = retained().campaign_supply;
        let mut shape = serde_json::to_value(&state).unwrap();
        assert!(serde_json::from_value::<SupplyState>(shape.clone()).is_ok());
        shape["cargo"][0].as_object_mut().unwrap()
            .insert("unrecorded_service".into(), serde_json::json!(2.0));
        assert!(serde_json::from_value::<SupplyState>(shape).is_err());
    }
