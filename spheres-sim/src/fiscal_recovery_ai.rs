...
mod tests {
    ...
    fn quarterly_ai_pays_ordinary_tax_price_and_never_changes_player_policy() {
        ...
        crate::apply_command(&mut manual, &proposed).unwrap();
        let pressure = w.fiscal_recovery.nations[&NationId::USA].confidence_pressure;
        tick(&mut w);
        assert_eq!(
            serde_json::to_value(w.nation(NationId::USA)).unwrap(),
            serde_json::to_value(manual.nation(NationId::USA)).unwrap(),
            "autonomous fiscal changes must be exactly the ordinary command's nation effects"
        );
    ...
    fn existing_ai_department_budgets_renew_without_industrial_competition() {
        ...
        assert!(w.fiscal_recovery.nations[&NationId::USA]
            .last_ai_review_day
            .is_none());
        let renewed = crate::save(&w);
        let mut resumed = crate::load(&renewed).unwrap();
        tick(&mut resumed);
        assert_eq!(
            crate::save(&resumed),
            renewed,
            "same-date reload cannot renew authority or grant cash twice"
        );
    }

    #[test]
    fn disabled_fiscal_ai_preserves_legacy_world_and_rng() {
