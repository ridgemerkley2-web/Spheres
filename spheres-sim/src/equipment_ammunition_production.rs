...
pub struct AmmoOrderQuote {
    ...
    pub daily_limit_bn: f64,
    pub recipe: [f64; 12],
    pub operating_fraction: f64,
    pub operating_rounds_per_day: f64,
}
...
pub fn ammo_order_quote(
    ...
    AmmoOrderQuote {
        ...
        operating_fraction: ammo_operating_fraction(w,district),
        operating_rounds_per_day: def.map_or(0.0,|d| d.rounds_per_day*ammo_operating_fraction(w,district)),
    }
}
...
fn ammo_site_blocker(w: &WorldState, id: NationId, job: &AmmoOrder, day: i32) -> Option<String> {
    ...
    let prior = crate::manufacturing::lines_for(w, id)
        .filter(|p| p.district == job.district && (!w.rules.industry_rebuild || !crate::manufacturing::is_naval(&p.kit)))
        .count()
...
/// Current staffed and powered factory service. The operating ledger also
/// checks and pays the support bundle once before actual ammunition work.
pub fn ammo_operating_fraction(w:&WorldState,district:&str)->f64 {
    crate::industry_operations::operating_fraction(w,district,crate::production::ProjectKind::ArmsPlant)
}
fn ammo_operating_advance(w:&WorldState,job:&AmmoOrder,available:f64)->f64 {
    if !crate::industry_operations::enabled(w) {return ammo_next_advance(job,available);}
    let unit=job.cost_bn/job.quantity as f64;
    (job.quantity as f64-job.work_rounds).max(0.0)
        .min(job.rounds_per_day*ammo_operating_fraction(w,&job.district))
        .min(job.daily_limit_bn/unit).min(available/unit)
}
...
fn tick_ammunition_work(w: &mut WorldState, id: NationId) {
    ...
    for (_, i) in jobs {
        ...
        let desired = ammo_operating_advance(w,&job,available);
        let mut advance =
        ...
        if advance <= 0.0 || job.work_rounds + advance == job.work_rounds {
            ammo_set_reason(w,id,i,if available<=0.0{"No maintenance and supply authority remains after fleet servicing."}
                else if ammo_operating_fraction(w,&job.district)<=0.0 {"The military factory lacks workers, electricity or funded operating support. Restore its operating service to resume ammunition production."}
                else{"Waiting for the physical raw inputs or enough funding for the next work increment."}.into());
            continue;
        }
        if advance > job.quantity as f64 - job.work_rounds {
...
fn ammunition_raw_plan(w: &WorldState, id: NationId, horizon: i32) -> EquipmentRawDemand {
    ...
    // Forecast current dockside repair support through the same naval invoice
    // quote as actual maintenance; future staffing or power changes may alter it.
    let requirement = fleet_maintenance_requirement(n) + legacy_maintenance_requirement_world(w, id);
    let mut used = [0.0; 12];
    for offset in 0..horizon {
        ...
        if day >= plan.from_day {
            ...
            for job in &mut jobs {
                ...
                // This remains the desired funded material plan. A shortage
                // of factory power must not erase the inventory required when
                // service resumes; current operating capacity is quoted apart.
...
mod ammunition_production_tests {
    ...
    #[test]
    fn rebuilt_ammunition_factory_stops_in_an_outage_without_losing_its_material_plan() {
        let (mut w,district)=fixture();
        w.rules.industry_rebuild=true;
        for kind in [crate::production::ProjectKind::Generation,crate::production::ProjectKind::PowerGrid] {
            crate::production::complete_capability(&mut w,&district,kind);
        }
        for c in [Commodity::Iron,Commodity::Copper,Commodity::Coal] {
            resources::set_stockpile_for_test(&mut w,USA,c,1_000_000.0);
        }
        let def=ammo_def("mg_127").unwrap();
        let quantity=(def.rounds_per_day*4.0) as u32;
        let id=start_ammo_order(&mut w,USA,"mg_127",&district,quantity,0.1).unwrap();
        open_next(&mut w);
        crate::industry_operations::begin_day(&mut w);
        near(ammo_operating_fraction(&w,&district),1.0,1e-12);
        settle_support(&mut w);
        assert!(job(&w,id).completed_rounds>0);
        assert!(job(&w,id).completed_rounds<quantity);
        ...
        crate::industry_operations::begin_day(&mut w);
        near(ammo_operating_fraction(&w,&district),0.0,1e-12);
        let forecast=ammunition_next_work(&w,USA);
        assert!(forecast.iter().any(|v|*v>0.0),"an outage must not erase the funded material request");
        let before=job(&w,id).clone();
        let stock=state(&w).stocks.clone();
        let raw=w.resources.clone();
        settle_support(&mut w);
        assert_eq!(job(&w,id).completed_rounds,before.completed_rounds);
        near(job(&w,id).spent_bn,before.spent_bn,1e-15);
        assert_eq!(state(&w).stocks,stock);
        assert_eq!(w.resources,raw);
        assert!(job(&w,id).reason.contains("electricity"));
