...
fn population_tolerance(population: f64) -> f64 {
    // Million-person units: rounding allowance, never an extra resident pool.
    1e-9 * population.abs().max(1.0)
}
/// Save/load boundary contract. Reject orphaned outcomes, partial books and
/// invalid cohorts instead of silently disabling demography or reseeding it.
/// This is a read-only validation pass, not a migration or daily tick.
pub fn validate(w: &WorldState) -> Result<(), String> {
    let state = &w.population_system;
    if !state.enabled {
        if !state.is_empty() || w.nations.iter().any(|n| n.population_outcomes.is_some()) {
            return Err(
                "Disabled population accounts must not retain a book or published outcomes.".into(),
            );
        }
        return Ok(());
    }
    if !clock::is_daily(w) {
        return Err("Enabled population accounts require the daily calendar.".into());
    }
    let day = clock::absolute_day(w);
    if state.last_day.is_some_and(|last| last > day) {
        return Err("Population settlement date is ahead of the campaign.".into());
    }
    let valid_nonnegative = |v: f64| v.is_finite() && v >= -1e-10;
    let valid_share = |v: f64| v.is_finite() && (-1e-10..=1.0 + 1e-10).contains(&v);
    let check_province = |label: &str, p: &ProvincePopulation| -> Result<(), String> {
        let bad = || format!("Invalid population or workforce account: {label}.");
        let scalars = [
            p.retirees_m,
            p.birth_rate_per_adult,
            p.reference_population_m,
            p.inherited_filled_m,
            p.underemployed_m,
            p.labor_force_m,
            p.military_m,
            p.training_capacity_m,
            p.annual_graduates_m,
            p.living_standards,
            p.social_mobility,
            p.baseline_education_share,
        ];
        if !scalars.into_iter().all(valid_nonnegative)
            || !p.children.values().copied().all(valid_nonnegative)
            || p.children.keys().any(|year| *year > w.year)
            || !p.working.into_iter().all(valid_nonnegative)
            || !p
                .jobs_reference
                .iter()
                .chain(&p.jobs)
                .chain(&p.project_jobs)
                .chain(&p.filled)
                .flatten()
                .copied()
                .all(valid_nonnegative)
            || !p
                .employed_by_skill
                .into_iter()
                .chain(p.unemployment_by_skill)
                .chain(p.wealth_indices)
                .chain(p.class_living)
                .chain(p.class_risk_reference)
                .chain(p.wage_indices)
                .all(valid_nonnegative)
            || ![
                p.participation_reference,
                p.participation,
                p.school_coverage,
                p.school_reference,
                p.opening_unemployment,
            ]
            .into_iter()
            .chain(p.class_shares)
            .chain(p.staffing_reference)
            .all(valid_share)
            || (p.class_shares.iter().sum::<f64>() - 1.0).abs() > 1e-8
            || p.intake_month > w.year * 12 + w.month as i32
        {
            return Err(bad());
        }
        let tolerance = population_tolerance(p.population_m());
        let mut students = [0.0; SKILLS];
        for course in &p.courses {
            if course.from >= SKILLS
                || course.to >= SKILLS
                || course.from >= course.to
                || !valid_nonnegative(course.people_m)
                || !valid_nonnegative(course.funded_days)
                || !course.required_days.is_finite()
                || course.required_days <= 0.0
                || course.started_day > day
                || course.funded_days > course.required_days + 1e-9
                || course.funded_days > (day as i64 - course.started_day as i64) as f64 + 1e-9
            {
                return Err(format!("Invalid dated training course: {label}."));
            }
            students[course.from] += course.people_m;
        }
        for k in 0..SKILLS {
            if students[k] > p.working[k] + tolerance
                || p.employed_by_skill[k] + p.unemployment_by_skill[k]
                    > p.working[k] - students[k] + tolerance
            {
                return Err(bad());
            }
        }
        if p.military_m + p.students_m() + p.labor_force_m > p.working_age_m() + tolerance
            || p.employed_m() > p.labor_force_m + tolerance
            || (p.employed_m() - p.employed_by_skill.iter().sum::<f64>()).abs() > tolerance
            || (p.labor_force_m - p.employed_m() - p.unemployment_by_skill.iter().sum::<f64>())
                .abs()
                > tolerance
        {
            return Err(bad());
        ...
        for s in 0..8 {
            for g in 0..GRADES {
                if p.filled[s][g] > p.jobs[s][g] + p.project_jobs[s][g] + tolerance {
                    return Err(bad());
                }
            }
        }
    ...
    for (d, p) in &state.provinces {
        let Some(&owner) = w.districts.get(d) else {
            return Err(format!("Population province {d} has no mapped owner."));
        };
        if p.last_owner != owner || !w.nation_opt(owner).is_some_and(|n| n.alive) {
            return Err(format!(
                "Population province {d} has unreconciled ownership."
            ));
        }
        check_province(d, p)?;
        let actual = districts::population_of(w, d).unwrap_or(0.0);
        if !actual.is_finite() || (actual - p.population_m()).abs() > population_tolerance(actual) {
            return Err(format!(
                "Population province {d} disagrees with mapped residents."
            ));
        }
    }
    for (&id, p) in &state.unallocated {
        if p.last_owner != id || !w.nation_opt(id).is_some_and(|n| n.alive) {
            return Err("Unallocated residents require an explicit living successor.".into());
        }
        check_province(id.name(), p)?;
    }
    for (&id, row) in &state.nations {
        if w.nation_opt(id).is_none()
            || ![
                row.opening_gdp_bn,
                row.opening_population_m,
                row.opening_education_share,
                row.opening_military_strength,
                row.research_reference,
                row.last_inherited_employed_m,
                row.hardship_reference,
            ]
            .into_iter()
            .all(valid_nonnegative)
            || !row.labor_growth.is_finite()
            || !row.population_growth.is_finite()
            || row.last_policy_day.is_some_and(|last| last > day)
        {
            return Err(format!(
                "Invalid national population account: {}.",
                id.name()
            ));
        }
    }
    for n in &w.nations {
        if !n.alive {
            if n.population_outcomes.is_some() {
                return Err("A dissolved nation retains published population outcomes.".into());
            }
            continue;
        }
        let row = state
            .nations
            .get(&n.id)
            .ok_or_else(|| format!("{} is missing its population account.", n.id.name()))?;
        let expected: Vec<_> = w
            ...
            .map(|(d, _)| d.clone())
            .collect();
        if row.province_ids != expected || expected.iter().any(|d| !state.provinces.contains_key(d))
        {
            return Err(format!(
                "{} population province membership is inconsistent.",
                n.id.name()
            ));
        }
        let residents = sum_rows(w, n.id, |p| p.population_m());
        if !n.population.is_finite()
            || n.population < 0.0
            || (residents - n.population).abs() > population_tolerance(n.population)
        {
            return Err(format!(
                "{} population accounts do not conserve current residents.",
                n.id.name()
            ));
        }
        let outcomes = n
            .population_outcomes
            .as_ref()
            .ok_or_else(|| format!("{} has no published population outcomes.", n.id.name()))?;
        if !outcomes.labor_growth.is_finite()
            || !valid_share(outcomes.unemployment)
            || (outcomes.labor_growth - row.labor_growth).abs() > 1e-12
            || (outcomes.unemployment - unemployment(w, n.id).unwrap_or(0.0)).abs() > 1e-12
        {
            return Err(format!(
                "{} has inconsistent published population outcomes.",
                n.id.name()
            ));
        }
    }
...
pub fn enable(w: &mut WorldState) -> Result<(), String> {
    ...
    for n in w.nations.iter().filter(|n| n.alive) {
        if !n.population.is_finite()
            ...
            || !n.gdp.is_finite()
            || n.gdp < 0.0
            || !n.mil_strength.is_finite()
            || n.mil_strength < 0.0
        {
            return Err(format!(
                "{} has invalid opening population accounts.",
                n.id.name()
            ));
        }
        let mapped = w
...
fn enroll(w: &mut WorldState) {
    ...
    // Adoption itself accounts for this date without aging or graduating
    // anyone. A same-date load/tick cannot settle the day a second time.
    w.population_system.last_day = Some(clock::absolute_day(w));
    publish_outcomes(w);
}
fn ensure(w: &mut WorldState) {
    ...
    for id in alive {
        let new_account = !w.population_system.nations.contains_key(&id);
        if new_account {
            let n = w.nation(id);
            let source = source_data()
                .get("countries")
            ...
            if w.year != 1990 {
                notes.push(format!("Adopted in {} using current residents. Unobserved current age and qualification shares are modeled from the sourced 1990 profile; this is not a contemporary census.", w.year));
            }
        ...
        for d in owned {
            ...
            if !w.population_system.provinces.contains_key(&d) {
                // New territorial bookkeeping after enrollment may move
                // existing residents, but cannot invent them from a GDP or
                // population scalar. Existing province cohorts follow land.
                let p = seed_province(w, id, Some(&d), if initial_enrollment { pop } else { 0.0 });
                w.population_system.provinces.insert(d, p);
            }
        }
        let missing = (w.nation(id).population - mapped).max(0.0);
        if initial_enrollment && missing > 0.0 && !w.population_system.unallocated.contains_key(&id)
        {
            let p = seed_province(w, id, None, missing);
            w.population_system.unallocated.insert(id, p);
        }
        if new_account && !initial_enrollment {
            // A successor's constructor may carry an approximate population
            // scalar. Its initial income-per-person reference uses the actual
            // inherited residents; neither residents nor GDP are reseeded.
            let residents = sum_rows(w, id, ProvincePopulation::population_m);
            w.population_system
                .nations
                .get_mut(&id)
                .unwrap()
                .opening_population_m = residents;
        }
    }
}
...
fn project_demands(w: &WorldState) -> BTreeMap<String, [[f64; 3]; 8]> {
    ...
    // Construction is funding-only. Queued projects and mine works do not
    // allocate a domestic crew, block work or manufacture paid employment.
    // Only installed operating facilities add qualification demand above.
    out
}
#[derive(Clone)]
struct Context {
...
fn schooling(p: &mut ProvincePopulation, c: &Context) {
    ...
    for course in &mut p.courses {
        // Neither a future start nor prepaid funding can bypass calendar
        // time. New intakes are added after this loop and first study later.
        if c.day <= course.started_day {
            continue;
        }
        course.funded_days = (course.funded_days + c.days * funded.min(1.0))
            .min((c.day - course.started_day) as f64)
            .min(course.required_days);
...
fn write_populations(w: &mut WorldState) {
    ...
    for n in w.nations.iter_mut().filter(|n| n.alive) {
        n.population = totals.get(&n.id).copied().unwrap_or(0.0);
    }
}
...
pub fn tick(w: &mut WorldState) {
    ...
    let day = clock::absolute_day(w);
    if w.population_system.last_day.is_some_and(|last| last >= day) {
        return;
    }
    ensure(w);
...
pub fn record_casualties(w: &mut WorldState, id: NationId, force_loss_fraction: f64) {
    if !active(w) || !force_loss_fraction.is_finite() || force_loss_fraction <= 0.0 {
        return;
    }
...
pub fn transfer_unallocated(w: &mut WorldState, from: NationId, to: NationId) {
    if !w.population_system.enabled || from == to || !w.nation_opt(to).is_some_and(|n| n.alive) {
        return;
    }
    if let Some(mut moved) = w.population_system.unallocated.remove(&from) {
        ...
        if let Some(existing) = w.population_system.unallocated.get_mut(&to) {
            ...
            existing.intake_month = existing.intake_month.max(moved.intake_month);
            match_jobs(existing);
        } else {
