...
/// A district development takes one game year. This is deliberately short
/// enough to be an arcade decision rather than a ten-year infrastructure
/// forecast, while still leaving the investment exposed to conquest.
pub const MINE_BUILD_MONTHS: u32 = 12;

/// The standing needed to commit the government to a new extraction project.
pub const MINE_PC_COST: f64 = 6.0;

/// Construction is priced at two years of the project's expected 1990-value
/// output, with a small-project floor and a cap that keeps a single click from
/// swallowing a great power's entire fiscal year. The treasury charge is a
/// one-shot debt increase; it is never an ambient GDP penalty.
...
pub fn tick(w: &mut WorldState) {
    ...
    if stale {
        ...
        // Oil's national ledger is refreshed every month, so restore any
        // district-bound field development after that refresh. Every other
        // mine remains in the cache until ownership or the roster changes.
...
/// Advance district-bound construction. A contested project pauses; a project
/// whose district changes hands keeps building for the new owner, making the
/// map and the resource system one strategic surface rather than two ledgers.
...
pub struct Resources {
    /// District-bound extraction projects, sorted by (district, commodity).
    /// They follow the ground if it changes hands: conquest can capture an
    /// investment before it comes online.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub mine_projects: Vec<MineProject>,
    /// Completed district developments, sorted by (district, commodity).
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub mines: Vec<Mine>,
...
/// A mine, field or drilling programme under construction. `started_by` is
/// history, not ownership: the current district holder receives the finished
/// production and the project pauses while the ground is contested.
#[derive(Clone, Debug, Serialize, Deserialize, PartialEq)]
pub struct MineProject {
    ...
    /// Annual output in the resource table's unit.
    pub output: f64,
}

/// A completed district development. Output follows the district's current
/// owner through the same HAVE ledger as the 1990 located production.
#[derive(Clone, Debug, Serialize, Deserialize, PartialEq)]
pub struct Mine {
    pub district: String,
    pub commodity: Commodity,
    /// Annual output in the resource table's unit.
    pub output: f64,
    /// Absolute month in which construction completed.
    pub completed: i32,
}
...
/// Output of a new district development, annual in the table's unit. The
/// sourced median located mine is the only magnitude the artifact can defend;
/// the district's evidence band scales it without pretending to be tonnage.
pub fn mine_output(district: &str, c: Commodity) -> Option<f64> {
    let quality = quality_of(district, c);
...
/// Annual money value of a proposed mine's production, in $bn.
...
/// The one-shot treasury investment for a proposed district development.
...
/// Pure command precondition, shared by the price/refusal path, the dispatcher
/// and the browser card. No failed click spends political capital.
pub fn mine_refusal(
    w: &WorldState,
    nation: NationId,
    district: &str,
...
/// Commit a district-bound extraction project. Treasury investment is paid
/// once through debt; production does not touch GDP and enters only the
/// resource HAVE ledger when construction completes.
pub fn start_mine(
    w: &mut WorldState,
    nation: NationId,
    district: &str,
...
/// Remember a human buyer's rejected resource ask using the same verdict and
/// refusal row as the AI buy pass. Shape errors, counters, and prices beyond
/// the buyer's cap are not refusals. This closes the player path to the
/// last-resort predicate: a failed `ProposeDeal` must be evidence that a door
/// was actually closed, not merely an error message on the web surface.
pub(crate) fn remember_player_refusal(
    w: &mut WorldState,
    buyer: NationId,
    seller: NationId,
    give: &[Leg],
    ...
    if !w.rules.resource_market || w.player != Some(buyer) {
        return;
    }
    let Verdict::Refuse(reason) = evaluate(w, buyer, seller, give, take, months) else {
        return;
    };
    if matches!(reason, Reason::NotForThatPrice { .. }) || reason.sentence() != refusal {
        return;
    }
    let asked: Vec<Commodity> = take
        .iter()
        ...
        .collect();
    for commodity in asked {
        remember_refusal(w, buyer, seller, commodity, reason.clone());
    }
}
...
mod tests {
    ...
    fn owned_deposit(w: &WorldState, owner: NationId, c: Commodity) -> String {
        w.districts
            .iter()
            .find(|(district, held)| **held == owner && quality_of(district, c) > 0)
            .map(|(district, _)| district.clone())
            .unwrap_or_else(|| panic!("{} has no mapped {} deposit", owner.name(), c.name()))
    }

    #[test]
    fn mine_command_pays_once_builds_for_a_year_and_enters_have() {
        let mut w = built(world_1990(GameRules {
            resource_gates: true,
            ..GameRules::default()
        }));
        let chile = code("Chile");
        let c = Commodity::Copper;
        let district = owned_deposit(&w, chile, c);
        let expected = mine_output(&district, c).expect("mapped deposit has a benchmark");
        let cost = mine_cost_bn(&w, &district, c).expect("mapped deposit has a cost");
        let baseline = flow(&w, chile, c);
        let gdp = w.nation(chile).gdp;
        let debt = w.nation(chile).debt_gdp;
        w.nation_mut(chile).political_capital = 100.0;

        let cmd = Command::DevelopResource {
            nation: chile,
            district: district.clone(),
            commodity: c,
        };
        apply_command(&mut w, &cmd).expect("the owned mapped deposit starts");
        assert_eq!(w.nation(chile).political_capital, 100.0 - MINE_PC_COST);
        assert!((w.nation(chile).debt_gdp - debt - cost / gdp).abs() < 1e-12);
        assert_eq!(flow(&w, chile, c), baseline, "construction is not production");
        assert_eq!(mine_project_at(&w, &district, c).unwrap().months_left, MINE_BUILD_MONTHS);

        let saved = crate::save(&w);
        assert!(saved.contains("mine_projects"));
        let mut w = crate::load(&saved).expect("mine project roundtrip");
        assert_eq!(mine_project_at(&w, &district, c).unwrap().investment_bn, cost);

        for _ in 0..MINE_BUILD_MONTHS - 1 {
            tick(&mut w);
        }
        assert_eq!(mine_project_at(&w, &district, c).unwrap().months_left, 1);
        assert_eq!(flow(&w, chile, c), baseline);
        tick(&mut w);
        assert!(mine_project_at(&w, &district, c).is_none());
        assert!(mine_at(&w, &district, c).is_some());
        assert!((flow(&w, chile, c) - baseline - expected).abs() < expected * 1e-12 + 1e-6);

        let held = w.nation(chile).political_capital;
    ...
    #[test]
    fn mine_project_and_output_follow_the_district_when_it_is_captured() {
        let mut w = built(world_1990(GameRules {
            resource_gates: true,
            ..GameRules::default()
        }));
        ...
        assert!(w.headlines.iter().any(|h| h.contains(peru.name()) && h.contains("opens")));
    }
    ...
    /// `debt_gdp` at delivery or a one-shot mine investment, the relation at
    /// signing and cancellation and the reputation at cancellation. The consent slices of people, output,
