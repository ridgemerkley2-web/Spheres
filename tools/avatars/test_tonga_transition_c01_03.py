...
class TongaTransition2024Tests(unittest.TestCase):
    ...
    def test_eke_is_an_appointment_event_with_no_inferred_start_or_end(self):
        ...
        # CLAUDE-C01-08 inserts seven 1990-2018 holders before the 2019 one.
        self.assertEqual([h['name'] for h in holders if isinstance(h, dict)],
                         ["Fatafehi Tu'ipelehake", 'Baron Vaea', "Prince 'Ulukalala Lavaka Ata", 'Feleti Sevele',
                          ...
                          "Pohiva Tu'i'onetoa", SOVALENI, EKE])
    ...
    def test_news_and_social_leads_are_not_sources(self):
        ...
        for text in ('matangi', 'rnz', 'reuters', '9:53'):
            self.assertNotIn(text, self.raw.lower())
        # The live PMO former-PM list stays a lead for this packet; CLAUDE-C01-08 imports it (year-level entries) as
        # to_pmo_former_pms_2026, which must remain its only source record and cite no claim of this packet.
        listed = [s for s in self.packet['sources'] if 'former-prime-minsiters' in s['url']]
        self.assertEqual([s['id'] for s in listed], ['to_pmo_former_pms_2026'])
        self.assertFalse({c['id'] for c in listed[0]['claims']} & NEW_CLAIMS)
