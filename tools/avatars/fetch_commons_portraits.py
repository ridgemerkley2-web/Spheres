...
def write_rust_table(entries: dict[str, dict]) -> None:
    from display_variants import variant, OUT
    ...
    provenance = []
    for filename, (_, include_path) in list(assets.items()):
        source = (RUST_TABLE.parent / include_path).resolve()
        display, record = variant(source)
        assets[filename] = ("image/webp", f"../ui/display-art/{display.name}")
        provenance.append(record)
    atomic_write_text(OUT / "manifest.json", json.dumps(provenance, indent=2) + "\n")
