...
def main():
    ...
    for name in ['README.md','CURRENT_ARCHITECTURE.md','DECISIONS.md','PLAYTEST.md','PLAYER_DECISIONS.md','MILITARY_OPERATIONS.md','CAMPAIGN_AIMS.md','SECTOR_PROFILES.md','TECH_REFERENCE_REPAIR.md','HEADLESS_BASELINE_2026-09-04.md','DAILY_CALIBRATION.md','PERFORMANCE.md','INVESTMENT_COMPLETION_AUDIT.md','ART_DIRECTION.md','MANUFACTURING.md','PROVINCE_ECONOMY.md']:
        source=ROOT/name
        if source.exists():shutil.copy2(source,release/name)
    provenance=release/'attribution';provenance.mkdir()
    for src,dst in [('spheres-web/ui/height-detail.json','height-detail.json'),
                    ('spheres-web/ui/terrain-tiles/manifest.json','terrain-tiles/manifest.json'),
                    ('spheres-web/ui/lake-surfaces.json','lake-surfaces.json'),
                    ('tools/terrain/README.md','terrain-sources.md')]:
        target=provenance/dst
