...
async function review(page,url,command,valid=true){
  ...
  let found;for(const candidate of candidates){if(tab==='flight'){const section=candidate.path.split('.')[1],flightPage=section==='aircraft'?'aircraft':['bases','base_actions'].includes(section)?'bases':['staff','staff_countries'].includes(section)?'reports':'command';const pageButton=page.locator('[data-equipment-flight-page='+q(flightPage)+']');if(await pageButton.getAttribute('aria-pressed')!=='true')await pageButton.click();}const control=page.locator('[data-equipment-action='+q(candidate.path)+']');if(await control.count()&&await control.first().isVisible()){found=candidate;break;}}
...
async function main(){
  ...
  const shot=async(name,selector)=>{await page.locator('#banner').waitFor({state:'hidden'});if(selector)await page.locator(selector).first().scrollIntoViewIfNeeded();await page.screenshot({path:path.join(out,name+'.png')});e.screenshots.push(name+'.png');};
  ...
  try{
    ...
    for(const step of manifest.steps){
      if(typeof step.command==='string'){mark(step.command);const command=manifest.commands[step.command];assert(command);await review(page,url,command);
        ...
        if(['delivery_transit','quiet_flown','defense_queued','defense_flown'].includes(step.checkpoint)){
          ...
          await page.locator('[data-equipment-flight-page='+q(subject.includes('Dated mission results')?'reports':subject.includes('Airbases')?'bases':'command')+']').click();
    ...
    await equipment(page);await page.locator('[data-equipment-flight-page=reports]').click();await shot('continued-funded-defense','section[aria-label="Dated mission results"] .eq-card');assert.deepEqual(e.errors,[]);assert.equal(audit.fileHash(binary),binaryHash);assert.equal(audit.fileHash(fixtureFile),e.fixture.sha256);assert.equal(audit.fileHash(manifestFile),e.fixture.manifest_sha256);for(const f of e.imported_fixtures)assert.equal(audit.fileHash(f.file),f.sha256);assert.equal(git(['rev-parse','HEAD']).toString().trim(),revision);assert.equal(git(['status','--porcelain']).toString().trim(),'');
    assert.equal(e.commands.length,manifest.steps.filter(s=>typeof s.command==='string').length);assert.equal(e.advances.length,manifest.days_advanced);e.passed=true;e.finished_utc=new Date().toISOString();write('result.json',e);console.log(JSON.stringify({passed:true,result:path.join(out,'result.json')}));
