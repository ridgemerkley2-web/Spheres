...
for(const [id,platform,name] of [['fighter','air_fighter','Spheres fighter aircraft'],['light-attack','air_light_attack','Spheres light attack aircraft'],['tactical-strike','air_tactical_strike','Spheres tactical strike aircraft']])variants.push({id,name,file:`spheres-air-${id}.glb`,spec:{platform}});
const tacticalCounts = [0, 1, 2].map(lod =>
  build({platform: 'air_tactical_strike', lod}).triangleCount.toLocaleString('en-US'));
const readme = `# Spheres configurable ground and aviation model assets

These thirteen glTF 2.0 binary files contain the actual triangle geometry used by the Spheres equipment designer. They cover nine ground vehicle types, three aviation platforms and a mobile tank preset. They are original procedural game art created for this project, with no downloaded models, textures, logos, or third-party art dependencies. The separate arsenal catalogue model deck is documented with its own source and provenance.
...
- **spheres-air-fighter.glb**: single-seat interceptor, swept wings, air-search radar and selectable short/medium-range missiles.
...
Aircraft carry eight independent specifications: engine, wing, radar, avionics, countermeasures, hardpoints, payload and fuel. External engine nacelles, wing planform, radomes, cockpit fittings and targeting pods, dispensers, mounting stations, bombs and fuel tanks change with the selected components. The parked models include landing gear, cockpit glazing and framing, intake fans and open exhaust nozzles. The tactical rebuild adds a real cockpit opening, two seats and instruments, transparent glass, separate trailing controls, blended wing shoulders and underwing fuel tanks. Its GLB stores opaque/glass primitive slices over unchanged global buffers; mesh extras retain surface ranges and their opacity alongside semantic parts. All three inspection aircraft exceed 100,000 triangles, with cheaper catalogue/map meshes available in the generator. Fixed airframe and landing-gear geometry shares the wing structure selection; it is not an additional simulation upgrade. All eight slots are selectable in the live preview and retained in GLB metadata. This release depicts light attack, tactical strike and fighter aircraft for the three CP1 mission families. All three have open cockpits, translucent glazing, seats and instruments, recessed intake fans and detailed exhaust interiors. Fighter and light attack are single-seat concepts; tactical strike retains two seats.
...
The tactical interior pass adds closed swept intake fan blades, recessed exhaust liners and turbine faces, converging nozzle petals and actuator links. The two-seat cockpit includes shaped seat pads, harness webbing and buckles, side consoles, throttles, control grips, rudder pedals and distinct analog gauges/digital display graphics. The default tactical inspection build contains ${tacticalCounts[0]} triangles; catalogue detail uses ${tacticalCounts[1]} and the map mesh uses ${tacticalCounts[2]}. The flight workshop's Cockpit, Engines and Intakes controls inspect actual mesh regions without changing the export.
