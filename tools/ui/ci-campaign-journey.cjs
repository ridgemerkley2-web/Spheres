...
async function main(){
 ...
 try{
  ...
  const next=await capture('successor-result');let expected=structuredClone(old.world),actual=next.world;while(expected.world){expected=expected.world;actual=actual.world;}expected.player='Russia';expected.player_set_rate=false;expected.campaign_aims=actual.campaign_aims;assert.deepEqual(actual,expected);assert.equal(next.journey.transitions.length,1);assert.equal(actual.campaign_aims.active,null);assert.equal(actual.campaign_aims.history.at(-1).goal.nation,'USSR');assert.equal(actual.campaign_aims.history.at(-1).outcome,'government ended');proof.checks.push('Russia retains exact existing simulation state except player control/manual-rate ownership and the explicitly verified former-country aim record.');
