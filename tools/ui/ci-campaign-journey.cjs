...
async function main(){
 ...
 try{
  ...
  async function load(slot){
   ...
   await page.locator('#loadBtn').click();if(replacing)await page.locator('#campaignConfirmAccept').click();assert((await response).ok());await page.locator('#app').waitFor({state:'visible'});await idle();
  }
