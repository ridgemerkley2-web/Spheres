...
function companyRefitAction(extra={}){return action({label:'Review manufacturer refit',command:{kind:'company_refit',company:1,source:'USA-fleet-31',product:11,quantity:2},inputs:[{key:'quantity',label:'Aircraft to withdraw for refit',type:'number',value:2,min:1,max:4,step:1},{key:'product',label:'Certified target model',type:'select',value:11,options:[{value:11,label:'Swift B · National Armor'},{value:12,label:'Swift C · National Armor'}]}],...extra});}
function refitModels(data,platform='air_light_attack'){
  const original=plain(data.platforms.find(row=>row.id===platform).default_spec),components={...original.components};
  if(platform.startsWith('air_'))components.air_payload='air_payload_guided';else components.sensors='upgraded-refit-sensor';
  return {source:{revision:'USA-fleet-31',name:'Swift A',spec:original},target:{revision:'USA-model-41',name:'Swift B',spec:{platform,components}}};
}
function companyRefitQuote(data,command=companyRefitAction().command,extra={}){return quote({detail:'The manufacturer earns the fixed service fee only as completed equipment returns.',refit:refitModels(data),
  comparison:{available:true,same_role:true,name:'Swift A',detail:'Exact certified profile changes.',rows:[{label:'Strike rating',before:1.25,after:1.7,delta:.45,unit:'rating',better_when:'higher'},{label:'Daily maintenance',before:.000002,after:.000003,delta:.000001,unit:'bn',better_when:'lower'}],changes:[{slot:'air_payload',label:'Mission payload',before:'Unguided payload',after:'Guided payload'}]},
  metrics:[{label:'Aircraft unavailable during refit',value:2,unit:'aircraft'},{label:'Manufacturer',value:'National Armor'}],costs:[{label:'Full contract held in escrow',amount_bn:.018,period:'Defense procurement at confirmation'}],timing:[{label:'Shared manufacturer queue',value:'After active development; before stock manufacture'},{label:'First return',value:'22 workdays when supplied'}],
  service_effects:service({title:'Fleet during conversion',metrics:[{label:'Available aircraft',value:'4 → 2'},{label:'Daily maintenance after all returns',value:'$8k → $10k'}],warnings:['Started units finish; cancellation releases only unstarted vehicles.'],actions:[]}),
  actions:[{label:'Commission fixed-price refit',command:{...command,quote:'reviewed-refit-escrow'}}],...extra});}
function companyRefitSnapshot(){
  const data=supplierAmmoSnapshot();data.lots[0].actions.push(companyRefitAction());
  data.companies.service_overview=service({title:'Manufacturer service capacity',metrics:[{label:'Contracts awaiting work',value:2}],warnings:['Development uses the shared plant before refits.'],actions:[{label:'Review Defense procurement',navigate:{action:'budget',ministry:'defense',department:3}}]});
  data.companies.services=[{id:'service:1:3',name:'Guardian conversion',supplier_name:'National Armor',status:'Returning completed vehicles',metrics:[{label:'Returned vehicles',value:1}],actions:[]},{id:'service:1:4',name:'Swift fleet conversion',supplier_name:'National Armor',status:'Blocked · awaiting steel',phase:'blocked',progress:.25,refit:refitModels(data),metrics:[{label:'Aircraft withdrawn',value:2},{label:'Unstarted aircraft',value:1}],costs:[{label:'Unearned escrow',amount_bn:.018}],blockers:['Waiting for warehouse steel.'],milestones:[{label:'Cancellation',value:'Only untouched vehicles can be released',detail:'Any started aircraft finishes.'}],actions:[{label:'Cancel unstarted vehicles',requires_preview:true,command:{kind:'company_refit_cancel',company:1,refit:4}}]}];
  return data;
}

test('manufacturer refit contracts render in both service and companies with original action paths and retained public work',()=>{
  const c=fixture(),data=companyRefitSnapshot();loaded(c,data);const before=plain(data);c.equipmentSelectTab('service');
  for(const text of ['Refit service contracts','Manufacturer service capacity','Swift fleet conversion','Waiting for warehouse steel.','Unearned escrow','Only untouched vehicles can be released','Swift A','Swift B'])assert(c.mount.innerHTML.includes(text),text);
  assert(c.mount.querySelector('[data-equipment-action="companies.services.1.actions.0"]'));assert(c.mount.querySelector('[data-equipment-action="lots.0.actions.0"]'));assert(c.mount.querySelector('[data-equipment-action="lots.0.actions.1"]'));assert.equal(c.mount.querySelectorAll('#equipmentCompanyServicesTitle').length,1);
  c.mount.querySelector('[data-equipment-action="companies.service_overview.actions.0"]').onclick();assert.deepEqual(c.calls,[{action:'budget',ministry:'defense',department:3}]);
  c.equipmentSelectTab('companies');c.equipmentSetCompanyFamily('ammunition');assert.match(c.mount.innerHTML,/Swift fleet conversion/);assert.match(c.mount.innerHTML,/Manufacturer service capacity/);assert.equal(c.mount.querySelector('[data-equipment-model]'),null);c.eq.query='Swift fleet conversion';c.equipmentRender();assert(c.mount.querySelector('[data-equipment-action="companies.services.1.actions.0"]'));assert.equal(c.mount.querySelector('[data-equipment-record="service:1:3"]'),null);assert.deepEqual(plain(data),before);
});

test('refit contracts are optional for old saves and empty state guides the available fleet without inventing work',()=>{
  const c=fixture();loaded(c,snapshot({companies:companies()}));c.equipmentSelectTab('service');assert.doesNotMatch(c.mount.innerHTML,/Refit service contracts/);assert(c.mount.querySelector('[data-equipment-action="lots.0.actions.0"]'));
  c.eq.data.companies.services=[];c.equipmentRender();assert.match(c.mount.innerHTML,/No manufacturer refits commissioned/);assert.match(c.mount.innerHTML,/compatible certified upgrade/);assert.equal(c.mount.querySelector('[data-equipment-model]'),null);assert.equal(c.calls.length,0);
});

test('manufacturer refit review shows authoritative escrow, readiness, queue and exact profile comparison before spending',async()=>{
  const c=fixture(),data=companyRefitSnapshot();loaded(c,data);c.equipmentSelectTab('service');c.api=async(...args)=>{c.requests.push(plain(args));return companyRefitQuote(data,plain(args[1].command));};c.mount.querySelector('[data-equipment-action="lots.0.actions.1"]').onclick();await tick();
  for(const text of ['Full contract held in escrow','$18m','Defense procurement at confirmation','Aircraft unavailable during refit','4 → 2','$8k → $10k','22 workdays','before stock manufacture','Swift A → Swift B','1.25','1.7','+0.45','Guided payload','USA-fleet-31','USA-model-41'])assert(c.mount.innerHTML.includes(text),text);
  assert.equal(c.mount.querySelector('#equipmentComparison'),null);assert.equal(c.mount.querySelectorAll('[data-equipment-order-input="quantity"]').length,1);assert.equal(c.mount.querySelector('[data-equipment-order-input="daily_budget_mn"]'),null);assert.equal(c.calls.length,0);assert.equal(c.requests.length,1);
});

test('source and target inspection keep one exact aircraft or ground model and preserve unfinished designer specifications',async()=>{
  for(const platform of ['air_light_attack','ground_apc']){
    const c=fixture(),data=companyRefitSnapshot();loaded(c,data);c.eq.draft.name='Unfinished national design';c.equipmentRender();const draft=plain(c.eq.draft),model=c.modelMounts[0];c.eq.preview.actions=[companyRefitAction()];c.eq.previewKey=c.equipmentDraftKey();c.equipmentRender();
    c.api=async(...args)=>companyRefitQuote(data,plain(args[1].command),{refit:refitModels(data,platform)});c.mount.querySelector('[data-equipment-action="actions.0"]').onclick();await tick();const states=refitModels(data,platform);
    assert.equal([...c.mount.innerHTML.matchAll(/data-equipment-model(?:\s|>)/g)].length,1);assert.equal(c.mount.querySelector('#equipmentName'),null);assert.equal(model.updates.at(-1).platform,platform);assert.deepEqual(model.updates.at(-1).components,states.target.spec.components);assert.equal(Object.keys(model.updates.at(-1).components).length,platform.startsWith('air_')?8:13);
    const targetControl=c.mount.querySelector('[data-equipment-refit-model="target"]');assert.equal(targetControl.attributes['aria-pressed'],'true');c.mount.querySelector('[data-equipment-refit-model="source"]').onclick();assert.deepEqual(model.updates.at(-1).components,states.source.spec.components);assert.equal(model.updates.at(-1).name,'Swift A');assert.equal(c.document.activeElement.dataset.equipmentFocus,'refit-model:source');
    c.mount['onequipment-part-select']({detail:{slot:'sensors'}});assert.deepEqual(plain(c.eq.draft),draft);assert.equal(model.selectedParts.length,0);assert.equal(c.modelMounts.length,1);assert.equal(c.calls.length,0);
    c.mount.querySelector('[data-equipment-dismiss]').onclick();assert.equal(c.mount.querySelector('#equipmentName').value,draft.name);assert.deepEqual(model.updates.at(-1).components,draft.components);assert.equal(targetControl.onclick(),false);assert.deepEqual(plain(c.eq.draft),draft);
  }
});

test('changing refit target or quantity hides stale identity and rejects old model controls and confirmation',async()=>{
  const c=fixture(),data=companyRefitSnapshot();loaded(c,data);c.equipmentSelectTab('service');c.api=async(...args)=>companyRefitQuote(data,plain(args[1].command));c.mount.querySelector('[data-equipment-action="lots.0.actions.1"]').onclick();await tick();
  const oldConfirm=c.mount.querySelector('[data-equipment-intent="0"]'),oldModel=c.mount.querySelector('[data-equipment-refit-model="source"]');let resolve;c.api=(...args)=>{c.requests.push(plain(args));return new Promise(r=>resolve=r);};const target=c.mount.querySelector('[data-equipment-order-input="product"]');target.value='12';target.onchange();assert.equal(oldModel.onclick(),false);assert.equal(await oldConfirm.onclick(),false);assert.equal(c.mount.querySelector('[data-equipment-model]'),null);assert.match(c.mount.innerHTML,/model review will update/);
  const command=plain(c.requests.at(-1)[1].command),models=refitModels(data);models.target.name='Swift C';models.target.revision='USA-model-42';resolve(companyRefitQuote(data,command,{refit:models}));await tick();assert.match(c.mount.innerHTML,/Swift C/);assert.equal(c.modelMounts.at(-1).initial.name,'Swift C');assert.equal(oldModel.onclick(),false);assert.equal(await oldConfirm.onclick(),false);assert.equal(command.product,12);
  const quantity=c.mount.querySelector('[data-equipment-order-input="quantity"]');quantity.value='3';quantity.oninput();assert.equal(await c.equipmentConfirm(0),false);const next=plain(c.requests.at(-1)[1].command);resolve(companyRefitQuote(data,next,{refit:models,costs:[{label:'Full contract held in escrow',amount_bn:.027}]}));await tick();assert.match(c.mount.innerHTML,/\$27m/);c.api=async path=>path.startsWith('/api/equipment?')?data:preview();assert.equal(await c.equipmentConfirm(0),true);assert.deepEqual(c.calls,[{...next,quote:'reviewed-refit-escrow'}]);await tick();
});

test('refit cancellation reviews released and finishing quantities plus deferred refunds without a daily funding control',async()=>{
  const c=fixture(),data=companyRefitSnapshot();loaded(c,data);c.equipmentSelectTab('service');c.api=async(...args)=>companyRefitQuote(data,plain(args[1].command),{metrics:[{label:'Unstarted aircraft released',value:1},{label:'Aircraft already in work',value:1}],costs:[{label:'Refund after original fiscal settlement',amount_bn:.009}],actions:[{label:'Cancel unstarted vehicles',command:{...plain(args[1].command),quote:'reviewed-cancel-refit'}}]});c.mount.querySelector('[data-equipment-action="companies.services.1.actions.0"]').onclick();await tick();
  for(const text of ['Unstarted aircraft released','Aircraft already in work','Refund after original fiscal settlement','$9m'])assert(c.mount.innerHTML.includes(text),text);assert.equal(c.mount.querySelector('[data-equipment-order-input="daily_budget_mn"]'),null);assert.equal(c.calls.length,0);c.api=async path=>path.startsWith('/api/equipment?')?data:preview();assert.equal(await c.equipmentConfirm(0),true);assert.deepEqual(c.calls,[{kind:'company_refit_cancel',company:1,refit:4,quote:'reviewed-cancel-refit'}]);await tick();
});

test('unavailable refit offers and changed campaigns block spending while unknown or ammunition specs never get a guessed vehicle model',async()=>{
  for(const bad of [{platform:'unrecognized-prototype',components:{}},{platform:'air_light_attack',components:{},family:'ammunition'}]){
    const c=fixture(),data=companyRefitSnapshot();loaded(c,data);c.equipmentSelectTab('service');const unsafe='"><img src=x onerror=alert(1)>',models={source:{name:unsafe,spec:bad,family:bad.family},target:{name:unsafe,spec:bad,family:bad.family}};
    c.api=async(...args)=>companyRefitQuote(data,plain(args[1].command),{valid:false,refit:models,blockers:['Only one available source aircraft remains.','Manufacturer has no compatible certified target.']});c.mount.querySelector('[data-equipment-action="lots.0.actions.1"]').onclick();await tick();assert.match(c.mount.innerHTML,/No 3D preview is available for this exact revision/);assert.doesNotMatch(c.mount.innerHTML,/<img src=x/);assert.equal(c.mount.querySelector('[data-equipment-model]'),null);assert.equal(await c.equipmentConfirm(0),false);assert.equal(c.calls.length,0);
  }
  const c=fixture(),data=companyRefitSnapshot();loaded(c,data);c.equipmentSelectTab('service');c.api=async(...args)=>companyRefitQuote(data,plain(args[1].command));c.mount.querySelector('[data-equipment-action="lots.0.actions.1"]').onclick();await tick();const model=c.mount.querySelector('[data-equipment-refit-model="source"]'),confirm=c.mount.querySelector('[data-equipment-intent="0"]');c.S={session_id:'two',player:'FRA'};assert.equal(model.onclick(),false);assert.equal(await confirm.onclick(),false);assert.equal(c.calls.length,0);
});

test('company refit bridge retains reviewed fixed-price commands, pending refunds and drafts through the transaction channel',async()=>{
