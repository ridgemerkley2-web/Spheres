...
// ---- ground platforms: the designer's nine, at their default specification ----
const GLB = {
...
const vehicles = Object.keys(GLB).map((platform) => {
  ...
  return {
    asset_id: `ground.${platform}.baseline.v1`,
    ...
    lod: { LOD0: m.triangleCount },
    parts: m.parts.length,
    slots: Object.keys(m.specification.components).sort(),
    materials: 1,
    pivot: "hull centreline, ground contact at Y=0",
    bounds: bounds(m),
...
const manifest = {
  ...
  pipeline: {
    ...
    runtime_loader: "none, deliberately: the game ships no GLB loader and needs no build step",
  },
...
const rows = [
  ...vehicles.map((a) => [a.asset_id, a.game_id, String(a.lod.LOD0), "—", `${a.bounds.size.join(" x ")} m`]),
...
${sites.filter((s) => s.placeholder).length} of the ${sites.length} construction kinds are
