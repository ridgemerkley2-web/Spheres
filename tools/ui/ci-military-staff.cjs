...
async function main(){
  ...
  try{
    ...
    await equipment(page);e.flight_pages=await require('./flight-page-review.cjs').review({page,data:await board(page,url),out});await check('flight-pages-preserve-campaign','before.json');
    for(const [enabled,label,days] of [[true,'active',4],[false,'paused',2]]){
      ...
      await page.locator('[data-equipment-flight-page=reports]').click();const detail=page.locator('.eq-staff-details');if(await detail.getAttribute('open')===null)await detail.locator('summary').click();assert((await detail.innerText()).includes('Italy'));assert((await detail.innerText()).includes('Procurement'));
