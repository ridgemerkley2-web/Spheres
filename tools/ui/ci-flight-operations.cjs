...
async function equipment(page,tab='flight'){
  ...
  if(await page.locator('[data-equipment-tab='+q(tab)+']').getAttribute('aria-selected')!=='true')await page.locator('[data-equipment-tab='+q(tab)+']').click();
...
async function main(){
  ...
  const shot=async name=>{await page.screenshot({path:path.join(out,name+'.png')});e.screenshots.push(name+'.png');};
  ...
  try{
    ...
    for(const step of manifest.steps){
      if(step.command){mark(step.command);const command=manifest.commands[step.command];assert(command);await review(page,url,command);
        if(['foundation_host','support_army','strike_target'].includes(step.command)){await within(page.locator('#equipmentRoot'),'Flight review');await shot(step.command+'-review-1440');if(step.command==='support_army'){await page.setViewportSize({width:390,height:844});await within(page.locator('#equipmentRoot'),'Narrow mission review');await shot('support-army-review-390');await page.setViewportSize({width:1440,height:1000});}}
        ...
        if(['bases_completed','foreign_transit','returned_ready','missions_queued','missions_flown','funded_recovery'].includes(step.checkpoint)){
          await shot(step.checkpoint+'-1440');await page.setViewportSize({width:390,height:844});await within(page.locator('#equipmentRoot'),'Narrow Air Force');await shot(step.checkpoint+'-390');await page.setViewportSize({width:1440,height:1000});
        }
