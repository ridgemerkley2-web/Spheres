    ...
    for(const step of manifest.steps){
      if(step.command){mark(step.command);const command=manifest.commands[step.command];assert(command);await review(page,url,command);
        ...
        if(step.command==='support_army'){
          ...
          await within(page.locator('#flightMapCard'),'Mission target map');await mapShot('support-army-target-map-1440');await page.setViewportSize({width:390,height:844});await within(page.locator('#flightMapCard'),'Narrow mission target map');await mapShot('support-army-target-map-390');await page.setViewportSize({width:1440,height:1000});
        ...
        if(['foreign_transit','returned_ready'].includes(step.checkpoint)){
          ...
          await page.locator('[data-equipment-action='+q(map.path)+']').click();await page.locator('#flightMapCard').waitFor({state:'visible'});await within(page.locator('#flightMapCard'),'Flight map explanation');await mapShot(step.checkpoint+'-map-1440');
          await page.setViewportSize({width:390,height:844});await within(page.locator('#flightMapCard'),'Narrow flight map');await mapShot(step.checkpoint+'-map-390');await page.setViewportSize({width:1440,height:1000});await page.locator('#flightMapCard button').filter({hasText:'Back to Air Force'}).click();await page.waitForFunction(()=>equipmentCurrent()&&EQUIP.tab==='flight');
        }
