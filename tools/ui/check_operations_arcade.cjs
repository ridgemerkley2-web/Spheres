...
const helpers = [
  ...
  'constructionProjectRole','productionFundingLabel', 'productionCatalogHtml', 'productionEligible', 'productionProvinceHtml',
...
test('empty manufacturing gives a useful next step and does not invent free plant capacity', () => {
  ...
  assert.match(html,/data-manu-build-plant/);assert.match(html,/data-manu-new>Browse equipment catalogue/);
  assert.doesNotMatch(html,/Start equipment line/);
  run(c,`MANU.data.catalog=[{id:'arm_gen3',name:'Third-Generation Armour',class:'Armour',unlocked:true,actions:{start:false},eligible_provinces:[]}];`);
  const catalog=run(c,'manufacturingCatalogHtml()');
  assert.match(catalog,/Third-Generation Armour/);
