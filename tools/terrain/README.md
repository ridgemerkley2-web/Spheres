...
This is the original coarse-map pipeline. **Do not run it to refresh the current
detailed shorelines:** `make_rivers.py` would replace them with its older 0.4-unit
simplification and also rewrite simulation river inputs. The six lake-surface
records can be checked independently with `make_lake_surfaces.py --check`;
see the detailed shoreline tolerance under *Determinism* below.
...
The detailed lake paths integrated in S04 were produced by the shoreline pass
in commit `541a6b11`, at **0.012 canvas units** (about 200 m at the equator).
`make_lake_surfaces.py` explicitly uses that tolerance when matching the six
HydroLAKES records to the shipped paths. The original river generator's default
remains 0.4; running it by itself would coarsen the lake shores. The surface
generator only writes the elevation/provenance table and refuses missing,
ambiguous or changed shores; it never rewrites rivers or simulation inputs.
Run its strict `--check` with the two source downloads described in its help,
and `python -B tools/terrain/test_lake_surfaces.py` for source-independent
detail and identity regressions.

`terrain-tiles/embed.rs` is checked out with LF endings through
`.gitattributes`, because the tile generator compares its generated bytes
exactly. Strict PNG reproduction also requires the recorded encoder versions
(currently Pillow 12.3.0, NumPy 2.3.5 and h5py 3.16.0); identical pixels encoded
by a different Pillow version are not a byte-for-byte reproduction.
